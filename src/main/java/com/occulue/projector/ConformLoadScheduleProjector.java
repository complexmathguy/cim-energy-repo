...
/**
 * Projector for ConformLoadSchedule as outlined for the CQRS pattern. All event handling and query
 * handling related to ConformLoadSchedule are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class ConformLoadScheduleProjector extends ConformLoadScheduleEntityProjector {
  ...
  // core constructor
  public ConformLoadScheduleProjector(
      ConformLoadScheduleRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<ConformLoadSchedule> handle(FindAllConformLoadScheduleQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindConformLoadSchedule, but only if the id matches
   *
  ...
  protected void emitFindConformLoadSchedule(ConformLoadSchedule entity) {
    ...
    queryUpdateEmitter.emit(
        FindConformLoadScheduleQuery.class,
        query ->
            query.getFilter().getConformLoadScheduleId().equals(entity.getConformLoadScheduleId()),
        entity);
  }
  ...
  protected void emitFindAllConformLoadSchedule(ConformLoadSchedule entity) {
    ...
    queryUpdateEmitter.emit(FindAllConformLoadScheduleQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConformLoadScheduleProjector.class.getName());
}
