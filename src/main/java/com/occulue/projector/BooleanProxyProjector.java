...
/**
 * Projector for BooleanProxy as outlined for the CQRS pattern. All event handling and query
 * handling related to BooleanProxy are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class BooleanProxyProjector extends BooleanProxyEntityProjector {
  ...
  // core constructor
  public BooleanProxyProjector(
      BooleanProxyRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindBooleanProxy, but only if the id matches
   *
  ...
  protected void emitFindAllBooleanProxy(BooleanProxy entity) {
    ...
    queryUpdateEmitter.emit(FindAllBooleanProxyQuery.class, query -> true, entity);
  }
