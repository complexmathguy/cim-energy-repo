...
public class NonlinearShuntCompensatorEntityProjector {
  ...
  public NonlinearShuntCompensator find(UUID id) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find a NonlinearShuntCompensator - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  public List<NonlinearShuntCompensator> findAll(FindAllNonlinearShuntCompensatorQuery query) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find all NonlinearShuntCompensator - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final NonlinearShuntCompensatorRepository repository;

  private static final Logger LOGGER =
      Logger.getLogger(NonlinearShuntCompensatorEntityProjector.class.getName());
}
