...
/**
 * Projector for WindAeroConstIEC as outlined for the CQRS pattern. All event handling and query
 * handling related to WindAeroConstIEC are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by WindAeroConstIECAggregate
 *
...
public class WindAeroConstIECProjector extends WindAeroConstIECEntityProjector {
  ...
  // core constructor
  public WindAeroConstIECProjector(
      WindAeroConstIECRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<WindAeroConstIEC> handle(FindAllWindAeroConstIECQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindWindAeroConstIEC, but only if the id matches
   *
  ...
  protected void emitFindWindAeroConstIEC(WindAeroConstIEC entity) {
    ...
    queryUpdateEmitter.emit(
        FindWindAeroConstIECQuery.class,
  ...
  protected void emitFindAllWindAeroConstIEC(WindAeroConstIEC entity) {
    ...
    queryUpdateEmitter.emit(FindAllWindAeroConstIECQuery.class, query -> true, entity);
  }
