...
/**
 * Projector for Susceptance as outlined for the CQRS pattern. All event handling and query handling
 * related to Susceptance are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class SusceptanceProjector extends SusceptanceEntityProjector {
  ...
  // core constructor
  public SusceptanceProjector(
      SusceptanceRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToSusceptanceEvent
   */
  @EventHandler(payloadType = AssignValueToSusceptanceEvent.class)
  public void handle(AssignValueToSusceptanceEvent event) {
    LOGGER.info("handling AssignValueToSusceptanceEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Susceptance entity = assignValue(event.getSusceptanceId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromSusceptanceEvent
   */
  @EventHandler(payloadType = UnAssignValueFromSusceptanceEvent.class)
  public void handle(UnAssignValueFromSusceptanceEvent event) {
    LOGGER.info("handling UnAssignValueFromSusceptanceEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Susceptance entity = unAssignValue(event.getSusceptanceId());
  ...
  /**
   * emit to subscription queries of type FindSusceptance, but only if the id matches
   *
  ...
  protected void emitFindAllSusceptance(Susceptance entity) {
    ...
    queryUpdateEmitter.emit(FindAllSusceptanceQuery.class, query -> true, entity);
  }
