...
/**
 * Projector for RegulatingControl as outlined for the CQRS pattern. All event handling and query
 * handling related to RegulatingControl are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class RegulatingControlProjector extends RegulatingControlEntityProjector {
  ...
  // core constructor
  public RegulatingControlProjector(
      RegulatingControlRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<RegulatingControl> handle(FindAllRegulatingControlQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindRegulatingControl, but only if the id matches
   *
  ...
  protected void emitFindRegulatingControl(RegulatingControl entity) {
    ...
    queryUpdateEmitter.emit(
        FindRegulatingControlQuery.class,
  ...
  protected void emitFindAllRegulatingControl(RegulatingControl entity) {
    ...
    queryUpdateEmitter.emit(FindAllRegulatingControlQuery.class, query -> true, entity);
  }
