...
/**
 * Projector for ActivePower as outlined for the CQRS pattern. All event handling and query handling
 * related to ActivePower are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class ActivePowerProjector extends ActivePowerEntityProjector {
  ...
  // core constructor
  public ActivePowerProjector(
      ActivePowerRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToActivePowerEvent
   */
  @EventHandler(payloadType = AssignValueToActivePowerEvent.class)
  public void handle(AssignValueToActivePowerEvent event) {
    LOGGER.info("handling AssignValueToActivePowerEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    ActivePower entity = assignValue(event.getActivePowerId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromActivePowerEvent
   */
  @EventHandler(payloadType = UnAssignValueFromActivePowerEvent.class)
  public void handle(UnAssignValueFromActivePowerEvent event) {
    LOGGER.info("handling UnAssignValueFromActivePowerEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    ActivePower entity = unAssignValue(event.getActivePowerId());
  ...
  /**
   * emit to subscription queries of type FindActivePower, but only if the id matches
   *
  ...
  protected void emitFindAllActivePower(ActivePower entity) {
    ...
    queryUpdateEmitter.emit(FindAllActivePowerQuery.class, query -> true, entity);
  }
