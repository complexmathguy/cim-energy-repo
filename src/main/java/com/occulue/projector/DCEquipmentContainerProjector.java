...
/**
 * Projector for DCEquipmentContainer as outlined for the CQRS pattern. All event handling and query
 * handling related to DCEquipmentContainer are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by DCEquipmentContainerAggregate
 *
...
public class DCEquipmentContainerProjector extends DCEquipmentContainerEntityProjector {
  ...
  // core constructor
  public DCEquipmentContainerProjector(
      DCEquipmentContainerRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<DCEquipmentContainer> handle(FindAllDCEquipmentContainerQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindDCEquipmentContainer, but only if the id matches
   *
  ...
  protected void emitFindDCEquipmentContainer(DCEquipmentContainer entity) {
    ...
    queryUpdateEmitter.emit(
        FindDCEquipmentContainerQuery.class,
        query ->
            query
                .getFilter()
                .getDCEquipmentContainerId()
                .equals(entity.getDCEquipmentContainerId()),
        entity);
  }
  ...
  protected void emitFindAllDCEquipmentContainer(DCEquipmentContainer entity) {
    ...
    queryUpdateEmitter.emit(FindAllDCEquipmentContainerQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DCEquipmentContainerProjector.class.getName());
}
