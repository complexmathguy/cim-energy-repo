...
/**
 * Projector for AnalogLimitSet as outlined for the CQRS pattern. All event handling and query
 * handling related to AnalogLimitSet are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class AnalogLimitSetProjector extends AnalogLimitSetEntityProjector {
  ...
  // core constructor
  public AnalogLimitSetProjector(
      AnalogLimitSetRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindAnalogLimitSet, but only if the id matches
   *
  ...
  protected void emitFindAllAnalogLimitSet(AnalogLimitSet entity) {
    ...
    queryUpdateEmitter.emit(FindAllAnalogLimitSetQuery.class, query -> true, entity);
  }
