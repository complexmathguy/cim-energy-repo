...
/**
 * Projector for DateProxy as outlined for the CQRS pattern. All event handling and query handling
 * related to DateProxy are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class DateProxyProjector extends DateProxyEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindDateProxy, but only if the id matches
   *
  ...
  protected void emitFindAllDateProxy(DateProxy entity) {
    ...
    queryUpdateEmitter.emit(FindAllDateProxyQuery.class, query -> true, entity);
  }
