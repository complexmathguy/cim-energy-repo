...
/**
 * Projector for Season as outlined for the CQRS pattern. All event handling and query handling
 * related to Season are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class SeasonProjector extends SeasonEntityProjector {
  ...
  /*
   * @param	event AssignEndDateToSeasonEvent
   */
  @EventHandler(payloadType = AssignEndDateToSeasonEvent.class)
  public void handle(AssignEndDateToSeasonEvent event) {
    LOGGER.info("handling AssignEndDateToSeasonEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Season entity = assignEndDate(event.getSeasonId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignEndDateFromSeasonEvent
   */
  @EventHandler(payloadType = UnAssignEndDateFromSeasonEvent.class)
  public void handle(UnAssignEndDateFromSeasonEvent event) {
    LOGGER.info("handling UnAssignEndDateFromSeasonEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Season entity = unAssignEndDate(event.getSeasonId());
  ...
  /*
   * @param	event AssignStartDateToSeasonEvent
   */
  @EventHandler(payloadType = AssignStartDateToSeasonEvent.class)
  public void handle(AssignStartDateToSeasonEvent event) {
    LOGGER.info("handling AssignStartDateToSeasonEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Season entity = assignStartDate(event.getSeasonId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignStartDateFromSeasonEvent
   */
  @EventHandler(payloadType = UnAssignStartDateFromSeasonEvent.class)
  public void handle(UnAssignStartDateFromSeasonEvent event) {
    LOGGER.info("handling UnAssignStartDateFromSeasonEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Season entity = unAssignStartDate(event.getSeasonId());
  ...
  @QueryHandler
  public Season handle(FindSeasonQuery query) throws ProcessingException, IllegalArgumentException {
  ...
  /**
   * emit to subscription queries of type FindSeason, but only if the id matches
   *
   * @param entity Season
   */
  ...
  protected void emitFindAllSeason(Season entity) {
    ...
    queryUpdateEmitter.emit(FindAllSeasonQuery.class, query -> true, entity);
  }
