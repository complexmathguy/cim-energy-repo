...
/**
 * Projector for OperationalLimitType as outlined for the CQRS pattern. All event handling and query
 * handling related to OperationalLimitType are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class OperationalLimitTypeProjector extends OperationalLimitTypeEntityProjector {
  ...
  // core constructor
  public OperationalLimitTypeProjector(
      OperationalLimitTypeRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignAcceptableDurationToOperationalLimitTypeEvent
   */
  @EventHandler(payloadType = AssignAcceptableDurationToOperationalLimitTypeEvent.class)
  public void handle(AssignAcceptableDurationToOperationalLimitTypeEvent event) {
    LOGGER.info("handling AssignAcceptableDurationToOperationalLimitTypeEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    OperationalLimitType entity =
        assignAcceptableDuration(event.getOperationalLimitTypeId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignAcceptableDurationFromOperationalLimitTypeEvent
   */
  @EventHandler(payloadType = UnAssignAcceptableDurationFromOperationalLimitTypeEvent.class)
  public void handle(UnAssignAcceptableDurationFromOperationalLimitTypeEvent event) {
    LOGGER.info("handling UnAssignAcceptableDurationFromOperationalLimitTypeEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    OperationalLimitType entity = unAssignAcceptableDuration(event.getOperationalLimitTypeId());
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<OperationalLimitType> handle(FindAllOperationalLimitTypeQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindOperationalLimitType, but only if the id matches
   *
  ...
  protected void emitFindOperationalLimitType(OperationalLimitType entity) {
    ...
    queryUpdateEmitter.emit(
        FindOperationalLimitTypeQuery.class,
        query ->
            query
                .getFilter()
                .getOperationalLimitTypeId()
                .equals(entity.getOperationalLimitTypeId()),
        entity);
  }
  ...
  protected void emitFindAllOperationalLimitType(OperationalLimitType entity) {
    ...
    queryUpdateEmitter.emit(FindAllOperationalLimitTypeQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(OperationalLimitTypeProjector.class.getName());
}
