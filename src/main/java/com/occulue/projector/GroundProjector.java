...
/**
 * Projector for Ground as outlined for the CQRS pattern. All event handling and query handling
 * related to Ground are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by GroundAggregate
 *
...
public class GroundProjector extends GroundEntityProjector {
  ...
  @QueryHandler
  public Ground handle(FindGroundQuery query) throws ProcessingException, IllegalArgumentException {
  ...
  /**
   * emit to subscription queries of type FindGround, but only if the id matches
   *
   * @param entity Ground
   */
  ...
  protected void emitFindAllGround(Ground entity) {
    ...
    queryUpdateEmitter.emit(FindAllGroundQuery.class, query -> true, entity);
  }
