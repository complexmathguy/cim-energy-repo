...
/**
 * Projector for Limit as outlined for the CQRS pattern. All event handling and query handling
 * related to Limit are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by LimitAggregate
 *
...
public class LimitProjector extends LimitEntityProjector {
  ...
  @QueryHandler
  public Limit handle(FindLimitQuery query) throws ProcessingException, IllegalArgumentException {
  ...
  /**
   * emit to subscription queries of type FindLimit, but only if the id matches
   *
   * @param entity Limit
   */
  ...
  protected void emitFindAllLimit(Limit entity) {
    ...
    queryUpdateEmitter.emit(FindAllLimitQuery.class, query -> true, entity);
  }
