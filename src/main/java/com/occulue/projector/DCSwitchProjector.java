...
/**
 * Projector for DCSwitch as outlined for the CQRS pattern. All event handling and query handling
 * related to DCSwitch are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class DCSwitchProjector extends DCSwitchEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindDCSwitch, but only if the id matches
   *
  ...
  protected void emitFindAllDCSwitch(DCSwitch entity) {
    ...
    queryUpdateEmitter.emit(FindAllDCSwitchQuery.class, query -> true, entity);
  }
