...
/**
 * Projector for Inductance as outlined for the CQRS pattern. All event handling and query handling
 * related to Inductance are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class InductanceProjector extends InductanceEntityProjector {
  ...
  // core constructor
  public InductanceProjector(
      InductanceRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToInductanceEvent
   */
  @EventHandler(payloadType = AssignValueToInductanceEvent.class)
  public void handle(AssignValueToInductanceEvent event) {
    LOGGER.info("handling AssignValueToInductanceEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Inductance entity = assignValue(event.getInductanceId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromInductanceEvent
   */
  @EventHandler(payloadType = UnAssignValueFromInductanceEvent.class)
  public void handle(UnAssignValueFromInductanceEvent event) {
    LOGGER.info("handling UnAssignValueFromInductanceEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Inductance entity = unAssignValue(event.getInductanceId());
  ...
  /**
   * emit to subscription queries of type FindInductance, but only if the id matches
   *
  ...
  protected void emitFindAllInductance(Inductance entity) {
    ...
    queryUpdateEmitter.emit(FindAllInductanceQuery.class, query -> true, entity);
  }
