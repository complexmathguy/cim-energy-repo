...
/**
 * Projector for DiagramObjectGluePoint as outlined for the CQRS pattern. All event handling and
 * query handling related to DiagramObjectGluePoint are invoked here and dispersed as an event to be
 * handled elsewhere.
 *
 * <p>Commands are handled by DiagramObjectGluePointAggregate
 *
...
public class DiagramObjectGluePointProjector extends DiagramObjectGluePointEntityProjector {
  ...
  // core constructor
  public DiagramObjectGluePointProjector(
      DiagramObjectGluePointRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<DiagramObjectGluePoint> handle(FindAllDiagramObjectGluePointQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindDiagramObjectGluePoint, but only if the id matches
   *
  ...
  protected void emitFindDiagramObjectGluePoint(DiagramObjectGluePoint entity) {
    ...
    queryUpdateEmitter.emit(
        FindDiagramObjectGluePointQuery.class,
        query ->
            query
                .getFilter()
                .getDiagramObjectGluePointId()
                .equals(entity.getDiagramObjectGluePointId()),
        entity);
  }
  ...
  protected void emitFindAllDiagramObjectGluePoint(DiagramObjectGluePoint entity) {
    ...
    queryUpdateEmitter.emit(FindAllDiagramObjectGluePointQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramObjectGluePointProjector.class.getName());
}
