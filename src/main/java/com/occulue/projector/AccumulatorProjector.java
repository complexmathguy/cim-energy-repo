...
/**
 * Projector for Accumulator as outlined for the CQRS pattern. All event handling and query handling
 * related to Accumulator are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by AccumulatorAggregate
 *
...
public class AccumulatorProjector extends AccumulatorEntityProjector {
  ...
  // core constructor
  public AccumulatorProjector(
      AccumulatorRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindAccumulator, but only if the id matches
   *
  ...
  protected void emitFindAllAccumulator(Accumulator entity) {
    ...
    queryUpdateEmitter.emit(FindAllAccumulatorQuery.class, query -> true, entity);
  }
