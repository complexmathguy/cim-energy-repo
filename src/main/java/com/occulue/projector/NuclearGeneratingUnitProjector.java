...
/**
 * Projector for NuclearGeneratingUnit as outlined for the CQRS pattern. All event handling and
 * query handling related to NuclearGeneratingUnit are invoked here and dispersed as an event to be
 * handled elsewhere.
 *
 * <p>Commands are handled by NuclearGeneratingUnitAggregate
 *
...
public class NuclearGeneratingUnitProjector extends NuclearGeneratingUnitEntityProjector {
  ...
  // core constructor
  public NuclearGeneratingUnitProjector(
      NuclearGeneratingUnitRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<NuclearGeneratingUnit> handle(FindAllNuclearGeneratingUnitQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindNuclearGeneratingUnit, but only if the id matches
   *
  ...
  protected void emitFindNuclearGeneratingUnit(NuclearGeneratingUnit entity) {
    ...
    queryUpdateEmitter.emit(
        FindNuclearGeneratingUnitQuery.class,
        query ->
            query
                .getFilter()
                .getNuclearGeneratingUnitId()
                .equals(entity.getNuclearGeneratingUnitId()),
        entity);
  }
  ...
  protected void emitFindAllNuclearGeneratingUnit(NuclearGeneratingUnit entity) {
    ...
    queryUpdateEmitter.emit(FindAllNuclearGeneratingUnitQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NuclearGeneratingUnitProjector.class.getName());
}
