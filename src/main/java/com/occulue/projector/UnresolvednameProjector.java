...
/**
 * Projector for Unresolvedname as outlined for the CQRS pattern. All event handling and query
 * handling related to Unresolvedname are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class UnresolvednameProjector extends UnresolvednameEntityProjector {
  ...
  // core constructor
  public UnresolvednameProjector(
      UnresolvednameRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindUnresolvedname, but only if the id matches
   *
  ...
  protected void emitFindAllUnresolvedname(Unresolvedname entity) {
    ...
    queryUpdateEmitter.emit(FindAllUnresolvednameQuery.class, query -> true, entity);
  }
