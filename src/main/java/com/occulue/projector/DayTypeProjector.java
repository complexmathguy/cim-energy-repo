...
/**
 * Projector for DayType as outlined for the CQRS pattern. All event handling and query handling
 * related to DayType are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class DayTypeProjector extends DayTypeEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindDayType, but only if the id matches
   *
  ...
  protected void emitFindAllDayType(DayType entity) {
    ...
    queryUpdateEmitter.emit(FindAllDayTypeQuery.class, query -> true, entity);
  }
