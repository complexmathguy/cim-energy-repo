...
/**
 * Projector for ENTSOETopologicalNode as outlined for the CQRS pattern. All event handling and
 * query handling related to ENTSOETopologicalNode are invoked here and dispersed as an event to be
 * handled elsewhere.
 *
 * <p>Commands are handled by ENTSOETopologicalNodeAggregate
 *
...
public class ENTSOETopologicalNodeProjector extends ENTSOETopologicalNodeEntityProjector {
  ...
  // core constructor
  public ENTSOETopologicalNodeProjector(
      ENTSOETopologicalNodeRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<ENTSOETopologicalNode> handle(FindAllENTSOETopologicalNodeQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindENTSOETopologicalNode, but only if the id matches
   *
  ...
  protected void emitFindENTSOETopologicalNode(ENTSOETopologicalNode entity) {
    ...
    queryUpdateEmitter.emit(
        FindENTSOETopologicalNodeQuery.class,
        query ->
            query
                .getFilter()
                .getENTSOETopologicalNodeId()
                .equals(entity.getENTSOETopologicalNodeId()),
        entity);
  }
  ...
  protected void emitFindAllENTSOETopologicalNode(ENTSOETopologicalNode entity) {
    ...
    queryUpdateEmitter.emit(FindAllENTSOETopologicalNodeQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ENTSOETopologicalNodeProjector.class.getName());
}
