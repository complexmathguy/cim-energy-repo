...
/**
 * Projector for PhaseTapChangerSymmetrical as outlined for the CQRS pattern. All event handling and
 * query handling related to PhaseTapChangerSymmetrical are invoked here and dispersed as an event
 * to be handled elsewhere.
 *
 * <p>Commands are handled by PhaseTapChangerSymmetricalAggregate
 *
...
public class PhaseTapChangerSymmetricalProjector extends PhaseTapChangerSymmetricalEntityProjector {
  ...
  // core constructor
  public PhaseTapChangerSymmetricalProjector(
      PhaseTapChangerSymmetricalRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<PhaseTapChangerSymmetrical> handle(FindAllPhaseTapChangerSymmetricalQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindPhaseTapChangerSymmetrical, but only if the id matches
   *
  ...
  protected void emitFindPhaseTapChangerSymmetrical(PhaseTapChangerSymmetrical entity) {
    ...
    queryUpdateEmitter.emit(
        FindPhaseTapChangerSymmetricalQuery.class,
        query ->
            query
                .getFilter()
                .getPhaseTapChangerSymmetricalId()
                .equals(entity.getPhaseTapChangerSymmetricalId()),
        entity);
  }
  ...
  protected void emitFindAllPhaseTapChangerSymmetrical(PhaseTapChangerSymmetrical entity) {
    ...
    queryUpdateEmitter.emit(FindAllPhaseTapChangerSymmetricalQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerSymmetricalProjector.class.getName());
}
