...
/**
 * Projector for TextDiagramObject as outlined for the CQRS pattern. All event handling and query
 * handling related to TextDiagramObject are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class TextDiagramObjectProjector extends TextDiagramObjectEntityProjector {
  ...
  // core constructor
  public TextDiagramObjectProjector(
      TextDiagramObjectRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignTextToTextDiagramObjectEvent
   */
  @EventHandler(payloadType = AssignTextToTextDiagramObjectEvent.class)
  public void handle(AssignTextToTextDiagramObjectEvent event) {
    LOGGER.info("handling AssignTextToTextDiagramObjectEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    TextDiagramObject entity = assignText(event.getTextDiagramObjectId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignTextFromTextDiagramObjectEvent
   */
  @EventHandler(payloadType = UnAssignTextFromTextDiagramObjectEvent.class)
  public void handle(UnAssignTextFromTextDiagramObjectEvent event) {
    LOGGER.info("handling UnAssignTextFromTextDiagramObjectEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    TextDiagramObject entity = unAssignText(event.getTextDiagramObjectId());
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<TextDiagramObject> handle(FindAllTextDiagramObjectQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindTextDiagramObject, but only if the id matches
   *
  ...
  protected void emitFindTextDiagramObject(TextDiagramObject entity) {
    ...
    queryUpdateEmitter.emit(
        FindTextDiagramObjectQuery.class,
  ...
  protected void emitFindAllTextDiagramObject(TextDiagramObject entity) {
    ...
    queryUpdateEmitter.emit(FindAllTextDiagramObjectQuery.class, query -> true, entity);
  }
