...
/**
 * Projector for TapSchedule as outlined for the CQRS pattern. All event handling and query handling
 * related to TapSchedule are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class TapScheduleProjector extends TapScheduleEntityProjector {
  ...
  // core constructor
  public TapScheduleProjector(
      TapScheduleRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindTapSchedule, but only if the id matches
   *
  ...
  protected void emitFindAllTapSchedule(TapSchedule entity) {
    ...
    queryUpdateEmitter.emit(FindAllTapScheduleQuery.class, query -> true, entity);
  }
