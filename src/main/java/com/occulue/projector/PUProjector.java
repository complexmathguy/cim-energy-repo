...
/**
 * Projector for PU as outlined for the CQRS pattern. All event handling and query handling related
 * to PU are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class PUProjector extends PUEntityProjector {
  ...
  /*
   * @param	event AssignValueToPUEvent
   */
  @EventHandler(payloadType = AssignValueToPUEvent.class)
  public void handle(AssignValueToPUEvent event) {
    LOGGER.info("handling AssignValueToPUEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    PU entity = assignValue(event.getPUId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromPUEvent
   */
  @EventHandler(payloadType = UnAssignValueFromPUEvent.class)
  public void handle(UnAssignValueFromPUEvent event) {
    LOGGER.info("handling UnAssignValueFromPUEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    PU entity = unAssignValue(event.getPUId());
  ...
  @QueryHandler
  public PU handle(FindPUQuery query) throws ProcessingException, IllegalArgumentException {
  ...
  /**
   * emit to subscription queries of type FindPU, but only if the id matches
   *
   * @param entity PU
   */
  protected void emitFindPU(PU entity) {
    ...
    queryUpdateEmitter.emit(
        FindPUQuery.class, query -> query.getFilter().getPUId().equals(entity.getPUId()), entity);
  }
  ...
  protected void emitFindAllPU(PU entity) {
    ...
    queryUpdateEmitter.emit(FindAllPUQuery.class, query -> true, entity);
  }
