...
/**
 * Projector for Connector as outlined for the CQRS pattern. All event handling and query handling
 * related to Connector are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by ConnectorAggregate
 *
...
public class ConnectorProjector extends ConnectorEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindConnector, but only if the id matches
   *
  ...
  protected void emitFindAllConnector(Connector entity) {
    ...
    queryUpdateEmitter.emit(FindAllConnectorQuery.class, query -> true, entity);
  }
