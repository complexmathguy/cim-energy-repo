...
/**
 * Projector for EquivalentNetwork as outlined for the CQRS pattern. All event handling and query
 * handling related to EquivalentNetwork are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by EquivalentNetworkAggregate
 *
...
public class EquivalentNetworkProjector extends EquivalentNetworkEntityProjector {
  ...
  // core constructor
  public EquivalentNetworkProjector(
      EquivalentNetworkRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<EquivalentNetwork> handle(FindAllEquivalentNetworkQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindEquivalentNetwork, but only if the id matches
   *
  ...
  protected void emitFindEquivalentNetwork(EquivalentNetwork entity) {
    ...
    queryUpdateEmitter.emit(
        FindEquivalentNetworkQuery.class,
  ...
  protected void emitFindAllEquivalentNetwork(EquivalentNetwork entity) {
    ...
    queryUpdateEmitter.emit(FindAllEquivalentNetworkQuery.class, query -> true, entity);
  }
