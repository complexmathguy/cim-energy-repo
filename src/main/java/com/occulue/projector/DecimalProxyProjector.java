...
/**
 * Projector for DecimalProxy as outlined for the CQRS pattern. All event handling and query
 * handling related to DecimalProxy are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class DecimalProxyProjector extends DecimalProxyEntityProjector {
  ...
  // core constructor
  public DecimalProxyProjector(
      DecimalProxyRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindDecimalProxy, but only if the id matches
   *
  ...
  protected void emitFindAllDecimalProxy(DecimalProxy entity) {
    ...
    queryUpdateEmitter.emit(FindAllDecimalProxyQuery.class, query -> true, entity);
  }
