...
/**
 * Projector for IntegerProxy as outlined for the CQRS pattern. All event handling and query
 * handling related to IntegerProxy are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class IntegerProxyProjector extends IntegerProxyEntityProjector {
  ...
  // core constructor
  public IntegerProxyProjector(
      IntegerProxyRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindIntegerProxy, but only if the id matches
   *
  ...
  protected void emitFindAllIntegerProxy(IntegerProxy entity) {
    ...
    queryUpdateEmitter.emit(FindAllIntegerProxyQuery.class, query -> true, entity);
  }
