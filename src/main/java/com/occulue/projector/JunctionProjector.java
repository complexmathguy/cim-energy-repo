...
/**
 * Projector for Junction as outlined for the CQRS pattern. All event handling and query handling
 * related to Junction are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by JunctionAggregate
 *
...
public class JunctionProjector extends JunctionEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindJunction, but only if the id matches
   *
  ...
  protected void emitFindAllJunction(Junction entity) {
    ...
    queryUpdateEmitter.emit(FindAllJunctionQuery.class, query -> true, entity);
  }
