...
/**
 * Projector for LoadArea as outlined for the CQRS pattern. All event handling and query handling
 * related to LoadArea are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by LoadAreaAggregate
 *
...
public class LoadAreaProjector extends LoadAreaEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindLoadArea, but only if the id matches
   *
  ...
  protected void emitFindAllLoadArea(LoadArea entity) {
    ...
    queryUpdateEmitter.emit(FindAllLoadAreaQuery.class, query -> true, entity);
  }
