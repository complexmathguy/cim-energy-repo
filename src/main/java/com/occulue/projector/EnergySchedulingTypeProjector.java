...
/**
 * Projector for EnergySchedulingType as outlined for the CQRS pattern. All event handling and query
 * handling related to EnergySchedulingType are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by EnergySchedulingTypeAggregate
 *
...
public class EnergySchedulingTypeProjector extends EnergySchedulingTypeEntityProjector {
  ...
  // core constructor
  public EnergySchedulingTypeProjector(
      EnergySchedulingTypeRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<EnergySchedulingType> handle(FindAllEnergySchedulingTypeQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindEnergySchedulingType, but only if the id matches
   *
  ...
  protected void emitFindEnergySchedulingType(EnergySchedulingType entity) {
    ...
    queryUpdateEmitter.emit(
        FindEnergySchedulingTypeQuery.class,
        query ->
            query
                .getFilter()
                .getEnergySchedulingTypeId()
                .equals(entity.getEnergySchedulingTypeId()),
        entity);
  }
  ...
  protected void emitFindAllEnergySchedulingType(EnergySchedulingType entity) {
    ...
    queryUpdateEmitter.emit(FindAllEnergySchedulingTypeQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EnergySchedulingTypeProjector.class.getName());
}
