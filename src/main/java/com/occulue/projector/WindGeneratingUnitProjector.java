...
/**
 * Projector for WindGeneratingUnit as outlined for the CQRS pattern. All event handling and query
 * handling related to WindGeneratingUnit are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by WindGeneratingUnitAggregate
 *
...
public class WindGeneratingUnitProjector extends WindGeneratingUnitEntityProjector {
  ...
  // core constructor
  public WindGeneratingUnitProjector(
      WindGeneratingUnitRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<WindGeneratingUnit> handle(FindAllWindGeneratingUnitQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindWindGeneratingUnit, but only if the id matches
   *
  ...
  protected void emitFindWindGeneratingUnit(WindGeneratingUnit entity) {
    ...
    queryUpdateEmitter.emit(
        FindWindGeneratingUnitQuery.class,
        query ->
            query.getFilter().getWindGeneratingUnitId().equals(entity.getWindGeneratingUnitId()),
        entity);
  }
  ...
  protected void emitFindAllWindGeneratingUnit(WindGeneratingUnit entity) {
    ...
    queryUpdateEmitter.emit(FindAllWindGeneratingUnitQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(WindGeneratingUnitProjector.class.getName());
}
