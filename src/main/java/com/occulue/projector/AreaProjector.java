...
/**
 * Projector for Area as outlined for the CQRS pattern. All event handling and query handling
 * related to Area are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class AreaProjector extends AreaEntityProjector {
  ...
  /*
   * @param	event AssignValueToAreaEvent
   */
  @EventHandler(payloadType = AssignValueToAreaEvent.class)
  public void handle(AssignValueToAreaEvent event) {
    LOGGER.info("handling AssignValueToAreaEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Area entity = assignValue(event.getAreaId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromAreaEvent
   */
  @EventHandler(payloadType = UnAssignValueFromAreaEvent.class)
  public void handle(UnAssignValueFromAreaEvent event) {
    LOGGER.info("handling UnAssignValueFromAreaEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Area entity = unAssignValue(event.getAreaId());
  ...
  @QueryHandler
  public Area handle(FindAreaQuery query) throws ProcessingException, IllegalArgumentException {
  ...
  /**
   * emit to subscription queries of type FindArea, but only if the id matches
   *
   * @param entity Area
   */
  ...
  protected void emitFindAllArea(Area entity) {
    ...
    queryUpdateEmitter.emit(FindAllAreaQuery.class, query -> true, entity);
  }
