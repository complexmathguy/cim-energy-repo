...
/**
 * Projector for ThermalGeneratingUnit as outlined for the CQRS pattern. All event handling and
 * query handling related to ThermalGeneratingUnit are invoked here and dispersed as an event to be
 * handled elsewhere.
 *
 * <p>Commands are handled by ThermalGeneratingUnitAggregate
 *
...
public class ThermalGeneratingUnitProjector extends ThermalGeneratingUnitEntityProjector {
  ...
  // core constructor
  public ThermalGeneratingUnitProjector(
      ThermalGeneratingUnitRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<ThermalGeneratingUnit> handle(FindAllThermalGeneratingUnitQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindThermalGeneratingUnit, but only if the id matches
   *
  ...
  protected void emitFindThermalGeneratingUnit(ThermalGeneratingUnit entity) {
    ...
    queryUpdateEmitter.emit(
        FindThermalGeneratingUnitQuery.class,
        query ->
            query
                .getFilter()
                .getThermalGeneratingUnitId()
                .equals(entity.getThermalGeneratingUnitId()),
        entity);
  }
  ...
  protected void emitFindAllThermalGeneratingUnit(ThermalGeneratingUnit entity) {
    ...
    queryUpdateEmitter.emit(FindAllThermalGeneratingUnitQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ThermalGeneratingUnitProjector.class.getName());
}
