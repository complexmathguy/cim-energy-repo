...
/**
 * Projector for ReportingGroup as outlined for the CQRS pattern. All event handling and query
 * handling related to ReportingGroup are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class ReportingGroupProjector extends ReportingGroupEntityProjector {
  ...
  // core constructor
  public ReportingGroupProjector(
      ReportingGroupRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindReportingGroup, but only if the id matches
   *
  ...
  protected void emitFindAllReportingGroup(ReportingGroup entity) {
    ...
    queryUpdateEmitter.emit(FindAllReportingGroupQuery.class, query -> true, entity);
  }
