...
/**
 * Projector for Terminal as outlined for the CQRS pattern. All event handling and query handling
 * related to Terminal are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class TerminalProjector extends TerminalEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindTerminal, but only if the id matches
   *
  ...
  protected void emitFindAllTerminal(Terminal entity) {
    ...
    queryUpdateEmitter.emit(FindAllTerminalQuery.class, query -> true, entity);
  }
