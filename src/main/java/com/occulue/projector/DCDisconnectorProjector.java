...
/**
 * Projector for DCDisconnector as outlined for the CQRS pattern. All event handling and query
 * handling related to DCDisconnector are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by DCDisconnectorAggregate
 *
...
public class DCDisconnectorProjector extends DCDisconnectorEntityProjector {
  ...
  // core constructor
  public DCDisconnectorProjector(
      DCDisconnectorRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindDCDisconnector, but only if the id matches
   *
  ...
  protected void emitFindAllDCDisconnector(DCDisconnector entity) {
    ...
    queryUpdateEmitter.emit(FindAllDCDisconnectorQuery.class, query -> true, entity);
  }
