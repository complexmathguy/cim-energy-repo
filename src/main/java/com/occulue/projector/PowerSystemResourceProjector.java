...
/**
 * Projector for PowerSystemResource as outlined for the CQRS pattern. All event handling and query
 * handling related to PowerSystemResource are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by PowerSystemResourceAggregate
 *
...
public class PowerSystemResourceProjector extends PowerSystemResourceEntityProjector {
  ...
  // core constructor
  public PowerSystemResourceProjector(
      PowerSystemResourceRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<PowerSystemResource> handle(FindAllPowerSystemResourceQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindPowerSystemResource, but only if the id matches
   *
  ...
  protected void emitFindPowerSystemResource(PowerSystemResource entity) {
    ...
    queryUpdateEmitter.emit(
        FindPowerSystemResourceQuery.class,
        query ->
            query.getFilter().getPowerSystemResourceId().equals(entity.getPowerSystemResourceId()),
        entity);
  }
  ...
  protected void emitFindAllPowerSystemResource(PowerSystemResource entity) {
    ...
    queryUpdateEmitter.emit(FindAllPowerSystemResourceQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PowerSystemResourceProjector.class.getName());
}
