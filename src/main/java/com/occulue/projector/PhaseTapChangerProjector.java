...
/**
 * Projector for PhaseTapChanger as outlined for the CQRS pattern. All event handling and query
 * handling related to PhaseTapChanger are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class PhaseTapChangerProjector extends PhaseTapChangerEntityProjector {
  ...
  // core constructor
  public PhaseTapChangerProjector(
      PhaseTapChangerRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<PhaseTapChanger> handle(FindAllPhaseTapChangerQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindPhaseTapChanger, but only if the id matches
   *
  ...
  protected void emitFindPhaseTapChanger(PhaseTapChanger entity) {
    ...
    queryUpdateEmitter.emit(
        FindPhaseTapChangerQuery.class,
  ...
  protected void emitFindAllPhaseTapChanger(PhaseTapChanger entity) {
    ...
    queryUpdateEmitter.emit(FindAllPhaseTapChangerQuery.class, query -> true, entity);
  }
