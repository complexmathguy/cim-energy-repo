...
/**
 * Projector for Analog as outlined for the CQRS pattern. All event handling and query handling
 * related to Analog are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class AnalogProjector extends AnalogEntityProjector {
  ...
  /*
   * @param	event AssignPositiveFlowInToAnalogEvent
   */
  @EventHandler(payloadType = AssignPositiveFlowInToAnalogEvent.class)
  public void handle(AssignPositiveFlowInToAnalogEvent event) {
    LOGGER.info("handling AssignPositiveFlowInToAnalogEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Analog entity = assignPositiveFlowIn(event.getAnalogId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignPositiveFlowInFromAnalogEvent
   */
  @EventHandler(payloadType = UnAssignPositiveFlowInFromAnalogEvent.class)
  public void handle(UnAssignPositiveFlowInFromAnalogEvent event) {
    LOGGER.info("handling UnAssignPositiveFlowInFromAnalogEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Analog entity = unAssignPositiveFlowIn(event.getAnalogId());
  ...
  @QueryHandler
  public Analog handle(FindAnalogQuery query) throws ProcessingException, IllegalArgumentException {
  ...
  /**
   * emit to subscription queries of type FindAnalog, but only if the id matches
   *
   * @param entity Analog
   */
  ...
  protected void emitFindAllAnalog(Analog entity) {
    ...
    queryUpdateEmitter.emit(FindAllAnalogQuery.class, query -> true, entity);
  }
