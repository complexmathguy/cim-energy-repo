...
/**
 * Projector for ConformLoad as outlined for the CQRS pattern. All event handling and query handling
 * related to ConformLoad are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class ConformLoadProjector extends ConformLoadEntityProjector {
  ...
  // core constructor
  public ConformLoadProjector(
      ConformLoadRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindConformLoad, but only if the id matches
   *
  ...
  protected void emitFindAllConformLoad(ConformLoad entity) {
    ...
    queryUpdateEmitter.emit(FindAllConformLoadQuery.class, query -> true, entity);
  }
