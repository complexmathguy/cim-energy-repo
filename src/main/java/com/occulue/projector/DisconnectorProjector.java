...
/**
 * Projector for Disconnector as outlined for the CQRS pattern. All event handling and query
 * handling related to Disconnector are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class DisconnectorProjector extends DisconnectorEntityProjector {
  ...
  // core constructor
  public DisconnectorProjector(
      DisconnectorRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindDisconnector, but only if the id matches
   *
  ...
  protected void emitFindAllDisconnector(Disconnector entity) {
    ...
    queryUpdateEmitter.emit(FindAllDisconnectorQuery.class, query -> true, entity);
  }
