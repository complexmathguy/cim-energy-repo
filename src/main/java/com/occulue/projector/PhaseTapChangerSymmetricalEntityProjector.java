...
public class PhaseTapChangerSymmetricalEntityProjector {
  ...
  // core constructor
  public PhaseTapChangerSymmetricalEntityProjector(
      PhaseTapChangerSymmetricalRepository repository) {
  ...
  public PhaseTapChangerSymmetrical find(UUID id) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find a PhaseTapChangerSymmetrical - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  public List<PhaseTapChangerSymmetrical> findAll(FindAllPhaseTapChangerSymmetricalQuery query) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find all PhaseTapChangerSymmetrical - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final PhaseTapChangerSymmetricalRepository repository;

  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerSymmetricalEntityProjector.class.getName());
}
