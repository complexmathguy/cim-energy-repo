...
/**
 * Projector for DiagramObjectStyle as outlined for the CQRS pattern. All event handling and query
 * handling related to DiagramObjectStyle are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by DiagramObjectStyleAggregate
 *
...
public class DiagramObjectStyleProjector extends DiagramObjectStyleEntityProjector {
  ...
  // core constructor
  public DiagramObjectStyleProjector(
      DiagramObjectStyleRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<DiagramObjectStyle> handle(FindAllDiagramObjectStyleQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindDiagramObjectStyle, but only if the id matches
   *
  ...
  protected void emitFindDiagramObjectStyle(DiagramObjectStyle entity) {
    ...
    queryUpdateEmitter.emit(
        FindDiagramObjectStyleQuery.class,
        query ->
            query.getFilter().getDiagramObjectStyleId().equals(entity.getDiagramObjectStyleId()),
        entity);
  }
  ...
  protected void emitFindAllDiagramObjectStyle(DiagramObjectStyle entity) {
    ...
    queryUpdateEmitter.emit(FindAllDiagramObjectStyleQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(DiagramObjectStyleProjector.class.getName());
}
