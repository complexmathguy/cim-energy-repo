...
/**
 * Projector for DCBusbar as outlined for the CQRS pattern. All event handling and query handling
 * related to DCBusbar are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by DCBusbarAggregate
 *
...
public class DCBusbarProjector extends DCBusbarEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindDCBusbar, but only if the id matches
   *
  ...
  protected void emitFindAllDCBusbar(DCBusbar entity) {
    ...
    queryUpdateEmitter.emit(FindAllDCBusbarQuery.class, query -> true, entity);
  }
