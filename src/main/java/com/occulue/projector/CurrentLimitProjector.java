...
/**
 * Projector for CurrentLimit as outlined for the CQRS pattern. All event handling and query
 * handling related to CurrentLimit are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class CurrentLimitProjector extends CurrentLimitEntityProjector {
  ...
  // core constructor
  public CurrentLimitProjector(
      CurrentLimitRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToCurrentLimitEvent
   */
  @EventHandler(payloadType = AssignValueToCurrentLimitEvent.class)
  public void handle(AssignValueToCurrentLimitEvent event) {
    LOGGER.info("handling AssignValueToCurrentLimitEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    CurrentLimit entity = assignValue(event.getCurrentLimitId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromCurrentLimitEvent
   */
  @EventHandler(payloadType = UnAssignValueFromCurrentLimitEvent.class)
  public void handle(UnAssignValueFromCurrentLimitEvent event) {
    LOGGER.info("handling UnAssignValueFromCurrentLimitEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    CurrentLimit entity = unAssignValue(event.getCurrentLimitId());
  ...
  /**
   * emit to subscription queries of type FindCurrentLimit, but only if the id matches
   *
  ...
  protected void emitFindAllCurrentLimit(CurrentLimit entity) {
    ...
    queryUpdateEmitter.emit(FindAllCurrentLimitQuery.class, query -> true, entity);
  }
