...
/**
 * Projector for DCChopper as outlined for the CQRS pattern. All event handling and query handling
 * related to DCChopper are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class DCChopperProjector extends DCChopperEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindDCChopper, but only if the id matches
   *
  ...
  protected void emitFindAllDCChopper(DCChopper entity) {
    ...
    queryUpdateEmitter.emit(FindAllDCChopperQuery.class, query -> true, entity);
  }
