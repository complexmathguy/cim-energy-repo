...
/**
 * Projector for DateTime as outlined for the CQRS pattern. All event handling and query handling
 * related to DateTime are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class DateTimeProjector extends DateTimeEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindDateTime, but only if the id matches
   *
  ...
  protected void emitFindAllDateTime(DateTime entity) {
    ...
    queryUpdateEmitter.emit(FindAllDateTimeQuery.class, query -> true, entity);
  }
