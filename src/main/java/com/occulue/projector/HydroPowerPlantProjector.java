...
/**
 * Projector for HydroPowerPlant as outlined for the CQRS pattern. All event handling and query
 * handling related to HydroPowerPlant are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class HydroPowerPlantProjector extends HydroPowerPlantEntityProjector {
  ...
  // core constructor
  public HydroPowerPlantProjector(
      HydroPowerPlantRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<HydroPowerPlant> handle(FindAllHydroPowerPlantQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindHydroPowerPlant, but only if the id matches
   *
  ...
  protected void emitFindHydroPowerPlant(HydroPowerPlant entity) {
    ...
    queryUpdateEmitter.emit(
        FindHydroPowerPlantQuery.class,
  ...
  protected void emitFindAllHydroPowerPlant(HydroPowerPlant entity) {
    ...
    queryUpdateEmitter.emit(FindAllHydroPowerPlantQuery.class, query -> true, entity);
  }
