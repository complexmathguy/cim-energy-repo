...
/**
 * Projector for BaseVoltage as outlined for the CQRS pattern. All event handling and query handling
 * related to BaseVoltage are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class BaseVoltageProjector extends BaseVoltageEntityProjector {
  ...
  // core constructor
  public BaseVoltageProjector(
      BaseVoltageRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignNominalVoltageToBaseVoltageEvent
   */
  @EventHandler(payloadType = AssignNominalVoltageToBaseVoltageEvent.class)
  public void handle(AssignNominalVoltageToBaseVoltageEvent event) {
    LOGGER.info("handling AssignNominalVoltageToBaseVoltageEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    BaseVoltage entity = assignNominalVoltage(event.getBaseVoltageId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignNominalVoltageFromBaseVoltageEvent
   */
  @EventHandler(payloadType = UnAssignNominalVoltageFromBaseVoltageEvent.class)
  public void handle(UnAssignNominalVoltageFromBaseVoltageEvent event) {
    LOGGER.info("handling UnAssignNominalVoltageFromBaseVoltageEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    BaseVoltage entity = unAssignNominalVoltage(event.getBaseVoltageId());
  ...
  /**
   * emit to subscription queries of type FindBaseVoltage, but only if the id matches
   *
  ...
  protected void emitFindAllBaseVoltage(BaseVoltage entity) {
    ...
    queryUpdateEmitter.emit(FindAllBaseVoltageQuery.class, query -> true, entity);
  }
