...
/**
 * Projector for CurrentFlow as outlined for the CQRS pattern. All event handling and query handling
 * related to CurrentFlow are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class CurrentFlowProjector extends CurrentFlowEntityProjector {
  ...
  // core constructor
  public CurrentFlowProjector(
      CurrentFlowRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToCurrentFlowEvent
   */
  @EventHandler(payloadType = AssignValueToCurrentFlowEvent.class)
  public void handle(AssignValueToCurrentFlowEvent event) {
    LOGGER.info("handling AssignValueToCurrentFlowEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    CurrentFlow entity = assignValue(event.getCurrentFlowId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromCurrentFlowEvent
   */
  @EventHandler(payloadType = UnAssignValueFromCurrentFlowEvent.class)
  public void handle(UnAssignValueFromCurrentFlowEvent event) {
    LOGGER.info("handling UnAssignValueFromCurrentFlowEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    CurrentFlow entity = unAssignValue(event.getCurrentFlowId());
  ...
  /**
   * emit to subscription queries of type FindCurrentFlow, but only if the id matches
   *
  ...
  protected void emitFindAllCurrentFlow(CurrentFlow entity) {
    ...
    queryUpdateEmitter.emit(FindAllCurrentFlowQuery.class, query -> true, entity);
  }
