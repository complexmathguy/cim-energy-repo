...
/**
 * Projector for EquipmentContainer as outlined for the CQRS pattern. All event handling and query
 * handling related to EquipmentContainer are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class EquipmentContainerProjector extends EquipmentContainerEntityProjector {
  ...
  // core constructor
  public EquipmentContainerProjector(
      EquipmentContainerRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<EquipmentContainer> handle(FindAllEquipmentContainerQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindEquipmentContainer, but only if the id matches
   *
  ...
  protected void emitFindEquipmentContainer(EquipmentContainer entity) {
    ...
    queryUpdateEmitter.emit(
        FindEquipmentContainerQuery.class,
        query ->
            query.getFilter().getEquipmentContainerId().equals(entity.getEquipmentContainerId()),
        entity);
  }
  ...
  protected void emitFindAllEquipmentContainer(EquipmentContainer entity) {
    ...
    queryUpdateEmitter.emit(FindAllEquipmentContainerQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(EquipmentContainerProjector.class.getName());
}
