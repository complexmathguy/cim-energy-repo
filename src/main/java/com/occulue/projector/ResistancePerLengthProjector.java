...
/**
 * Projector for ResistancePerLength as outlined for the CQRS pattern. All event handling and query
 * handling related to ResistancePerLength are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class ResistancePerLengthProjector extends ResistancePerLengthEntityProjector {
  ...
  // core constructor
  public ResistancePerLengthProjector(
      ResistancePerLengthRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  public void handle(AssignValueToResistancePerLengthEvent event) {
    ...
    // ------------------------------------------
    ResistancePerLength entity =
        assignValue(event.getResistancePerLengthId(), event.getAssignment());
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<ResistancePerLength> handle(FindAllResistancePerLengthQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindResistancePerLength, but only if the id matches
   *
  ...
  protected void emitFindResistancePerLength(ResistancePerLength entity) {
    ...
    queryUpdateEmitter.emit(
        FindResistancePerLengthQuery.class,
        query ->
            query.getFilter().getResistancePerLengthId().equals(entity.getResistancePerLengthId()),
        entity);
  }
  ...
  protected void emitFindAllResistancePerLength(ResistancePerLength entity) {
    ...
    queryUpdateEmitter.emit(FindAllResistancePerLengthQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ResistancePerLengthProjector.class.getName());
}
