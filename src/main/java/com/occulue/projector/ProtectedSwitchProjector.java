...
/**
 * Projector for ProtectedSwitch as outlined for the CQRS pattern. All event handling and query
 * handling related to ProtectedSwitch are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by ProtectedSwitchAggregate
 *
...
public class ProtectedSwitchProjector extends ProtectedSwitchEntityProjector {
  ...
  // core constructor
  public ProtectedSwitchProjector(
      ProtectedSwitchRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<ProtectedSwitch> handle(FindAllProtectedSwitchQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindProtectedSwitch, but only if the id matches
   *
  ...
  protected void emitFindProtectedSwitch(ProtectedSwitch entity) {
    ...
    queryUpdateEmitter.emit(
        FindProtectedSwitchQuery.class,
  ...
  protected void emitFindAllProtectedSwitch(ProtectedSwitch entity) {
    ...
    queryUpdateEmitter.emit(FindAllProtectedSwitchQuery.class, query -> true, entity);
  }
