...
/**
 * Projector for NonlinearShuntCompensator as outlined for the CQRS pattern. All event handling and
 * query handling related to NonlinearShuntCompensator are invoked here and dispersed as an event to
 * be handled elsewhere.
 *
 * <p>Commands are handled by NonlinearShuntCompensatorAggregate
 *
...
public class NonlinearShuntCompensatorProjector extends NonlinearShuntCompensatorEntityProjector {
  ...
  // core constructor
  public NonlinearShuntCompensatorProjector(
      NonlinearShuntCompensatorRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<NonlinearShuntCompensator> handle(FindAllNonlinearShuntCompensatorQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindNonlinearShuntCompensator, but only if the id matches
   *
  ...
  protected void emitFindNonlinearShuntCompensator(NonlinearShuntCompensator entity) {
    ...
    queryUpdateEmitter.emit(
        FindNonlinearShuntCompensatorQuery.class,
        query ->
            query
                .getFilter()
                .getNonlinearShuntCompensatorId()
                .equals(entity.getNonlinearShuntCompensatorId()),
        entity);
  }
  ...
  protected void emitFindAllNonlinearShuntCompensator(NonlinearShuntCompensator entity) {
    ...
    queryUpdateEmitter.emit(FindAllNonlinearShuntCompensatorQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NonlinearShuntCompensatorProjector.class.getName());
}
