...
/**
 * Projector for NonConformLoadGroup as outlined for the CQRS pattern. All event handling and query
 * handling related to NonConformLoadGroup are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class NonConformLoadGroupProjector extends NonConformLoadGroupEntityProjector {
  ...
  // core constructor
  public NonConformLoadGroupProjector(
      NonConformLoadGroupRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<NonConformLoadGroup> handle(FindAllNonConformLoadGroupQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindNonConformLoadGroup, but only if the id matches
   *
  ...
  protected void emitFindNonConformLoadGroup(NonConformLoadGroup entity) {
    ...
    queryUpdateEmitter.emit(
        FindNonConformLoadGroupQuery.class,
        query ->
            query.getFilter().getNonConformLoadGroupId().equals(entity.getNonConformLoadGroupId()),
        entity);
  }
  ...
  protected void emitFindAllNonConformLoadGroup(NonConformLoadGroup entity) {
    ...
    queryUpdateEmitter.emit(FindAllNonConformLoadGroupQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NonConformLoadGroupProjector.class.getName());
}
