...
/**
 * Projector for GroundDisconnector as outlined for the CQRS pattern. All event handling and query
 * handling related to GroundDisconnector are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class GroundDisconnectorProjector extends GroundDisconnectorEntityProjector {
  ...
  // core constructor
  public GroundDisconnectorProjector(
      GroundDisconnectorRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<GroundDisconnector> handle(FindAllGroundDisconnectorQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindGroundDisconnector, but only if the id matches
   *
  ...
  protected void emitFindGroundDisconnector(GroundDisconnector entity) {
    ...
    queryUpdateEmitter.emit(
        FindGroundDisconnectorQuery.class,
        query ->
            query.getFilter().getGroundDisconnectorId().equals(entity.getGroundDisconnectorId()),
        entity);
  }
  ...
  protected void emitFindAllGroundDisconnector(GroundDisconnector entity) {
    ...
    queryUpdateEmitter.emit(FindAllGroundDisconnectorQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GroundDisconnectorProjector.class.getName());
}
