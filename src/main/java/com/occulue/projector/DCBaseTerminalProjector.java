...
/**
 * Projector for DCBaseTerminal as outlined for the CQRS pattern. All event handling and query
 * handling related to DCBaseTerminal are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class DCBaseTerminalProjector extends DCBaseTerminalEntityProjector {
  ...
  // core constructor
  public DCBaseTerminalProjector(
      DCBaseTerminalRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindDCBaseTerminal, but only if the id matches
   *
  ...
  protected void emitFindAllDCBaseTerminal(DCBaseTerminal entity) {
    ...
    queryUpdateEmitter.emit(FindAllDCBaseTerminalQuery.class, query -> true, entity);
  }
