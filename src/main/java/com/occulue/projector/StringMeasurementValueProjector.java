...
/**
 * Projector for StringMeasurementValue as outlined for the CQRS pattern. All event handling and
 * query handling related to StringMeasurementValue are invoked here and dispersed as an event to be
 * handled elsewhere.
 *
...
public class StringMeasurementValueProjector extends StringMeasurementValueEntityProjector {
  ...
  // core constructor
  public StringMeasurementValueProjector(
      StringMeasurementValueRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToStringMeasurementValueEvent
   */
  @EventHandler(payloadType = AssignValueToStringMeasurementValueEvent.class)
  public void handle(AssignValueToStringMeasurementValueEvent event) {
    LOGGER.info("handling AssignValueToStringMeasurementValueEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    StringMeasurementValue entity =
        assignValue(event.getStringMeasurementValueId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromStringMeasurementValueEvent
   */
  @EventHandler(payloadType = UnAssignValueFromStringMeasurementValueEvent.class)
  public void handle(UnAssignValueFromStringMeasurementValueEvent event) {
    LOGGER.info("handling UnAssignValueFromStringMeasurementValueEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    StringMeasurementValue entity = unAssignValue(event.getStringMeasurementValueId());
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<StringMeasurementValue> handle(FindAllStringMeasurementValueQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindStringMeasurementValue, but only if the id matches
   *
  ...
  protected void emitFindStringMeasurementValue(StringMeasurementValue entity) {
    ...
    queryUpdateEmitter.emit(
        FindStringMeasurementValueQuery.class,
        query ->
            query
                .getFilter()
                .getStringMeasurementValueId()
                .equals(entity.getStringMeasurementValueId()),
        entity);
  }
  ...
  protected void emitFindAllStringMeasurementValue(StringMeasurementValue entity) {
    ...
    queryUpdateEmitter.emit(FindAllStringMeasurementValueQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(StringMeasurementValueProjector.class.getName());
}
