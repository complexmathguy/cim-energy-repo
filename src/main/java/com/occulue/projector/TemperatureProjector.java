...
/**
 * Projector for Temperature as outlined for the CQRS pattern. All event handling and query handling
 * related to Temperature are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class TemperatureProjector extends TemperatureEntityProjector {
  ...
  // core constructor
  public TemperatureProjector(
      TemperatureRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToTemperatureEvent
   */
  @EventHandler(payloadType = AssignValueToTemperatureEvent.class)
  public void handle(AssignValueToTemperatureEvent event) {
    LOGGER.info("handling AssignValueToTemperatureEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Temperature entity = assignValue(event.getTemperatureId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromTemperatureEvent
   */
  @EventHandler(payloadType = UnAssignValueFromTemperatureEvent.class)
  public void handle(UnAssignValueFromTemperatureEvent event) {
    LOGGER.info("handling UnAssignValueFromTemperatureEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Temperature entity = unAssignValue(event.getTemperatureId());
  ...
  /**
   * emit to subscription queries of type FindTemperature, but only if the id matches
   *
  ...
  protected void emitFindAllTemperature(Temperature entity) {
    ...
    queryUpdateEmitter.emit(FindAllTemperatureQuery.class, query -> true, entity);
  }
