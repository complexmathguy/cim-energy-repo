...
/**
 * Projector for LoadAggregate as outlined for the CQRS pattern. All event handling and query
 * handling related to LoadAggregate are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by LoadAggregateAggregate
 *
...
public class LoadAggregateProjector extends LoadAggregateEntityProjector {
  ...
  // core constructor
  public LoadAggregateProjector(
      LoadAggregateRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindLoadAggregate, but only if the id matches
   *
  ...
  protected void emitFindAllLoadAggregate(LoadAggregate entity) {
    ...
    queryUpdateEmitter.emit(FindAllLoadAggregateQuery.class, query -> true, entity);
  }
