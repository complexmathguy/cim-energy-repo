...
/**
 * Projector for Resistance as outlined for the CQRS pattern. All event handling and query handling
 * related to Resistance are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class ResistanceProjector extends ResistanceEntityProjector {
  ...
  // core constructor
  public ResistanceProjector(
      ResistanceRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToResistanceEvent
   */
  @EventHandler(payloadType = AssignValueToResistanceEvent.class)
  public void handle(AssignValueToResistanceEvent event) {
    LOGGER.info("handling AssignValueToResistanceEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Resistance entity = assignValue(event.getResistanceId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromResistanceEvent
   */
  @EventHandler(payloadType = UnAssignValueFromResistanceEvent.class)
  public void handle(UnAssignValueFromResistanceEvent event) {
    LOGGER.info("handling UnAssignValueFromResistanceEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Resistance entity = unAssignValue(event.getResistanceId());
  ...
  /**
   * emit to subscription queries of type FindResistance, but only if the id matches
   *
  ...
  protected void emitFindAllResistance(Resistance entity) {
    ...
    queryUpdateEmitter.emit(FindAllResistanceQuery.class, query -> true, entity);
  }
