...
/**
 * Projector for DiscreteValue as outlined for the CQRS pattern. All event handling and query
 * handling related to DiscreteValue are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class DiscreteValueProjector extends DiscreteValueEntityProjector {
  ...
  // core constructor
  public DiscreteValueProjector(
      DiscreteValueRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToDiscreteValueEvent
   */
  @EventHandler(payloadType = AssignValueToDiscreteValueEvent.class)
  public void handle(AssignValueToDiscreteValueEvent event) {
    LOGGER.info("handling AssignValueToDiscreteValueEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    DiscreteValue entity = assignValue(event.getDiscreteValueId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromDiscreteValueEvent
   */
  @EventHandler(payloadType = UnAssignValueFromDiscreteValueEvent.class)
  public void handle(UnAssignValueFromDiscreteValueEvent event) {
    LOGGER.info("handling UnAssignValueFromDiscreteValueEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    DiscreteValue entity = unAssignValue(event.getDiscreteValueId());
  ...
  /**
   * emit to subscription queries of type FindDiscreteValue, but only if the id matches
   *
  ...
  protected void emitFindAllDiscreteValue(DiscreteValue entity) {
    ...
    queryUpdateEmitter.emit(FindAllDiscreteValueQuery.class, query -> true, entity);
  }
