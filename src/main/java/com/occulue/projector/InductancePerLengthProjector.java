...
/**
 * Projector for InductancePerLength as outlined for the CQRS pattern. All event handling and query
 * handling related to InductancePerLength are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class InductancePerLengthProjector extends InductancePerLengthEntityProjector {
  ...
  // core constructor
  public InductancePerLengthProjector(
      InductancePerLengthRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToInductancePerLengthEvent
   */
  @EventHandler(payloadType = AssignValueToInductancePerLengthEvent.class)
  public void handle(AssignValueToInductancePerLengthEvent event) {
    LOGGER.info("handling AssignValueToInductancePerLengthEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    InductancePerLength entity =
        assignValue(event.getInductancePerLengthId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromInductancePerLengthEvent
   */
  @EventHandler(payloadType = UnAssignValueFromInductancePerLengthEvent.class)
  public void handle(UnAssignValueFromInductancePerLengthEvent event) {
    LOGGER.info("handling UnAssignValueFromInductancePerLengthEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    InductancePerLength entity = unAssignValue(event.getInductancePerLengthId());
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<InductancePerLength> handle(FindAllInductancePerLengthQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindInductancePerLength, but only if the id matches
   *
  ...
  protected void emitFindInductancePerLength(InductancePerLength entity) {
    ...
    queryUpdateEmitter.emit(
        FindInductancePerLengthQuery.class,
        query ->
            query.getFilter().getInductancePerLengthId().equals(entity.getInductancePerLengthId()),
        entity);
  }
  ...
  protected void emitFindAllInductancePerLength(InductancePerLength entity) {
    ...
    queryUpdateEmitter.emit(FindAllInductancePerLengthQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(InductancePerLengthProjector.class.getName());
}
