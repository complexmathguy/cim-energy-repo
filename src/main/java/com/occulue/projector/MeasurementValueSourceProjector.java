...
/**
 * Projector for MeasurementValueSource as outlined for the CQRS pattern. All event handling and
 * query handling related to MeasurementValueSource are invoked here and dispersed as an event to be
 * handled elsewhere.
 *
...
public class MeasurementValueSourceProjector extends MeasurementValueSourceEntityProjector {
  ...
  // core constructor
  public MeasurementValueSourceProjector(
      MeasurementValueSourceRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<MeasurementValueSource> handle(FindAllMeasurementValueSourceQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindMeasurementValueSource, but only if the id matches
   *
  ...
  protected void emitFindMeasurementValueSource(MeasurementValueSource entity) {
    ...
    queryUpdateEmitter.emit(
        FindMeasurementValueSourceQuery.class,
        query ->
            query
                .getFilter()
                .getMeasurementValueSourceId()
                .equals(entity.getMeasurementValueSourceId()),
        entity);
  }
  ...
  protected void emitFindAllMeasurementValueSource(MeasurementValueSource entity) {
    ...
    queryUpdateEmitter.emit(FindAllMeasurementValueSourceQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(MeasurementValueSourceProjector.class.getName());
}
