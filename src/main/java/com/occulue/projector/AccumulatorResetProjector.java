...
/**
 * Projector for AccumulatorReset as outlined for the CQRS pattern. All event handling and query
 * handling related to AccumulatorReset are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class AccumulatorResetProjector extends AccumulatorResetEntityProjector {
  ...
  // core constructor
  public AccumulatorResetProjector(
      AccumulatorResetRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<AccumulatorReset> handle(FindAllAccumulatorResetQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindAccumulatorReset, but only if the id matches
   *
  ...
  protected void emitFindAccumulatorReset(AccumulatorReset entity) {
    ...
    queryUpdateEmitter.emit(
        FindAccumulatorResetQuery.class,
  ...
  protected void emitFindAllAccumulatorReset(AccumulatorReset entity) {
    ...
    queryUpdateEmitter.emit(FindAllAccumulatorResetQuery.class, query -> true, entity);
  }
