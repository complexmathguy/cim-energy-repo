...
/**
 * Projector for LoadDynamics as outlined for the CQRS pattern. All event handling and query
 * handling related to LoadDynamics are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by LoadDynamicsAggregate
 *
...
public class LoadDynamicsProjector extends LoadDynamicsEntityProjector {
  ...
  // core constructor
  public LoadDynamicsProjector(
      LoadDynamicsRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindLoadDynamics, but only if the id matches
   *
  ...
  protected void emitFindAllLoadDynamics(LoadDynamics entity) {
    ...
    queryUpdateEmitter.emit(FindAllLoadDynamicsQuery.class, query -> true, entity);
  }
