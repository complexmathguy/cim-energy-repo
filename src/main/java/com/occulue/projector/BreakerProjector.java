...
/**
 * Projector for Breaker as outlined for the CQRS pattern. All event handling and query handling
 * related to Breaker are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by BreakerAggregate
 *
...
public class BreakerProjector extends BreakerEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindBreaker, but only if the id matches
   *
  ...
  protected void emitFindAllBreaker(Breaker entity) {
    ...
    queryUpdateEmitter.emit(FindAllBreakerQuery.class, query -> true, entity);
  }
