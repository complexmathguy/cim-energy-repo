...
/**
 * Projector for NonConformLoadSchedule as outlined for the CQRS pattern. All event handling and
 * query handling related to NonConformLoadSchedule are invoked here and dispersed as an event to be
 * handled elsewhere.
 *
...
public class NonConformLoadScheduleProjector extends NonConformLoadScheduleEntityProjector {
  ...
  // core constructor
  public NonConformLoadScheduleProjector(
      NonConformLoadScheduleRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<NonConformLoadSchedule> handle(FindAllNonConformLoadScheduleQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindNonConformLoadSchedule, but only if the id matches
   *
  ...
  protected void emitFindNonConformLoadSchedule(NonConformLoadSchedule entity) {
    ...
    queryUpdateEmitter.emit(
        FindNonConformLoadScheduleQuery.class,
        query ->
            query
                .getFilter()
                .getNonConformLoadScheduleId()
                .equals(entity.getNonConformLoadScheduleId()),
        entity);
  }
  ...
  protected void emitFindAllNonConformLoadSchedule(NonConformLoadSchedule entity) {
    ...
    queryUpdateEmitter.emit(FindAllNonConformLoadScheduleQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(NonConformLoadScheduleProjector.class.getName());
}
