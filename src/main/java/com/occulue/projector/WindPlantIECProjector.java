...
/**
 * Projector for WindPlantIEC as outlined for the CQRS pattern. All event handling and query
 * handling related to WindPlantIEC are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class WindPlantIECProjector extends WindPlantIECEntityProjector {
  ...
  // core constructor
  public WindPlantIECProjector(
      WindPlantIECRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindWindPlantIEC, but only if the id matches
   *
  ...
  protected void emitFindAllWindPlantIEC(WindPlantIEC entity) {
    ...
    queryUpdateEmitter.emit(FindAllWindPlantIECQuery.class, query -> true, entity);
  }
