...
/**
 * Projector for DiagramStyle as outlined for the CQRS pattern. All event handling and query
 * handling related to DiagramStyle are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by DiagramStyleAggregate
 *
...
public class DiagramStyleProjector extends DiagramStyleEntityProjector {
  ...
  // core constructor
  public DiagramStyleProjector(
      DiagramStyleRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindDiagramStyle, but only if the id matches
   *
  ...
  protected void emitFindAllDiagramStyle(DiagramStyle entity) {
    ...
    queryUpdateEmitter.emit(FindAllDiagramStyleQuery.class, query -> true, entity);
  }
