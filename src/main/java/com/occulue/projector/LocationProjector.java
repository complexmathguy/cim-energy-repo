...
/**
 * Projector for Location as outlined for the CQRS pattern. All event handling and query handling
 * related to Location are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class LocationProjector extends LocationEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindLocation, but only if the id matches
   *
  ...
  protected void emitFindAllLocation(Location entity) {
    ...
    queryUpdateEmitter.emit(FindAllLocationQuery.class, query -> true, entity);
  }
