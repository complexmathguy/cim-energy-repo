...
/**
 * Projector for ValueAliasSet as outlined for the CQRS pattern. All event handling and query
 * handling related to ValueAliasSet are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by ValueAliasSetAggregate
 *
...
public class ValueAliasSetProjector extends ValueAliasSetEntityProjector {
  ...
  // core constructor
  public ValueAliasSetProjector(
      ValueAliasSetRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindValueAliasSet, but only if the id matches
   *
  ...
  protected void emitFindAllValueAliasSet(ValueAliasSet entity) {
    ...
    queryUpdateEmitter.emit(FindAllValueAliasSetQuery.class, query -> true, entity);
  }
