...
/**
 * Projector for StationSupply as outlined for the CQRS pattern. All event handling and query
 * handling related to StationSupply are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class StationSupplyProjector extends StationSupplyEntityProjector {
  ...
  // core constructor
  public StationSupplyProjector(
      StationSupplyRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindStationSupply, but only if the id matches
   *
  ...
  protected void emitFindAllStationSupply(StationSupply entity) {
    ...
    queryUpdateEmitter.emit(FindAllStationSupplyQuery.class, query -> true, entity);
  }
