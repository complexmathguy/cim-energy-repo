...
/**
 * Projector for FloatProxy as outlined for the CQRS pattern. All event handling and query handling
 * related to FloatProxy are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class FloatProxyProjector extends FloatProxyEntityProjector {
  ...
  // core constructor
  public FloatProxyProjector(
      FloatProxyRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindFloatProxy, but only if the id matches
   *
  ...
  protected void emitFindAllFloatProxy(FloatProxy entity) {
    ...
    queryUpdateEmitter.emit(FindAllFloatProxyQuery.class, query -> true, entity);
  }
