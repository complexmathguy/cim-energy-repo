...
/**
 * Projector for PhaseTapChangerTable as outlined for the CQRS pattern. All event handling and query
 * handling related to PhaseTapChangerTable are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class PhaseTapChangerTableProjector extends PhaseTapChangerTableEntityProjector {
  ...
  // core constructor
  public PhaseTapChangerTableProjector(
      PhaseTapChangerTableRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<PhaseTapChangerTable> handle(FindAllPhaseTapChangerTableQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindPhaseTapChangerTable, but only if the id matches
   *
  ...
  protected void emitFindPhaseTapChangerTable(PhaseTapChangerTable entity) {
    ...
    queryUpdateEmitter.emit(
        FindPhaseTapChangerTableQuery.class,
        query ->
            query
                .getFilter()
                .getPhaseTapChangerTableId()
                .equals(entity.getPhaseTapChangerTableId()),
        entity);
  }
  ...
  protected void emitFindAllPhaseTapChangerTable(PhaseTapChangerTable entity) {
    ...
    queryUpdateEmitter.emit(FindAllPhaseTapChangerTableQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(PhaseTapChangerTableProjector.class.getName());
}
