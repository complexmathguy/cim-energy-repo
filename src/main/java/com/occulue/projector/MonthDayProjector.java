...
/**
 * Projector for MonthDay as outlined for the CQRS pattern. All event handling and query handling
 * related to MonthDay are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class MonthDayProjector extends MonthDayEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindMonthDay, but only if the id matches
   *
  ...
  protected void emitFindAllMonthDay(MonthDay entity) {
    ...
    queryUpdateEmitter.emit(FindAllMonthDayQuery.class, query -> true, entity);
  }
