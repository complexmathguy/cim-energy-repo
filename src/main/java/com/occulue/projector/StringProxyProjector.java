...
/**
 * Projector for StringProxy as outlined for the CQRS pattern. All event handling and query handling
 * related to StringProxy are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class StringProxyProjector extends StringProxyEntityProjector {
  ...
  // core constructor
  public StringProxyProjector(
      StringProxyRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindStringProxy, but only if the id matches
   *
  ...
  protected void emitFindAllStringProxy(StringProxy entity) {
    ...
    queryUpdateEmitter.emit(FindAllStringProxyQuery.class, query -> true, entity);
  }
