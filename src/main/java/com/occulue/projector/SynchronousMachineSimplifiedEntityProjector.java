...
public class SynchronousMachineSimplifiedEntityProjector {
  ...
  // core constructor
  public SynchronousMachineSimplifiedEntityProjector(
      SynchronousMachineSimplifiedRepository repository) {
   ...
   * FindSynchronousMachineSimplifiedQuery
   *
  ...
  public SynchronousMachineSimplified find(UUID id) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find a SynchronousMachineSimplified - {0}", exc.getMessage());
    }
    return null;
  }
   ...
   */
  @SuppressWarnings("unused")
  public List<SynchronousMachineSimplified> findAll(
      FindAllSynchronousMachineSimplifiedQuery query) {
    LOGGER.info("handling findAll using " + query.toString());
    try {
      return repository.findAll();
    } catch (IllegalArgumentException exc) {
      LOGGER.log(
          Level.WARNING, "Failed to find all SynchronousMachineSimplified - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final SynchronousMachineSimplifiedRepository repository;

  private static final Logger LOGGER =
      Logger.getLogger(SynchronousMachineSimplifiedEntityProjector.class.getName());
}
