...
/**
 * Projector for AngleDegrees as outlined for the CQRS pattern. All event handling and query
 * handling related to AngleDegrees are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class AngleDegreesProjector extends AngleDegreesEntityProjector {
  ...
  // core constructor
  public AngleDegreesProjector(
      AngleDegreesRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /*
   * @param	event AssignValueToAngleDegreesEvent
   */
  @EventHandler(payloadType = AssignValueToAngleDegreesEvent.class)
  public void handle(AssignValueToAngleDegreesEvent event) {
    LOGGER.info("handling AssignValueToAngleDegreesEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    AngleDegrees entity = assignValue(event.getAngleDegreesId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromAngleDegreesEvent
   */
  @EventHandler(payloadType = UnAssignValueFromAngleDegreesEvent.class)
  public void handle(UnAssignValueFromAngleDegreesEvent event) {
    LOGGER.info("handling UnAssignValueFromAngleDegreesEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    AngleDegrees entity = unAssignValue(event.getAngleDegreesId());
  ...
  /**
   * emit to subscription queries of type FindAngleDegrees, but only if the id matches
   *
  ...
  protected void emitFindAllAngleDegrees(AngleDegrees entity) {
    ...
    queryUpdateEmitter.emit(FindAllAngleDegreesQuery.class, query -> true, entity);
  }
