...
public class ConnectivityNodeContainerEntityProjector {
  ...
  public ConnectivityNodeContainer find(UUID id) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find a ConnectivityNodeContainer - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  public List<ConnectivityNodeContainer> findAll(FindAllConnectivityNodeContainerQuery query) {
    ...
    try {
      ...
      LOGGER.log(
          Level.WARNING, "Failed to find all ConnectivityNodeContainer - {0}", exc.getMessage());
    }
    return null;
  }
  ...
  // attributes
  // --------------------------------------------------
  @Autowired protected final ConnectivityNodeContainerRepository repository;

  private static final Logger LOGGER =
      Logger.getLogger(ConnectivityNodeContainerEntityProjector.class.getName());
}
