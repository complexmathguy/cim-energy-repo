...
/**
 * Projector for FossilFuel as outlined for the CQRS pattern. All event handling and query handling
 * related to FossilFuel are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class FossilFuelProjector extends FossilFuelEntityProjector {
  ...
  // core constructor
  public FossilFuelProjector(
      FossilFuelRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindFossilFuel, but only if the id matches
   *
  ...
  protected void emitFindAllFossilFuel(FossilFuel entity) {
    ...
    queryUpdateEmitter.emit(FindAllFossilFuelQuery.class, query -> true, entity);
  }
