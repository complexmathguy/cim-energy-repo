...
/**
 * Projector for ControlArea as outlined for the CQRS pattern. All event handling and query handling
 * related to ControlArea are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class ControlAreaProjector extends ControlAreaEntityProjector {
  ...
  // core constructor
  public ControlAreaProjector(
      ControlAreaRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindControlArea, but only if the id matches
   *
  ...
  protected void emitFindAllControlArea(ControlArea entity) {
    ...
    queryUpdateEmitter.emit(FindAllControlAreaQuery.class, query -> true, entity);
  }
