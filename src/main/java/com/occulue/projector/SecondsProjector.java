...
/**
 * Projector for Seconds as outlined for the CQRS pattern. All event handling and query handling
 * related to Seconds are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class SecondsProjector extends SecondsEntityProjector {
  ...
  /*
   * @param	event AssignValueToSecondsEvent
   */
  @EventHandler(payloadType = AssignValueToSecondsEvent.class)
  public void handle(AssignValueToSecondsEvent event) {
    LOGGER.info("handling AssignValueToSecondsEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    Seconds entity = assignValue(event.getSecondsId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromSecondsEvent
   */
  @EventHandler(payloadType = UnAssignValueFromSecondsEvent.class)
  public void handle(UnAssignValueFromSecondsEvent event) {
    LOGGER.info("handling UnAssignValueFromSecondsEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    Seconds entity = unAssignValue(event.getSecondsId());
  ...
  /**
   * emit to subscription queries of type FindSeconds, but only if the id matches
   *
  ...
  protected void emitFindAllSeconds(Seconds entity) {
    ...
    queryUpdateEmitter.emit(FindAllSecondsQuery.class, query -> true, entity);
  }
