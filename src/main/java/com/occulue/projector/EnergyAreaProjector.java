...
/**
 * Projector for EnergyArea as outlined for the CQRS pattern. All event handling and query handling
 * related to EnergyArea are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class EnergyAreaProjector extends EnergyAreaEntityProjector {
  ...
  // core constructor
  public EnergyAreaProjector(
      EnergyAreaRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindEnergyArea, but only if the id matches
   *
  ...
  protected void emitFindAllEnergyArea(EnergyArea entity) {
    ...
    queryUpdateEmitter.emit(FindAllEnergyAreaQuery.class, query -> true, entity);
  }
