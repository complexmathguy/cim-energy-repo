...
/**
 * Projector for GeographicalRegion as outlined for the CQRS pattern. All event handling and query
 * handling related to GeographicalRegion are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by GeographicalRegionAggregate
 *
...
public class GeographicalRegionProjector extends GeographicalRegionEntityProjector {
  ...
  // core constructor
  public GeographicalRegionProjector(
      GeographicalRegionRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<GeographicalRegion> handle(FindAllGeographicalRegionQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindGeographicalRegion, but only if the id matches
   *
  ...
  protected void emitFindGeographicalRegion(GeographicalRegion entity) {
    ...
    queryUpdateEmitter.emit(
        FindGeographicalRegionQuery.class,
        query ->
            query.getFilter().getGeographicalRegionId().equals(entity.getGeographicalRegionId()),
        entity);
  }
  ...
  protected void emitFindAllGeographicalRegion(GeographicalRegion entity) {
    ...
    queryUpdateEmitter.emit(FindAllGeographicalRegionQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(GeographicalRegionProjector.class.getName());
}
