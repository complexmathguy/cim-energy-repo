...
/**
 * Projector for DCBreaker as outlined for the CQRS pattern. All event handling and query handling
 * related to DCBreaker are invoked here and dispersed as an event to be handled elsewhere.
 *
 * <p>Commands are handled by DCBreakerAggregate
 *
...
public class DCBreakerProjector extends DCBreakerEntityProjector {
  ...
  /**
   * emit to subscription queries of type FindDCBreaker, but only if the id matches
   *
  ...
  protected void emitFindAllDCBreaker(DCBreaker entity) {
    ...
    queryUpdateEmitter.emit(FindAllDCBreakerQuery.class, query -> true, entity);
  }
