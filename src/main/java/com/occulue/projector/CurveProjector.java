...
/**
 * Projector for Curve as outlined for the CQRS pattern. All event handling and query handling
 * related to Curve are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class CurveProjector extends CurveEntityProjector {
  ...
  @QueryHandler
  public Curve handle(FindCurveQuery query) throws ProcessingException, IllegalArgumentException {
  ...
  /**
   * emit to subscription queries of type FindCurve, but only if the id matches
   *
   * @param entity Curve
   */
  ...
  protected void emitFindAllCurve(Curve entity) {
    ...
    queryUpdateEmitter.emit(FindAllCurveQuery.class, query -> true, entity);
  }
