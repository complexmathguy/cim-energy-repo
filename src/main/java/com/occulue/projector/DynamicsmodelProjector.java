...
/**
 * Projector for Dynamicsmodel as outlined for the CQRS pattern. All event handling and query
 * handling related to Dynamicsmodel are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
...
public class DynamicsmodelProjector extends DynamicsmodelEntityProjector {
  ...
  // core constructor
  public DynamicsmodelProjector(
      DynamicsmodelRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  /**
   * emit to subscription queries of type FindDynamicsmodel, but only if the id matches
   *
  ...
  protected void emitFindAllDynamicsmodel(Dynamicsmodel entity) {
    ...
    queryUpdateEmitter.emit(FindAllDynamicsmodelQuery.class, query -> true, entity);
  }
