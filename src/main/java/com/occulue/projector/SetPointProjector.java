...
/**
 * Projector for SetPoint as outlined for the CQRS pattern. All event handling and query handling
 * related to SetPoint are invoked here and dispersed as an event to be handled elsewhere.
 *
...
public class SetPointProjector extends SetPointEntityProjector {
  ...
  /*
   * @param	event AssignNormalValueToSetPointEvent
   */
  @EventHandler(payloadType = AssignNormalValueToSetPointEvent.class)
  public void handle(AssignNormalValueToSetPointEvent event) {
    LOGGER.info("handling AssignNormalValueToSetPointEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    SetPoint entity = assignNormalValue(event.getSetPointId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignNormalValueFromSetPointEvent
   */
  @EventHandler(payloadType = UnAssignNormalValueFromSetPointEvent.class)
  public void handle(UnAssignNormalValueFromSetPointEvent event) {
    LOGGER.info("handling UnAssignNormalValueFromSetPointEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    SetPoint entity = unAssignNormalValue(event.getSetPointId());
  ...
  /*
   * @param	event AssignValueToSetPointEvent
   */
  @EventHandler(payloadType = AssignValueToSetPointEvent.class)
  public void handle(AssignValueToSetPointEvent event) {
    LOGGER.info("handling AssignValueToSetPointEvent - " + event);

    // ------------------------------------------
    // delegate to assignTo
    // ------------------------------------------
    SetPoint entity = assignValue(event.getSetPointId(), event.getAssignment());
  ...
  /*
   * @param	event UnAssignValueFromSetPointEvent
   */
  @EventHandler(payloadType = UnAssignValueFromSetPointEvent.class)
  public void handle(UnAssignValueFromSetPointEvent event) {
    LOGGER.info("handling UnAssignValueFromSetPointEvent - " + event);

    // ------------------------------------------
    // delegate to unAssignFrom
    // ------------------------------------------
    SetPoint entity = unAssignValue(event.getSetPointId());
  ...
  /**
   * emit to subscription queries of type FindSetPoint, but only if the id matches
   *
  ...
  protected void emitFindAllSetPoint(SetPoint entity) {
    ...
    queryUpdateEmitter.emit(FindAllSetPointQuery.class, query -> true, entity);
  }
