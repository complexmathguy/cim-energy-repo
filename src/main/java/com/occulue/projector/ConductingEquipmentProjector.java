...
/**
 * Projector for ConductingEquipment as outlined for the CQRS pattern. All event handling and query
 * handling related to ConductingEquipment are invoked here and dispersed as an event to be handled
 * elsewhere.
 *
 * <p>Commands are handled by ConductingEquipmentAggregate
 *
...
public class ConductingEquipmentProjector extends ConductingEquipmentEntityProjector {
  ...
  // core constructor
  public ConductingEquipmentProjector(
      ConductingEquipmentRepository repository, QueryUpdateEmitter queryUpdateEmitter) {
    super(repository);
  ...
  @SuppressWarnings("unused")
  @QueryHandler
  public List<ConductingEquipment> handle(FindAllConductingEquipmentQuery query)
      throws ProcessingException {
    return findAll(query);
  }

  /**
   * emit to subscription queries of type FindConductingEquipment, but only if the id matches
   *
  ...
  protected void emitFindConductingEquipment(ConductingEquipment entity) {
    ...
    queryUpdateEmitter.emit(
        FindConductingEquipmentQuery.class,
        query ->
            query.getFilter().getConductingEquipmentId().equals(entity.getConductingEquipmentId()),
        entity);
  }
  ...
  protected void emitFindAllConductingEquipment(ConductingEquipment entity) {
    ...
    queryUpdateEmitter.emit(FindAllConductingEquipmentQuery.class, query -> true, entity);
  }
  ...
  private static final Logger LOGGER =
      Logger.getLogger(ConductingEquipmentProjector.class.getName());
}
