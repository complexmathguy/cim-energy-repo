...
/**
 * Aggregate handler for HydroPowerPlant as outlined for the CQRS pattern, all write
 * responsibilities related to HydroPowerPlant are handled here.
 *
...
public class HydroPowerPlantAggregate {
  ...
  public HydroPowerPlantAggregate(CreateHydroPowerPlantCommand command) throws Exception {
    ...
    CreateHydroPowerPlantEvent event =
        new CreateHydroPowerPlantEvent(
            command.getHydroPowerPlantId(), command.getHydroPlantStorageType());
  ...
  public void handle(UpdateHydroPowerPlantCommand command) throws Exception {
    ...
    UpdateHydroPowerPlantEvent event =
        new UpdateHydroPowerPlantEvent(
            command.getHydroPowerPlantId(), command.getHydroPlantStorageType());
  ...
  @AggregateIdentifier private UUID hydroPowerPlantId;
