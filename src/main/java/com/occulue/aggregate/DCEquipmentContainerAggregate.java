...
/**
 * Aggregate handler for DCEquipmentContainer as outlined for the CQRS pattern, all write
 * responsibilities related to DCEquipmentContainer are handled here.
 *
...
public class DCEquipmentContainerAggregate {
  ...
  public DCEquipmentContainerAggregate(CreateDCEquipmentContainerCommand command) throws Exception {
    ...
    CreateDCEquipmentContainerEvent event =
        new CreateDCEquipmentContainerEvent(command.getDCEquipmentContainerId());
  ...
  public void handle(UpdateDCEquipmentContainerCommand command) throws Exception {
    ...
    UpdateDCEquipmentContainerEvent event =
        new UpdateDCEquipmentContainerEvent(command.getDCEquipmentContainerId());
  ...
  @AggregateIdentifier private UUID dCEquipmentContainerId;

  private static final Logger LOGGER =
      Logger.getLogger(DCEquipmentContainerAggregate.class.getName());
}
