...
/**
 * Aggregate handler for WindGeneratingUnit as outlined for the CQRS pattern, all write
 * responsibilities related to WindGeneratingUnit are handled here.
 *
...
public class WindGeneratingUnitAggregate {
  ...
  public WindGeneratingUnitAggregate(CreateWindGeneratingUnitCommand command) throws Exception {
    ...
    CreateWindGeneratingUnitEvent event =
        new CreateWindGeneratingUnitEvent(
            command.getWindGeneratingUnitId(), command.getWindGenUnitType());
  ...
  public void handle(UpdateWindGeneratingUnitCommand command) throws Exception {
    ...
    UpdateWindGeneratingUnitEvent event =
        new UpdateWindGeneratingUnitEvent(
            command.getWindGeneratingUnitId(), command.getWindGenUnitType());
  ...
  @AggregateIdentifier private UUID windGeneratingUnitId;

  private WindGenUnitKind windGenUnitType;

  private static final Logger LOGGER =
      Logger.getLogger(WindGeneratingUnitAggregate.class.getName());
}
