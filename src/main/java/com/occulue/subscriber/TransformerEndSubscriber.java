...
public class TransformerEndSubscriber extends BaseSubscriber {
  ...
  public SubscriptionQueryResult<TransformerEnd, TransformerEnd> transformerEndSubscribe(
      @DestinationVariable UUID transformerEndId) {
    return queryGateway.subscriptionQuery(
        new FindTransformerEndQuery(new LoadTransformerEndFilter(transformerEndId)),
        ResponseTypes.instanceOf(TransformerEnd.class),
        ResponseTypes.instanceOf(TransformerEnd.class));
  }
