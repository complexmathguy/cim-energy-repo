...
public class SwitchProxyTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for SwitchProxy : "
                          + successValue.getSwitchProxyId());
                  if (successValue.getSwitchProxyId().equals(switchProxyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSwitchProxyList - 1,
      ...
      subscriber
          .switchProxySubscribe(switchProxyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for SwitchProxy : "
                          + successValue.getSwitchProxyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SwitchProxyBusinessDelegate.getSwitchProxyInstance()
          .delete(new DeleteSwitchProxyCommand(entity.getSwitchProxyId()))
          .get();
