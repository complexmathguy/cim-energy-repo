...
public class FossilFuelTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    FossilFuelBusinessDelegate.getFossilFuelInstance().createFossilFuel(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for FossilFuel : " + successValue.getFossilFuelId());
                  if (successValue.getFossilFuelId().equals(fossilFuelId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfFossilFuelList - 1,
      ...
      subscriber
          .fossilFuelSubscribe(fossilFuelId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for FossilFuel : "
                          + successValue.getFossilFuelId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      FossilFuelBusinessDelegate.getFossilFuelInstance()
          .delete(new DeleteFossilFuelCommand(entity.getFossilFuelId()))
          .get();
  ...
  protected UpdateFossilFuelCommand generateUpdateCommand() {
    UpdateFossilFuelCommand command = new UpdateFossilFuelCommand(null, FuelType.values()[0]);
