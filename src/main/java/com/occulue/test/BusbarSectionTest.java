...
public class BusbarSectionTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for BusbarSection : "
                          + successValue.getBusbarSectionId());
                  if (successValue.getBusbarSectionId().equals(busbarSectionId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfBusbarSectionList - 1,
      ...
      subscriber
          .busbarSectionSubscribe(busbarSectionId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for BusbarSection : "
                          + successValue.getBusbarSectionId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected BusbarSection read() throws Throwable {
    ...
    BusbarSectionFetchOneSummary fetchOneSummary =
        new BusbarSectionFetchOneSummary(busbarSectionId);

    try {
      entity =
          BusbarSectionBusinessDelegate.getBusbarSectionInstance()
              .getBusbarSection(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      BusbarSectionBusinessDelegate proxy =
          BusbarSectionBusinessDelegate.getBusbarSectionInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      BusbarSectionBusinessDelegate.getBusbarSectionInstance()
          .delete(new DeleteBusbarSectionCommand(entity.getBusbarSectionId()))
          .get();
  ...
  protected List<BusbarSection> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getBusbarSectionId(), "-- entity in Collection has a null primary key");
