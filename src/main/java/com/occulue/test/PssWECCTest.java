...
public class PssWECCTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    pssWECCId =
        PssWECCBusinessDelegate.getPssWECCInstance().createPssWECC(generateNewCommand()).get();
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for PssWECC : " + successValue.getPssWECCId());
                  if (successValue.getPssWECCId().equals(pssWECCId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfPssWECCList - 1, "value not deleted from list");
                      LOGGER.info("**********************************************************");
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetOne update received for PssWECC : "
                          + successValue.getPssWECCId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + pssWECCId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      PssWECCBusinessDelegate.getPssWECCInstance()
          .delete(new DeletePssWECCCommand(entity.getPssWECCId()))
          .get();
  ...
  protected CreatePssWECCCommand generateNewCommand() {
    CreatePssWECCCommand command =
        new CreatePssWECCCommand(null, InputSignalKind.values()[0], InputSignalKind.values()[0]);
  ...
  protected UpdatePssWECCCommand generateUpdateCommand() {
    UpdatePssWECCCommand command =
        new UpdatePssWECCCommand(
            null,
