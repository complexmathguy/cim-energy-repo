...
public class WindPitchContEmulIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindPitchContEmulIEC : "
                          + successValue.getWindPitchContEmulIECId());
                  if (successValue.getWindPitchContEmulIECId().equals(windPitchContEmulIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindPitchContEmulIECList - 1,
      ...
      subscriber
          .windPitchContEmulIECSubscribe(windPitchContEmulIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindPitchContEmulIEC : "
                          + successValue.getWindPitchContEmulIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on windPitchContEmulIEC for windPitchContEmulIECId consumed"));
  ...
  protected WindPitchContEmulIEC read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read WindPitchContEmulIEC with primary key");
    msg.append(windPitchContEmulIECId);

    WindPitchContEmulIECFetchOneSummary fetchOneSummary =
        new WindPitchContEmulIECFetchOneSummary(windPitchContEmulIECId);

    try {
      entity =
          WindPitchContEmulIECBusinessDelegate.getWindPitchContEmulIECInstance()
              .getWindPitchContEmulIEC(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      WindPitchContEmulIECBusinessDelegate proxy =
          WindPitchContEmulIECBusinessDelegate.getWindPitchContEmulIECInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindPitchContEmulIECBusinessDelegate.getWindPitchContEmulIECInstance()
          .delete(new DeleteWindPitchContEmulIECCommand(entity.getWindPitchContEmulIECId()))
          .get();
  ...
  protected List<WindPitchContEmulIEC> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of WindPitchContEmulIEC was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getWindPitchContEmulIECId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateWindPitchContEmulIECCommand generateUpdateCommand() {
    UpdateWindPitchContEmulIECCommand command =
        new UpdateWindPitchContEmulIECCommand(
