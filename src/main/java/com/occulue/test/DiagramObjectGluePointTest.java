...
public class DiagramObjectGluePointTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DiagramObjectGluePoint : "
                          + successValue.getDiagramObjectGluePointId());
                  if (successValue.getDiagramObjectGluePointId().equals(diagramObjectGluePointId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDiagramObjectGluePointList - 1,
      ...
      subscriber
          .diagramObjectGluePointSubscribe(diagramObjectGluePointId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DiagramObjectGluePoint : "
                          + successValue.getDiagramObjectGluePointId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on diagramObjectGluePoint for diagramObjectGluePointId consumed"));
  ...
  protected DiagramObjectGluePoint read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read DiagramObjectGluePoint with primary key");
    msg.append(diagramObjectGluePointId);

    DiagramObjectGluePointFetchOneSummary fetchOneSummary =
        new DiagramObjectGluePointFetchOneSummary(diagramObjectGluePointId);

    try {
      entity =
          DiagramObjectGluePointBusinessDelegate.getDiagramObjectGluePointInstance()
              .getDiagramObjectGluePoint(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DiagramObjectGluePointBusinessDelegate proxy =
          DiagramObjectGluePointBusinessDelegate.getDiagramObjectGluePointInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read DiagramObjectGluePoint with id " + diagramObjectGluePointId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read DiagramObjectGluePoint with id " + diagramObjectGluePointId);
    ...
    try {
      DiagramObjectGluePointBusinessDelegate.getDiagramObjectGluePointInstance()
          .delete(new DeleteDiagramObjectGluePointCommand(entity.getDiagramObjectGluePointId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted DiagramObjectGluePoint with id " + diagramObjectGluePointId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete DiagramObjectGluePoint with id " + diagramObjectGluePointId);
  ...
  protected List<DiagramObjectGluePoint> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          DiagramObjectGluePointBusinessDelegate.getDiagramObjectGluePointInstance()
              .getAllDiagramObjectGluePoint();
      assertNotNull(
          collection, "An Empty collection of DiagramObjectGluePoint was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDiagramObjectGluePointId(), "-- entity in Collection has a null primary key");
