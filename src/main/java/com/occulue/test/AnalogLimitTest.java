...
public class AnalogLimitTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for AnalogLimit : "
                          + successValue.getAnalogLimitId());
                  if (successValue.getAnalogLimitId().equals(analogLimitId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfAnalogLimitList - 1,
      ...
      subscriber
          .analogLimitSubscribe(analogLimitId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for AnalogLimit : "
                          + successValue.getAnalogLimitId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      AnalogLimitBusinessDelegate.getAnalogLimitInstance()
          .delete(new DeleteAnalogLimitCommand(entity.getAnalogLimitId()))
          .get();
  ...
  protected UpdateAnalogLimitCommand generateUpdateCommand() {
    UpdateAnalogLimitCommand command = new UpdateAnalogLimitCommand(null, null);
