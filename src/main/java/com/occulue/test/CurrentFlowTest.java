...
public class CurrentFlowTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for CurrentFlow : "
                          + successValue.getCurrentFlowId());
                  if (successValue.getCurrentFlowId().equals(currentFlowId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfCurrentFlowList - 1,
      ...
      subscriber
          .currentFlowSubscribe(currentFlowId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for CurrentFlow : "
                          + successValue.getCurrentFlowId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      CurrentFlowBusinessDelegate.getCurrentFlowInstance()
          .delete(new DeleteCurrentFlowCommand(entity.getCurrentFlowId()))
          .get();
  ...
  protected CreateCurrentFlowCommand generateNewCommand() {
    CreateCurrentFlowCommand command =
        new CreateCurrentFlowCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateCurrentFlowCommand generateUpdateCommand() {
    UpdateCurrentFlowCommand command =
        new UpdateCurrentFlowCommand(
