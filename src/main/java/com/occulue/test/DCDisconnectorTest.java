...
public class DCDisconnectorTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DCDisconnector : "
                          + successValue.getDCDisconnectorId());
                  if (successValue.getDCDisconnectorId().equals(dCDisconnectorId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDCDisconnectorList - 1,
      ...
      subscriber
          .dCDisconnectorSubscribe(dCDisconnectorId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DCDisconnector : "
                          + successValue.getDCDisconnectorId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DCDisconnector read() throws Throwable {
    ...
    DCDisconnectorFetchOneSummary fetchOneSummary =
        new DCDisconnectorFetchOneSummary(dCDisconnectorId);

    try {
      entity =
          DCDisconnectorBusinessDelegate.getDCDisconnectorInstance()
              .getDCDisconnector(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DCDisconnectorBusinessDelegate proxy =
          DCDisconnectorBusinessDelegate.getDCDisconnectorInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DCDisconnectorBusinessDelegate.getDCDisconnectorInstance()
          .delete(new DeleteDCDisconnectorCommand(entity.getDCDisconnectorId()))
          .get();
  ...
  protected List<DCDisconnector> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDCDisconnectorId(), "-- entity in Collection has a null primary key");
