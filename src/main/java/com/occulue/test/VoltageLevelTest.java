...
public class VoltageLevelTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for VoltageLevel : "
                          + successValue.getVoltageLevelId());
                  if (successValue.getVoltageLevelId().equals(voltageLevelId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfVoltageLevelList - 1,
      ...
      subscriber
          .voltageLevelSubscribe(voltageLevelId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for VoltageLevel : "
                          + successValue.getVoltageLevelId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected VoltageLevel read() throws Throwable {
    ...
    try {
      entity =
          VoltageLevelBusinessDelegate.getVoltageLevelInstance().getVoltageLevel(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      VoltageLevelBusinessDelegate.getVoltageLevelInstance()
          .delete(new DeleteVoltageLevelCommand(entity.getVoltageLevelId()))
          .get();
  ...
  protected UpdateVoltageLevelCommand generateUpdateCommand() {
    UpdateVoltageLevelCommand command = new UpdateVoltageLevelCommand(null, null, null);
