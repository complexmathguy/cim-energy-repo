...
public class SvInjectionTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for SvInjection : "
                          + successValue.getSvInjectionId());
                  if (successValue.getSvInjectionId().equals(svInjectionId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSvInjectionList - 1,
      ...
      subscriber
          .svInjectionSubscribe(svInjectionId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for SvInjection : "
                          + successValue.getSvInjectionId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SvInjectionBusinessDelegate.getSvInjectionInstance()
          .delete(new DeleteSvInjectionCommand(entity.getSvInjectionId()))
          .get();
  ...
  protected UpdateSvInjectionCommand generateUpdateCommand() {
    UpdateSvInjectionCommand command = new UpdateSvInjectionCommand(null, null, null);
