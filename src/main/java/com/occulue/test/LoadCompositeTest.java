...
public class LoadCompositeTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for LoadComposite : "
                          + successValue.getLoadCompositeId());
                  if (successValue.getLoadCompositeId().equals(loadCompositeId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfLoadCompositeList - 1,
      ...
      subscriber
          .loadCompositeSubscribe(loadCompositeId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for LoadComposite : "
                          + successValue.getLoadCompositeId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected LoadComposite read() throws Throwable {
    ...
    LoadCompositeFetchOneSummary fetchOneSummary =
        new LoadCompositeFetchOneSummary(loadCompositeId);

    try {
      entity =
          LoadCompositeBusinessDelegate.getLoadCompositeInstance()
              .getLoadComposite(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LoadCompositeBusinessDelegate proxy =
          LoadCompositeBusinessDelegate.getLoadCompositeInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      LoadCompositeBusinessDelegate.getLoadCompositeInstance()
          .delete(new DeleteLoadCompositeCommand(entity.getLoadCompositeId()))
          .get();
  ...
  protected List<LoadComposite> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getLoadCompositeId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateLoadCompositeCommand generateUpdateCommand() {
    UpdateLoadCompositeCommand command =
        new UpdateLoadCompositeCommand(
