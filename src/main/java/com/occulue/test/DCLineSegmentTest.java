...
public class DCLineSegmentTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DCLineSegment : "
                          + successValue.getDCLineSegmentId());
                  if (successValue.getDCLineSegmentId().equals(dCLineSegmentId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDCLineSegmentList - 1,
      ...
      subscriber
          .dCLineSegmentSubscribe(dCLineSegmentId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DCLineSegment : "
                          + successValue.getDCLineSegmentId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DCLineSegment read() throws Throwable {
    ...
    DCLineSegmentFetchOneSummary fetchOneSummary =
        new DCLineSegmentFetchOneSummary(dCLineSegmentId);

    try {
      entity =
          DCLineSegmentBusinessDelegate.getDCLineSegmentInstance()
              .getDCLineSegment(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DCLineSegmentBusinessDelegate proxy =
          DCLineSegmentBusinessDelegate.getDCLineSegmentInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DCLineSegmentBusinessDelegate.getDCLineSegmentInstance()
          .delete(new DeleteDCLineSegmentCommand(entity.getDCLineSegmentId()))
          .get();
  ...
  protected List<DCLineSegment> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDCLineSegmentId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateDCLineSegmentCommand generateUpdateCommand() {
    UpdateDCLineSegmentCommand command =
        new UpdateDCLineSegmentCommand(null, null, null, null, null);
