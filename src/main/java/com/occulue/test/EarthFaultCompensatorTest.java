...
public class EarthFaultCompensatorTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for EarthFaultCompensator : "
                          + successValue.getEarthFaultCompensatorId());
                  if (successValue.getEarthFaultCompensatorId().equals(earthFaultCompensatorId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfEarthFaultCompensatorList - 1,
      ...
      subscriber
          .earthFaultCompensatorSubscribe(earthFaultCompensatorId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for EarthFaultCompensator : "
                          + successValue.getEarthFaultCompensatorId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on earthFaultCompensator for earthFaultCompensatorId consumed"));
  ...
  protected EarthFaultCompensator read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read EarthFaultCompensator with primary key");
    msg.append(earthFaultCompensatorId);

    EarthFaultCompensatorFetchOneSummary fetchOneSummary =
        new EarthFaultCompensatorFetchOneSummary(earthFaultCompensatorId);

    try {
      entity =
          EarthFaultCompensatorBusinessDelegate.getEarthFaultCompensatorInstance()
              .getEarthFaultCompensator(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      EarthFaultCompensatorBusinessDelegate proxy =
          EarthFaultCompensatorBusinessDelegate.getEarthFaultCompensatorInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      EarthFaultCompensatorBusinessDelegate.getEarthFaultCompensatorInstance()
          .delete(new DeleteEarthFaultCompensatorCommand(entity.getEarthFaultCompensatorId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted EarthFaultCompensator with id " + earthFaultCompensatorId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete EarthFaultCompensator with id " + earthFaultCompensatorId);
  ...
  protected List<EarthFaultCompensator> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          EarthFaultCompensatorBusinessDelegate.getEarthFaultCompensatorInstance()
              .getAllEarthFaultCompensator();
      assertNotNull(
          collection, "An Empty collection of EarthFaultCompensator was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getEarthFaultCompensatorId(), "-- entity in Collection has a null primary key");
