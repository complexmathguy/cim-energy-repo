...
public class PerCentTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    perCentId =
        PerCentBusinessDelegate.getPerCentInstance().createPerCent(generateNewCommand()).get();
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for PerCent : " + successValue.getPerCentId());
                  if (successValue.getPerCentId().equals(perCentId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfPerCentList - 1, "value not deleted from list");
                      LOGGER.info("**********************************************************");
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetOne update received for PerCent : "
                          + successValue.getPerCentId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + perCentId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      PerCentBusinessDelegate.getPerCentInstance()
          .delete(new DeletePerCentCommand(entity.getPerCentId()))
          .get();
  ...
  protected CreatePerCentCommand generateNewCommand() {
    CreatePerCentCommand command =
        new CreatePerCentCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdatePerCentCommand generateUpdateCommand() {
    UpdatePerCentCommand command =
        new UpdatePerCentCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
