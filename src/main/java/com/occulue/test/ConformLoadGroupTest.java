...
public class ConformLoadGroupTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ConformLoadGroup : "
                          + successValue.getConformLoadGroupId());
                  if (successValue.getConformLoadGroupId().equals(conformLoadGroupId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfConformLoadGroupList - 1,
      ...
      subscriber
          .conformLoadGroupSubscribe(conformLoadGroupId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ConformLoadGroup : "
                          + successValue.getConformLoadGroupId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected ConformLoadGroup read() throws Throwable {
    ...
    ConformLoadGroupFetchOneSummary fetchOneSummary =
        new ConformLoadGroupFetchOneSummary(conformLoadGroupId);

    try {
      entity =
          ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance()
              .getConformLoadGroup(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ConformLoadGroupBusinessDelegate proxy =
          ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance()
          .delete(new DeleteConformLoadGroupCommand(entity.getConformLoadGroupId()))
          .get();
  ...
  protected List<ConformLoadGroup> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          ConformLoadGroupBusinessDelegate.getConformLoadGroupInstance().getAllConformLoadGroup();
      assertNotNull(
          collection, "An Empty collection of ConformLoadGroup was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getConformLoadGroupId(), "-- entity in Collection has a null primary key");
