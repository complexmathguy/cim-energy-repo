...
public class CapacitanceTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Capacitance : "
                          + successValue.getCapacitanceId());
                  if (successValue.getCapacitanceId().equals(capacitanceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfCapacitanceList - 1,
      ...
      subscriber
          .capacitanceSubscribe(capacitanceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Capacitance : "
                          + successValue.getCapacitanceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      CapacitanceBusinessDelegate.getCapacitanceInstance()
          .delete(new DeleteCapacitanceCommand(entity.getCapacitanceId()))
          .get();
  ...
  protected CreateCapacitanceCommand generateNewCommand() {
    CreateCapacitanceCommand command =
        new CreateCapacitanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateCapacitanceCommand generateUpdateCommand() {
    UpdateCapacitanceCommand command =
        new UpdateCapacitanceCommand(
