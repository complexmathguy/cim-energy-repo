...
public class SusceptanceTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Susceptance : "
                          + successValue.getSusceptanceId());
                  if (successValue.getSusceptanceId().equals(susceptanceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSusceptanceList - 1,
      ...
      subscriber
          .susceptanceSubscribe(susceptanceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Susceptance : "
                          + successValue.getSusceptanceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SusceptanceBusinessDelegate.getSusceptanceInstance()
          .delete(new DeleteSusceptanceCommand(entity.getSusceptanceId()))
          .get();
  ...
  protected CreateSusceptanceCommand generateNewCommand() {
    CreateSusceptanceCommand command =
        new CreateSusceptanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateSusceptanceCommand generateUpdateCommand() {
    UpdateSusceptanceCommand command =
        new UpdateSusceptanceCommand(
