...
public class VoltageAdjusterDynamicsTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for VoltageAdjusterDynamics : "
                          + successValue.getVoltageAdjusterDynamicsId());
                  if (successValue
                      .getVoltageAdjusterDynamicsId()
                      .equals(voltageAdjusterDynamicsId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfVoltageAdjusterDynamicsList - 1,
      ...
      subscriber
          .voltageAdjusterDynamicsSubscribe(voltageAdjusterDynamicsId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for VoltageAdjusterDynamics : "
                          + successValue.getVoltageAdjusterDynamicsId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on voltageAdjusterDynamics for voltageAdjusterDynamicsId consumed"));
  ...
  protected VoltageAdjusterDynamics read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read VoltageAdjusterDynamics with primary key");
    msg.append(voltageAdjusterDynamicsId);

    VoltageAdjusterDynamicsFetchOneSummary fetchOneSummary =
        new VoltageAdjusterDynamicsFetchOneSummary(voltageAdjusterDynamicsId);

    try {
      entity =
          VoltageAdjusterDynamicsBusinessDelegate.getVoltageAdjusterDynamicsInstance()
              .getVoltageAdjusterDynamics(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      VoltageAdjusterDynamicsBusinessDelegate proxy =
          VoltageAdjusterDynamicsBusinessDelegate.getVoltageAdjusterDynamicsInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read VoltageAdjusterDynamics with id " + voltageAdjusterDynamicsId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read VoltageAdjusterDynamics with id " + voltageAdjusterDynamicsId);
    ...
    try {
      VoltageAdjusterDynamicsBusinessDelegate.getVoltageAdjusterDynamicsInstance()
          .delete(new DeleteVoltageAdjusterDynamicsCommand(entity.getVoltageAdjusterDynamicsId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted VoltageAdjusterDynamics with id " + voltageAdjusterDynamicsId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete VoltageAdjusterDynamics with id " + voltageAdjusterDynamicsId);
  ...
  protected List<VoltageAdjusterDynamics> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          VoltageAdjusterDynamicsBusinessDelegate.getVoltageAdjusterDynamicsInstance()
              .getAllVoltageAdjusterDynamics();
      assertNotNull(
          collection, "An Empty collection of VoltageAdjusterDynamics was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getVoltageAdjusterDynamicsId(),
