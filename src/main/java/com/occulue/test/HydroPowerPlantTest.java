...
public class HydroPowerPlantTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for HydroPowerPlant : "
                          + successValue.getHydroPowerPlantId());
                  if (successValue.getHydroPowerPlantId().equals(hydroPowerPlantId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfHydroPowerPlantList - 1,
      ...
      subscriber
          .hydroPowerPlantSubscribe(hydroPowerPlantId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for HydroPowerPlant : "
                          + successValue.getHydroPowerPlantId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected HydroPowerPlant read() throws Throwable {
    ...
    HydroPowerPlantFetchOneSummary fetchOneSummary =
        new HydroPowerPlantFetchOneSummary(hydroPowerPlantId);

    try {
      entity =
          HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance()
              .getHydroPowerPlant(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      HydroPowerPlantBusinessDelegate proxy =
          HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance()
          .delete(new DeleteHydroPowerPlantCommand(entity.getHydroPowerPlantId()))
          .get();
  ...
  protected List<HydroPowerPlant> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          HydroPowerPlantBusinessDelegate.getHydroPowerPlantInstance().getAllHydroPowerPlant();
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getHydroPowerPlantId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateHydroPowerPlantCommand generateNewCommand() {
    CreateHydroPowerPlantCommand command =
        new CreateHydroPowerPlantCommand(null, HydroPlantStorageKind.values()[0]);
  ...
  protected UpdateHydroPowerPlantCommand generateUpdateCommand() {
    UpdateHydroPowerPlantCommand command =
        new UpdateHydroPowerPlantCommand(null, HydroPlantStorageKind.values()[0]);
