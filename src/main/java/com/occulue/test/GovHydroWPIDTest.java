...
public class GovHydroWPIDTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for GovHydroWPID : "
                          + successValue.getGovHydroWPIDId());
                  if (successValue.getGovHydroWPIDId().equals(govHydroWPIDId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfGovHydroWPIDList - 1,
      ...
      subscriber
          .govHydroWPIDSubscribe(govHydroWPIDId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for GovHydroWPID : "
                          + successValue.getGovHydroWPIDId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected GovHydroWPID read() throws Throwable {
    ...
    try {
      entity =
          GovHydroWPIDBusinessDelegate.getGovHydroWPIDInstance().getGovHydroWPID(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      GovHydroWPIDBusinessDelegate.getGovHydroWPIDInstance()
          .delete(new DeleteGovHydroWPIDCommand(entity.getGovHydroWPIDId()))
          .get();
  ...
  protected UpdateGovHydroWPIDCommand generateUpdateCommand() {
    UpdateGovHydroWPIDCommand command =
        new UpdateGovHydroWPIDCommand(
