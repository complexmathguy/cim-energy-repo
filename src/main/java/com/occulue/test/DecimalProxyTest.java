...
public class DecimalProxyTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DecimalProxy : "
                          + successValue.getDecimalProxyId());
                  if (successValue.getDecimalProxyId().equals(decimalProxyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDecimalProxyList - 1,
      ...
      subscriber
          .decimalProxySubscribe(decimalProxyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DecimalProxy : "
                          + successValue.getDecimalProxyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DecimalProxy read() throws Throwable {
    ...
    try {
      entity =
          DecimalProxyBusinessDelegate.getDecimalProxyInstance().getDecimalProxy(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DecimalProxyBusinessDelegate.getDecimalProxyInstance()
          .delete(new DeleteDecimalProxyCommand(entity.getDecimalProxyId()))
          .get();
