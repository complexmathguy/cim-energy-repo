...
public class WindPlantIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindPlantIEC : "
                          + successValue.getWindPlantIECId());
                  if (successValue.getWindPlantIECId().equals(windPlantIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindPlantIECList - 1,
      ...
      subscriber
          .windPlantIECSubscribe(windPlantIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindPlantIEC : "
                          + successValue.getWindPlantIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected WindPlantIEC read() throws Throwable {
    ...
    try {
      entity =
          WindPlantIECBusinessDelegate.getWindPlantIECInstance().getWindPlantIEC(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindPlantIECBusinessDelegate.getWindPlantIECInstance()
          .delete(new DeleteWindPlantIECCommand(entity.getWindPlantIECId()))
          .get();
