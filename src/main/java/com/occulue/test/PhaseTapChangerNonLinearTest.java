...
public class PhaseTapChangerNonLinearTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for PhaseTapChangerNonLinear : "
                          + successValue.getPhaseTapChangerNonLinearId());
                  if (successValue
                      .getPhaseTapChangerNonLinearId()
                      .equals(phaseTapChangerNonLinearId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfPhaseTapChangerNonLinearList - 1,
      ...
      subscriber
          .phaseTapChangerNonLinearSubscribe(phaseTapChangerNonLinearId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for PhaseTapChangerNonLinear : "
                          + successValue.getPhaseTapChangerNonLinearId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on phaseTapChangerNonLinear for phaseTapChangerNonLinearId consumed"));
  ...
  protected PhaseTapChangerNonLinear read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read PhaseTapChangerNonLinear with primary key");
    msg.append(phaseTapChangerNonLinearId);

    PhaseTapChangerNonLinearFetchOneSummary fetchOneSummary =
        new PhaseTapChangerNonLinearFetchOneSummary(phaseTapChangerNonLinearId);

    try {
      entity =
          PhaseTapChangerNonLinearBusinessDelegate.getPhaseTapChangerNonLinearInstance()
              .getPhaseTapChangerNonLinear(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      PhaseTapChangerNonLinearBusinessDelegate proxy =
          PhaseTapChangerNonLinearBusinessDelegate.getPhaseTapChangerNonLinearInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read PhaseTapChangerNonLinear with id " + phaseTapChangerNonLinearId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read PhaseTapChangerNonLinear with id " + phaseTapChangerNonLinearId);
    ...
    try {
      PhaseTapChangerNonLinearBusinessDelegate.getPhaseTapChangerNonLinearInstance()
          .delete(new DeletePhaseTapChangerNonLinearCommand(entity.getPhaseTapChangerNonLinearId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted PhaseTapChangerNonLinear with id " + phaseTapChangerNonLinearId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete PhaseTapChangerNonLinear with id " + phaseTapChangerNonLinearId);
  ...
  protected List<PhaseTapChangerNonLinear> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          PhaseTapChangerNonLinearBusinessDelegate.getPhaseTapChangerNonLinearInstance()
              .getAllPhaseTapChangerNonLinear();
      assertNotNull(
          collection, "An Empty collection of PhaseTapChangerNonLinear was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getPhaseTapChangerNonLinearId(),
  ...
  protected UpdatePhaseTapChangerNonLinearCommand generateUpdateCommand() {
    UpdatePhaseTapChangerNonLinearCommand command =
        new UpdatePhaseTapChangerNonLinearCommand(null, null, null, null);
