...
public class CapacitancePerLengthTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for CapacitancePerLength : "
                          + successValue.getCapacitancePerLengthId());
                  if (successValue.getCapacitancePerLengthId().equals(capacitancePerLengthId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfCapacitancePerLengthList - 1,
      ...
      subscriber
          .capacitancePerLengthSubscribe(capacitancePerLengthId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for CapacitancePerLength : "
                          + successValue.getCapacitancePerLengthId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on capacitancePerLength for capacitancePerLengthId consumed"));
  ...
  protected CapacitancePerLength read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read CapacitancePerLength with primary key");
    msg.append(capacitancePerLengthId);

    CapacitancePerLengthFetchOneSummary fetchOneSummary =
        new CapacitancePerLengthFetchOneSummary(capacitancePerLengthId);

    try {
      entity =
          CapacitancePerLengthBusinessDelegate.getCapacitancePerLengthInstance()
              .getCapacitancePerLength(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      CapacitancePerLengthBusinessDelegate proxy =
          CapacitancePerLengthBusinessDelegate.getCapacitancePerLengthInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      CapacitancePerLengthBusinessDelegate.getCapacitancePerLengthInstance()
          .delete(new DeleteCapacitancePerLengthCommand(entity.getCapacitancePerLengthId()))
          .get();
  ...
  protected List<CapacitancePerLength> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of CapacitancePerLength was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getCapacitancePerLengthId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateCapacitancePerLengthCommand generateNewCommand() {
    CreateCapacitancePerLengthCommand command =
        new CreateCapacitancePerLengthCommand(
            null,
  ...
  protected UpdateCapacitancePerLengthCommand generateUpdateCommand() {
    UpdateCapacitancePerLengthCommand command =
        new UpdateCapacitancePerLengthCommand(
            null,
            UnitMultiplier.values()[0],
            UnitSymbol.values()[0],
            UnitMultiplier.values()[0],
