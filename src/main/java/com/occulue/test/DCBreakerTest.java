...
public class DCBreakerTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    DCBreakerBusinessDelegate.getDCBreakerInstance().createDCBreaker(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DCBreaker : " + successValue.getDCBreakerId());
                  if (successValue.getDCBreakerId().equals(dCBreakerId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDCBreakerList - 1,
      ...
      subscriber
          .dCBreakerSubscribe(dCBreakerId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DCBreaker : "
                          + successValue.getDCBreakerId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + dCBreakerId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DCBreakerBusinessDelegate.getDCBreakerInstance()
          .delete(new DeleteDCBreakerCommand(entity.getDCBreakerId()))
          .get();
