...
public class AngleRadiansTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for AngleRadians : "
                          + successValue.getAngleRadiansId());
                  if (successValue.getAngleRadiansId().equals(angleRadiansId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfAngleRadiansList - 1,
      ...
      subscriber
          .angleRadiansSubscribe(angleRadiansId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for AngleRadians : "
                          + successValue.getAngleRadiansId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected AngleRadians read() throws Throwable {
    ...
    try {
      entity =
          AngleRadiansBusinessDelegate.getAngleRadiansInstance().getAngleRadians(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      AngleRadiansBusinessDelegate.getAngleRadiansInstance()
          .delete(new DeleteAngleRadiansCommand(entity.getAngleRadiansId()))
          .get();
  ...
  protected CreateAngleRadiansCommand generateNewCommand() {
    CreateAngleRadiansCommand command =
        new CreateAngleRadiansCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateAngleRadiansCommand generateUpdateCommand() {
    UpdateAngleRadiansCommand command =
        new UpdateAngleRadiansCommand(
