...
public class AnalogValueTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for AnalogValue : "
                          + successValue.getAnalogValueId());
                  if (successValue.getAnalogValueId().equals(analogValueId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfAnalogValueList - 1,
      ...
      subscriber
          .analogValueSubscribe(analogValueId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for AnalogValue : "
                          + successValue.getAnalogValueId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      AnalogValueBusinessDelegate.getAnalogValueInstance()
          .delete(new DeleteAnalogValueCommand(entity.getAnalogValueId()))
          .get();
  ...
  protected UpdateAnalogValueCommand generateUpdateCommand() {
    UpdateAnalogValueCommand command = new UpdateAnalogValueCommand(null, null);
