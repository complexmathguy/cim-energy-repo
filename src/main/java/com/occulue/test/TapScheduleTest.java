...
public class TapScheduleTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for TapSchedule : "
                          + successValue.getTapScheduleId());
                  if (successValue.getTapScheduleId().equals(tapScheduleId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfTapScheduleList - 1,
      ...
      subscriber
          .tapScheduleSubscribe(tapScheduleId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for TapSchedule : "
                          + successValue.getTapScheduleId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      TapScheduleBusinessDelegate.getTapScheduleInstance()
          .delete(new DeleteTapScheduleCommand(entity.getTapScheduleId()))
          .get();
