...
public class NonConformLoadTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for NonConformLoad : "
                          + successValue.getNonConformLoadId());
                  if (successValue.getNonConformLoadId().equals(nonConformLoadId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfNonConformLoadList - 1,
      ...
      subscriber
          .nonConformLoadSubscribe(nonConformLoadId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for NonConformLoad : "
                          + successValue.getNonConformLoadId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected NonConformLoad read() throws Throwable {
    ...
    NonConformLoadFetchOneSummary fetchOneSummary =
        new NonConformLoadFetchOneSummary(nonConformLoadId);

    try {
      entity =
          NonConformLoadBusinessDelegate.getNonConformLoadInstance()
              .getNonConformLoad(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      NonConformLoadBusinessDelegate proxy =
          NonConformLoadBusinessDelegate.getNonConformLoadInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      NonConformLoadBusinessDelegate.getNonConformLoadInstance()
          .delete(new DeleteNonConformLoadCommand(entity.getNonConformLoadId()))
          .get();
  ...
  protected List<NonConformLoad> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getNonConformLoadId(), "-- entity in Collection has a null primary key");
