...
public class ActivePowerTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ActivePower : "
                          + successValue.getActivePowerId());
                  if (successValue.getActivePowerId().equals(activePowerId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfActivePowerList - 1,
      ...
      subscriber
          .activePowerSubscribe(activePowerId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ActivePower : "
                          + successValue.getActivePowerId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ActivePowerBusinessDelegate.getActivePowerInstance()
          .delete(new DeleteActivePowerCommand(entity.getActivePowerId()))
          .get();
  ...
  protected CreateActivePowerCommand generateNewCommand() {
    CreateActivePowerCommand command =
        new CreateActivePowerCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateActivePowerCommand generateUpdateCommand() {
    UpdateActivePowerCommand command =
        new UpdateActivePowerCommand(
