...
public class WindAeroConstIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindAeroConstIEC : "
                          + successValue.getWindAeroConstIECId());
                  if (successValue.getWindAeroConstIECId().equals(windAeroConstIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindAeroConstIECList - 1,
      ...
      subscriber
          .windAeroConstIECSubscribe(windAeroConstIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindAeroConstIEC : "
                          + successValue.getWindAeroConstIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected WindAeroConstIEC read() throws Throwable {
    ...
    WindAeroConstIECFetchOneSummary fetchOneSummary =
        new WindAeroConstIECFetchOneSummary(windAeroConstIECId);

    try {
      entity =
          WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance()
              .getWindAeroConstIEC(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      WindAeroConstIECBusinessDelegate proxy =
          WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance()
          .delete(new DeleteWindAeroConstIECCommand(entity.getWindAeroConstIECId()))
          .get();
  ...
  protected List<WindAeroConstIEC> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          WindAeroConstIECBusinessDelegate.getWindAeroConstIECInstance().getAllWindAeroConstIEC();
      assertNotNull(
          collection, "An Empty collection of WindAeroConstIEC was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getWindAeroConstIECId(), "-- entity in Collection has a null primary key");
