...
public class ControlTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    controlId =
        ControlBusinessDelegate.getControlInstance().createControl(generateNewCommand()).get();
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Control : " + successValue.getControlId());
                  if (successValue.getControlId().equals(controlId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfControlList - 1, "value not deleted from list");
                      LOGGER.info("**********************************************************");
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetOne update received for Control : "
                          + successValue.getControlId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + controlId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ControlBusinessDelegate.getControlInstance()
          .delete(new DeleteControlCommand(entity.getControlId()))
          .get();
  ...
  protected CreateControlCommand generateNewCommand() {
    CreateControlCommand command =
        new CreateControlCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateControlCommand generateUpdateCommand() {
    UpdateControlCommand command =
        new UpdateControlCommand(
            null, null, null, null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
