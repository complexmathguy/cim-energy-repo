...
public class DisconnectorTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Disconnector : "
                          + successValue.getDisconnectorId());
                  if (successValue.getDisconnectorId().equals(disconnectorId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDisconnectorList - 1,
      ...
      subscriber
          .disconnectorSubscribe(disconnectorId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Disconnector : "
                          + successValue.getDisconnectorId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected Disconnector read() throws Throwable {
    ...
    try {
      entity =
          DisconnectorBusinessDelegate.getDisconnectorInstance().getDisconnector(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DisconnectorBusinessDelegate.getDisconnectorInstance()
          .delete(new DeleteDisconnectorCommand(entity.getDisconnectorId()))
          .get();
