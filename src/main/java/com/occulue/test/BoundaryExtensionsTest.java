...
public class BoundaryExtensionsTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for BoundaryExtensions : "
                          + successValue.getBoundaryExtensionsId());
                  if (successValue.getBoundaryExtensionsId().equals(boundaryExtensionsId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfBoundaryExtensionsList - 1,
      ...
      subscriber
          .boundaryExtensionsSubscribe(boundaryExtensionsId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for BoundaryExtensions : "
                          + successValue.getBoundaryExtensionsId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on boundaryExtensions for boundaryExtensionsId consumed"));
  ...
  protected BoundaryExtensions read() throws Throwable {
    ...
    BoundaryExtensionsFetchOneSummary fetchOneSummary =
        new BoundaryExtensionsFetchOneSummary(boundaryExtensionsId);

    try {
      entity =
          BoundaryExtensionsBusinessDelegate.getBoundaryExtensionsInstance()
              .getBoundaryExtensions(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      BoundaryExtensionsBusinessDelegate proxy =
          BoundaryExtensionsBusinessDelegate.getBoundaryExtensionsInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      BoundaryExtensionsBusinessDelegate.getBoundaryExtensionsInstance()
          .delete(new DeleteBoundaryExtensionsCommand(entity.getBoundaryExtensionsId()))
          .get();
  ...
  protected List<BoundaryExtensions> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          BoundaryExtensionsBusinessDelegate.getBoundaryExtensionsInstance()
              .getAllBoundaryExtensions();
      assertNotNull(
          collection, "An Empty collection of BoundaryExtensions was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getBoundaryExtensionsId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateBoundaryExtensionsCommand generateUpdateCommand() {
    UpdateBoundaryExtensionsCommand command =
        new UpdateBoundaryExtensionsCommand(null, null, null, null, null, null, null, null);
