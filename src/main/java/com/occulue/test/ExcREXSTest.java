...
public class ExcREXSTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    excREXSId =
        ExcREXSBusinessDelegate.getExcREXSInstance().createExcREXS(generateNewCommand()).get();
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ExcREXS : " + successValue.getExcREXSId());
                  if (successValue.getExcREXSId().equals(excREXSId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfExcREXSList - 1, "value not deleted from list");
                      LOGGER.info("**********************************************************");
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetOne update received for ExcREXS : "
                          + successValue.getExcREXSId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + excREXSId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ExcREXSBusinessDelegate.getExcREXSInstance()
          .delete(new DeleteExcREXSCommand(entity.getExcREXSId()))
          .get();
  ...
  protected CreateExcREXSCommand generateNewCommand() {
    CreateExcREXSCommand command =
        new CreateExcREXSCommand(null, ExcREXSFeedbackSignalKind.values()[0]);
  ...
  protected UpdateExcREXSCommand generateUpdateCommand() {
    UpdateExcREXSCommand command =
        new UpdateExcREXSCommand(
            null,
            null,
            null,
            ExcREXSFeedbackSignalKind.values()[0],
            null,
            null,
            null,
            null,
