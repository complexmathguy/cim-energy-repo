...
public class StationSupplyTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for StationSupply : "
                          + successValue.getStationSupplyId());
                  if (successValue.getStationSupplyId().equals(stationSupplyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfStationSupplyList - 1,
      ...
      subscriber
          .stationSupplySubscribe(stationSupplyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for StationSupply : "
                          + successValue.getStationSupplyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected StationSupply read() throws Throwable {
    ...
    StationSupplyFetchOneSummary fetchOneSummary =
        new StationSupplyFetchOneSummary(stationSupplyId);

    try {
      entity =
          StationSupplyBusinessDelegate.getStationSupplyInstance()
              .getStationSupply(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      StationSupplyBusinessDelegate proxy =
          StationSupplyBusinessDelegate.getStationSupplyInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      StationSupplyBusinessDelegate.getStationSupplyInstance()
          .delete(new DeleteStationSupplyCommand(entity.getStationSupplyId()))
          .get();
  ...
  protected List<StationSupply> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getStationSupplyId(), "-- entity in Collection has a null primary key");
