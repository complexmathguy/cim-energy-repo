...
public class RegulatingControlTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for RegulatingControl : "
                          + successValue.getRegulatingControlId());
                  if (successValue.getRegulatingControlId().equals(regulatingControlId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfRegulatingControlList - 1,
      ...
      subscriber
          .regulatingControlSubscribe(regulatingControlId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for RegulatingControl : "
                          + successValue.getRegulatingControlId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on regulatingControl for regulatingControlId consumed"));
  ...
  protected RegulatingControl read() throws Throwable {
    ...
    RegulatingControlFetchOneSummary fetchOneSummary =
        new RegulatingControlFetchOneSummary(regulatingControlId);

    try {
      entity =
          RegulatingControlBusinessDelegate.getRegulatingControlInstance()
              .getRegulatingControl(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      RegulatingControlBusinessDelegate proxy =
          RegulatingControlBusinessDelegate.getRegulatingControlInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      RegulatingControlBusinessDelegate.getRegulatingControlInstance()
          .delete(new DeleteRegulatingControlCommand(entity.getRegulatingControlId()))
          .get();
  ...
  protected List<RegulatingControl> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of RegulatingControl was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getRegulatingControlId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateRegulatingControlCommand generateNewCommand() {
    CreateRegulatingControlCommand command =
        new CreateRegulatingControlCommand(null, RegulatingControlModeKind.values()[0]);
  ...
  protected UpdateRegulatingControlCommand generateUpdateCommand() {
    UpdateRegulatingControlCommand command =
        new UpdateRegulatingControlCommand(null, RegulatingControlModeKind.values()[0]);
