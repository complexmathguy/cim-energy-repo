...
public class EquivalentInjectionTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for EquivalentInjection : "
                          + successValue.getEquivalentInjectionId());
                  if (successValue.getEquivalentInjectionId().equals(equivalentInjectionId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfEquivalentInjectionList - 1,
      ...
      subscriber
          .equivalentInjectionSubscribe(equivalentInjectionId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for EquivalentInjection : "
                          + successValue.getEquivalentInjectionId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on equivalentInjection for equivalentInjectionId consumed"));
  ...
  protected EquivalentInjection read() throws Throwable {
    ...
    EquivalentInjectionFetchOneSummary fetchOneSummary =
        new EquivalentInjectionFetchOneSummary(equivalentInjectionId);

    try {
      entity =
          EquivalentInjectionBusinessDelegate.getEquivalentInjectionInstance()
              .getEquivalentInjection(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      EquivalentInjectionBusinessDelegate proxy =
          EquivalentInjectionBusinessDelegate.getEquivalentInjectionInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      EquivalentInjectionBusinessDelegate.getEquivalentInjectionInstance()
          .delete(new DeleteEquivalentInjectionCommand(entity.getEquivalentInjectionId()))
          .get();
  ...
  protected List<EquivalentInjection> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          EquivalentInjectionBusinessDelegate.getEquivalentInjectionInstance()
              .getAllEquivalentInjection();
      assertNotNull(
          collection, "An Empty collection of EquivalentInjection was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getEquivalentInjectionId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateEquivalentInjectionCommand generateUpdateCommand() {
    UpdateEquivalentInjectionCommand command =
        new UpdateEquivalentInjectionCommand(
