...
public class PhaseTapChangerTableTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for PhaseTapChangerTable : "
                          + successValue.getPhaseTapChangerTableId());
                  if (successValue.getPhaseTapChangerTableId().equals(phaseTapChangerTableId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfPhaseTapChangerTableList - 1,
      ...
      subscriber
          .phaseTapChangerTableSubscribe(phaseTapChangerTableId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for PhaseTapChangerTable : "
                          + successValue.getPhaseTapChangerTableId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on phaseTapChangerTable for phaseTapChangerTableId consumed"));
  ...
  protected PhaseTapChangerTable read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read PhaseTapChangerTable with primary key");
    msg.append(phaseTapChangerTableId);

    PhaseTapChangerTableFetchOneSummary fetchOneSummary =
        new PhaseTapChangerTableFetchOneSummary(phaseTapChangerTableId);

    try {
      entity =
          PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance()
              .getPhaseTapChangerTable(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      PhaseTapChangerTableBusinessDelegate proxy =
          PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance()
          .delete(new DeletePhaseTapChangerTableCommand(entity.getPhaseTapChangerTableId()))
          .get();
  ...
  protected List<PhaseTapChangerTable> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          PhaseTapChangerTableBusinessDelegate.getPhaseTapChangerTableInstance()
              .getAllPhaseTapChangerTable();
      assertNotNull(
          collection, "An Empty collection of PhaseTapChangerTable was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getPhaseTapChangerTableId(), "-- entity in Collection has a null primary key");
