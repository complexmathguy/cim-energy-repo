...
public class DiagramLayoutVersionTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DiagramLayoutVersion : "
                          + successValue.getDiagramLayoutVersionId());
                  if (successValue.getDiagramLayoutVersionId().equals(diagramLayoutVersionId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDiagramLayoutVersionList - 1,
      ...
      subscriber
          .diagramLayoutVersionSubscribe(diagramLayoutVersionId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DiagramLayoutVersion : "
                          + successValue.getDiagramLayoutVersionId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on diagramLayoutVersion for diagramLayoutVersionId consumed"));
  ...
  protected DiagramLayoutVersion read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read DiagramLayoutVersion with primary key");
    msg.append(diagramLayoutVersionId);

    DiagramLayoutVersionFetchOneSummary fetchOneSummary =
        new DiagramLayoutVersionFetchOneSummary(diagramLayoutVersionId);

    try {
      entity =
          DiagramLayoutVersionBusinessDelegate.getDiagramLayoutVersionInstance()
              .getDiagramLayoutVersion(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DiagramLayoutVersionBusinessDelegate proxy =
          DiagramLayoutVersionBusinessDelegate.getDiagramLayoutVersionInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DiagramLayoutVersionBusinessDelegate.getDiagramLayoutVersionInstance()
          .delete(new DeleteDiagramLayoutVersionCommand(entity.getDiagramLayoutVersionId()))
          .get();
  ...
  protected List<DiagramLayoutVersion> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          DiagramLayoutVersionBusinessDelegate.getDiagramLayoutVersionInstance()
              .getAllDiagramLayoutVersion();
      assertNotNull(
          collection, "An Empty collection of DiagramLayoutVersion was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDiagramLayoutVersionId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateDiagramLayoutVersionCommand generateUpdateCommand() {
    UpdateDiagramLayoutVersionCommand command =
        new UpdateDiagramLayoutVersionCommand(
