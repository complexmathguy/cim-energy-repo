...
public class MeasurementTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Measurement : "
                          + successValue.getMeasurementId());
                  if (successValue.getMeasurementId().equals(measurementId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfMeasurementList - 1,
      ...
      subscriber
          .measurementSubscribe(measurementId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Measurement : "
                          + successValue.getMeasurementId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      MeasurementBusinessDelegate.getMeasurementInstance()
          .delete(new DeleteMeasurementCommand(entity.getMeasurementId()))
          .get();
  ...
  protected CreateMeasurementCommand generateNewCommand() {
    CreateMeasurementCommand command =
        new CreateMeasurementCommand(
            null, PhaseCode.values()[0], UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateMeasurementCommand generateUpdateCommand() {
    UpdateMeasurementCommand command =
        new UpdateMeasurementCommand(
            null, null, PhaseCode.values()[0], UnitMultiplier.values()[0], UnitSymbol.values()[0]);
