...
public class DiscreteValueTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DiscreteValue : "
                          + successValue.getDiscreteValueId());
                  if (successValue.getDiscreteValueId().equals(discreteValueId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDiscreteValueList - 1,
      ...
      subscriber
          .discreteValueSubscribe(discreteValueId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DiscreteValue : "
                          + successValue.getDiscreteValueId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DiscreteValue read() throws Throwable {
    ...
    DiscreteValueFetchOneSummary fetchOneSummary =
        new DiscreteValueFetchOneSummary(discreteValueId);

    try {
      entity =
          DiscreteValueBusinessDelegate.getDiscreteValueInstance()
              .getDiscreteValue(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DiscreteValueBusinessDelegate proxy =
          DiscreteValueBusinessDelegate.getDiscreteValueInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DiscreteValueBusinessDelegate.getDiscreteValueInstance()
          .delete(new DeleteDiscreteValueCommand(entity.getDiscreteValueId()))
          .get();
  ...
  protected List<DiscreteValue> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDiscreteValueId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateDiscreteValueCommand generateUpdateCommand() {
    UpdateDiscreteValueCommand command = new UpdateDiscreteValueCommand(null, null);
