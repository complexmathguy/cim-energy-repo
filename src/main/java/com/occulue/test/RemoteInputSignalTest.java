...
public class RemoteInputSignalTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for RemoteInputSignal : "
                          + successValue.getRemoteInputSignalId());
                  if (successValue.getRemoteInputSignalId().equals(remoteInputSignalId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfRemoteInputSignalList - 1,
      ...
      subscriber
          .remoteInputSignalSubscribe(remoteInputSignalId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for RemoteInputSignal : "
                          + successValue.getRemoteInputSignalId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on remoteInputSignal for remoteInputSignalId consumed"));
  ...
  protected RemoteInputSignal read() throws Throwable {
    ...
    RemoteInputSignalFetchOneSummary fetchOneSummary =
        new RemoteInputSignalFetchOneSummary(remoteInputSignalId);

    try {
      entity =
          RemoteInputSignalBusinessDelegate.getRemoteInputSignalInstance()
              .getRemoteInputSignal(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      RemoteInputSignalBusinessDelegate proxy =
          RemoteInputSignalBusinessDelegate.getRemoteInputSignalInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      RemoteInputSignalBusinessDelegate.getRemoteInputSignalInstance()
          .delete(new DeleteRemoteInputSignalCommand(entity.getRemoteInputSignalId()))
          .get();
  ...
  protected List<RemoteInputSignal> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of RemoteInputSignal was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getRemoteInputSignalId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateRemoteInputSignalCommand generateNewCommand() {
    CreateRemoteInputSignalCommand command =
        new CreateRemoteInputSignalCommand(null, RemoteSignalKind.values()[0]);
  ...
  protected UpdateRemoteInputSignalCommand generateUpdateCommand() {
    UpdateRemoteInputSignalCommand command =
        new UpdateRemoteInputSignalCommand(null, RemoteSignalKind.values()[0]);
