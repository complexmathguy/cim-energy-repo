...
public class MeasurementValueQualityTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for MeasurementValueQuality : "
                          + successValue.getMeasurementValueQualityId());
                  if (successValue
                      .getMeasurementValueQualityId()
                      .equals(measurementValueQualityId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfMeasurementValueQualityList - 1,
      ...
      subscriber
          .measurementValueQualitySubscribe(measurementValueQualityId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for MeasurementValueQuality : "
                          + successValue.getMeasurementValueQualityId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on measurementValueQuality for measurementValueQualityId consumed"));
  ...
  protected MeasurementValueQuality read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read MeasurementValueQuality with primary key");
    msg.append(measurementValueQualityId);

    MeasurementValueQualityFetchOneSummary fetchOneSummary =
        new MeasurementValueQualityFetchOneSummary(measurementValueQualityId);

    try {
      entity =
          MeasurementValueQualityBusinessDelegate.getMeasurementValueQualityInstance()
              .getMeasurementValueQuality(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      MeasurementValueQualityBusinessDelegate proxy =
          MeasurementValueQualityBusinessDelegate.getMeasurementValueQualityInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read MeasurementValueQuality with id " + measurementValueQualityId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read MeasurementValueQuality with id " + measurementValueQualityId);
    ...
    try {
      MeasurementValueQualityBusinessDelegate.getMeasurementValueQualityInstance()
          .delete(new DeleteMeasurementValueQualityCommand(entity.getMeasurementValueQualityId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted MeasurementValueQuality with id " + measurementValueQualityId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete MeasurementValueQuality with id " + measurementValueQualityId);
  ...
  protected List<MeasurementValueQuality> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          MeasurementValueQualityBusinessDelegate.getMeasurementValueQualityInstance()
              .getAllMeasurementValueQuality();
      assertNotNull(
          collection, "An Empty collection of MeasurementValueQuality was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getMeasurementValueQualityId(),
