...
public class GovHydroPIDTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for GovHydroPID : "
                          + successValue.getGovHydroPIDId());
                  if (successValue.getGovHydroPIDId().equals(govHydroPIDId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfGovHydroPIDList - 1,
      ...
      subscriber
          .govHydroPIDSubscribe(govHydroPIDId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for GovHydroPID : "
                          + successValue.getGovHydroPIDId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      GovHydroPIDBusinessDelegate.getGovHydroPIDInstance()
          .delete(new DeleteGovHydroPIDCommand(entity.getGovHydroPIDId()))
          .get();
  ...
  protected UpdateGovHydroPIDCommand generateUpdateCommand() {
    UpdateGovHydroPIDCommand command =
        new UpdateGovHydroPIDCommand(
