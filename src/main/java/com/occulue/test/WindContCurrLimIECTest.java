...
public class WindContCurrLimIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindContCurrLimIEC : "
                          + successValue.getWindContCurrLimIECId());
                  if (successValue.getWindContCurrLimIECId().equals(windContCurrLimIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindContCurrLimIECList - 1,
      ...
      subscriber
          .windContCurrLimIECSubscribe(windContCurrLimIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindContCurrLimIEC : "
                          + successValue.getWindContCurrLimIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on windContCurrLimIEC for windContCurrLimIECId consumed"));
  ...
  protected WindContCurrLimIEC read() throws Throwable {
    ...
    WindContCurrLimIECFetchOneSummary fetchOneSummary =
        new WindContCurrLimIECFetchOneSummary(windContCurrLimIECId);

    try {
      entity =
          WindContCurrLimIECBusinessDelegate.getWindContCurrLimIECInstance()
              .getWindContCurrLimIEC(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      WindContCurrLimIECBusinessDelegate proxy =
          WindContCurrLimIECBusinessDelegate.getWindContCurrLimIECInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindContCurrLimIECBusinessDelegate.getWindContCurrLimIECInstance()
          .delete(new DeleteWindContCurrLimIECCommand(entity.getWindContCurrLimIECId()))
          .get();
  ...
  protected List<WindContCurrLimIEC> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          WindContCurrLimIECBusinessDelegate.getWindContCurrLimIECInstance()
              .getAllWindContCurrLimIEC();
      assertNotNull(
          collection, "An Empty collection of WindContCurrLimIEC was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getWindContCurrLimIECId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateWindContCurrLimIECCommand generateUpdateCommand() {
    UpdateWindContCurrLimIECCommand command =
        new UpdateWindContCurrLimIECCommand(null, null, null, null, null, null);
