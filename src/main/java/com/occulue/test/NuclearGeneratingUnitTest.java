...
public class NuclearGeneratingUnitTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for NuclearGeneratingUnit : "
                          + successValue.getNuclearGeneratingUnitId());
                  if (successValue.getNuclearGeneratingUnitId().equals(nuclearGeneratingUnitId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfNuclearGeneratingUnitList - 1,
      ...
      subscriber
          .nuclearGeneratingUnitSubscribe(nuclearGeneratingUnitId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for NuclearGeneratingUnit : "
                          + successValue.getNuclearGeneratingUnitId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on nuclearGeneratingUnit for nuclearGeneratingUnitId consumed"));
  ...
  protected NuclearGeneratingUnit read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read NuclearGeneratingUnit with primary key");
    msg.append(nuclearGeneratingUnitId);

    NuclearGeneratingUnitFetchOneSummary fetchOneSummary =
        new NuclearGeneratingUnitFetchOneSummary(nuclearGeneratingUnitId);

    try {
      entity =
          NuclearGeneratingUnitBusinessDelegate.getNuclearGeneratingUnitInstance()
              .getNuclearGeneratingUnit(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      NuclearGeneratingUnitBusinessDelegate proxy =
          NuclearGeneratingUnitBusinessDelegate.getNuclearGeneratingUnitInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      NuclearGeneratingUnitBusinessDelegate.getNuclearGeneratingUnitInstance()
          .delete(new DeleteNuclearGeneratingUnitCommand(entity.getNuclearGeneratingUnitId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted NuclearGeneratingUnit with id " + nuclearGeneratingUnitId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete NuclearGeneratingUnit with id " + nuclearGeneratingUnitId);
  ...
  protected List<NuclearGeneratingUnit> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of NuclearGeneratingUnit was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getNuclearGeneratingUnitId(), "-- entity in Collection has a null primary key");
