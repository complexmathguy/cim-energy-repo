...
public class ExcitationSystemDynamicsTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ExcitationSystemDynamics : "
                          + successValue.getExcitationSystemDynamicsId());
                  if (successValue
                      .getExcitationSystemDynamicsId()
                      .equals(excitationSystemDynamicsId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfExcitationSystemDynamicsList - 1,
      ...
      subscriber
          .excitationSystemDynamicsSubscribe(excitationSystemDynamicsId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ExcitationSystemDynamics : "
                          + successValue.getExcitationSystemDynamicsId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on excitationSystemDynamics for excitationSystemDynamicsId consumed"));
  ...
  protected ExcitationSystemDynamics read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read ExcitationSystemDynamics with primary key");
    msg.append(excitationSystemDynamicsId);

    ExcitationSystemDynamicsFetchOneSummary fetchOneSummary =
        new ExcitationSystemDynamicsFetchOneSummary(excitationSystemDynamicsId);

    try {
      entity =
          ExcitationSystemDynamicsBusinessDelegate.getExcitationSystemDynamicsInstance()
              .getExcitationSystemDynamics(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ExcitationSystemDynamicsBusinessDelegate proxy =
          ExcitationSystemDynamicsBusinessDelegate.getExcitationSystemDynamicsInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read ExcitationSystemDynamics with id " + excitationSystemDynamicsId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read ExcitationSystemDynamics with id " + excitationSystemDynamicsId);
    ...
    try {
      ExcitationSystemDynamicsBusinessDelegate.getExcitationSystemDynamicsInstance()
          .delete(new DeleteExcitationSystemDynamicsCommand(entity.getExcitationSystemDynamicsId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted ExcitationSystemDynamics with id " + excitationSystemDynamicsId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete ExcitationSystemDynamics with id " + excitationSystemDynamicsId);
  ...
  protected List<ExcitationSystemDynamics> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          ExcitationSystemDynamicsBusinessDelegate.getExcitationSystemDynamicsInstance()
              .getAllExcitationSystemDynamics();
      assertNotNull(
          collection, "An Empty collection of ExcitationSystemDynamics was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getExcitationSystemDynamicsId(),
