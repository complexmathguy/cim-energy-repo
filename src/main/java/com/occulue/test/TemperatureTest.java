...
public class TemperatureTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Temperature : "
                          + successValue.getTemperatureId());
                  if (successValue.getTemperatureId().equals(temperatureId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfTemperatureList - 1,
      ...
      subscriber
          .temperatureSubscribe(temperatureId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Temperature : "
                          + successValue.getTemperatureId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      TemperatureBusinessDelegate.getTemperatureInstance()
          .delete(new DeleteTemperatureCommand(entity.getTemperatureId()))
          .get();
  ...
  protected CreateTemperatureCommand generateNewCommand() {
    CreateTemperatureCommand command =
        new CreateTemperatureCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateTemperatureCommand generateUpdateCommand() {
    UpdateTemperatureCommand command =
        new UpdateTemperatureCommand(
