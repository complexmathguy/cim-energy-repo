...
public class VsConverterTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for VsConverter : "
                          + successValue.getVsConverterId());
                  if (successValue.getVsConverterId().equals(vsConverterId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfVsConverterList - 1,
      ...
      subscriber
          .vsConverterSubscribe(vsConverterId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for VsConverter : "
                          + successValue.getVsConverterId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      VsConverterBusinessDelegate.getVsConverterInstance()
          .delete(new DeleteVsConverterCommand(entity.getVsConverterId()))
          .get();
  ...
  protected UpdateVsConverterCommand generateUpdateCommand() {
    UpdateVsConverterCommand command = new UpdateVsConverterCommand(null, null, null);
