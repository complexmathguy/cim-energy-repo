...
public class TransformerEndTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for TransformerEnd : "
                          + successValue.getTransformerEndId());
                  if (successValue.getTransformerEndId().equals(transformerEndId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfTransformerEndList - 1,
      ...
      subscriber
          .transformerEndSubscribe(transformerEndId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for TransformerEnd : "
                          + successValue.getTransformerEndId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected TransformerEnd read() throws Throwable {
    ...
    TransformerEndFetchOneSummary fetchOneSummary =
        new TransformerEndFetchOneSummary(transformerEndId);

    try {
      entity =
          TransformerEndBusinessDelegate.getTransformerEndInstance()
              .getTransformerEnd(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      TransformerEndBusinessDelegate proxy =
          TransformerEndBusinessDelegate.getTransformerEndInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      TransformerEndBusinessDelegate.getTransformerEndInstance()
          .delete(new DeleteTransformerEndCommand(entity.getTransformerEndId()))
          .get();
  ...
  protected List<TransformerEnd> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          TransformerEndBusinessDelegate.getTransformerEndInstance().getAllTransformerEnd();
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getTransformerEndId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateTransformerEndCommand generateUpdateCommand() {
    UpdateTransformerEndCommand command =
        new UpdateTransformerEndCommand(null, null, null, null, null);
