...
public class ControlAreaTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ControlArea : "
                          + successValue.getControlAreaId());
                  if (successValue.getControlAreaId().equals(controlAreaId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfControlAreaList - 1,
      ...
      subscriber
          .controlAreaSubscribe(controlAreaId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ControlArea : "
                          + successValue.getControlAreaId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ControlAreaBusinessDelegate.getControlAreaInstance()
          .delete(new DeleteControlAreaCommand(entity.getControlAreaId()))
          .get();
  ...
  protected CreateControlAreaCommand generateNewCommand() {
    CreateControlAreaCommand command =
        new CreateControlAreaCommand(null, ControlAreaTypeKind.values()[0]);
  ...
  protected UpdateControlAreaCommand generateUpdateCommand() {
    UpdateControlAreaCommand command =
        new UpdateControlAreaCommand(null, ControlAreaTypeKind.values()[0]);
