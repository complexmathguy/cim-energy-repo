...
public class GovHydroWEHTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for GovHydroWEH : "
                          + successValue.getGovHydroWEHId());
                  if (successValue.getGovHydroWEHId().equals(govHydroWEHId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfGovHydroWEHList - 1,
      ...
      subscriber
          .govHydroWEHSubscribe(govHydroWEHId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for GovHydroWEH : "
                          + successValue.getGovHydroWEHId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      GovHydroWEHBusinessDelegate.getGovHydroWEHInstance()
          .delete(new DeleteGovHydroWEHCommand(entity.getGovHydroWEHId()))
          .get();
