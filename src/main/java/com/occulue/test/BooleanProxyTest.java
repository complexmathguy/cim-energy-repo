...
public class BooleanProxyTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for BooleanProxy : "
                          + successValue.getBooleanProxyId());
                  if (successValue.getBooleanProxyId().equals(booleanProxyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfBooleanProxyList - 1,
      ...
      subscriber
          .booleanProxySubscribe(booleanProxyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for BooleanProxy : "
                          + successValue.getBooleanProxyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected BooleanProxy read() throws Throwable {
    ...
    try {
      entity =
          BooleanProxyBusinessDelegate.getBooleanProxyInstance().getBooleanProxy(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      BooleanProxyBusinessDelegate.getBooleanProxyInstance()
          .delete(new DeleteBooleanProxyCommand(entity.getBooleanProxyId()))
          .get();
