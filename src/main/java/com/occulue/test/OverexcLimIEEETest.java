...
public class OverexcLimIEEETest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for OverexcLimIEEE : "
                          + successValue.getOverexcLimIEEEId());
                  if (successValue.getOverexcLimIEEEId().equals(overexcLimIEEEId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfOverexcLimIEEEList - 1,
      ...
      subscriber
          .overexcLimIEEESubscribe(overexcLimIEEEId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for OverexcLimIEEE : "
                          + successValue.getOverexcLimIEEEId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected OverexcLimIEEE read() throws Throwable {
    ...
    OverexcLimIEEEFetchOneSummary fetchOneSummary =
        new OverexcLimIEEEFetchOneSummary(overexcLimIEEEId);

    try {
      entity =
          OverexcLimIEEEBusinessDelegate.getOverexcLimIEEEInstance()
              .getOverexcLimIEEE(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      OverexcLimIEEEBusinessDelegate proxy =
          OverexcLimIEEEBusinessDelegate.getOverexcLimIEEEInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      OverexcLimIEEEBusinessDelegate.getOverexcLimIEEEInstance()
          .delete(new DeleteOverexcLimIEEECommand(entity.getOverexcLimIEEEId()))
          .get();
  ...
  protected List<OverexcLimIEEE> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getOverexcLimIEEEId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateOverexcLimIEEECommand generateUpdateCommand() {
    UpdateOverexcLimIEEECommand command =
        new UpdateOverexcLimIEEECommand(null, null, null, null, null, null, null);
