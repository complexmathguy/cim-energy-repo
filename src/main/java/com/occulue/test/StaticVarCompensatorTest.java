...
public class StaticVarCompensatorTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for StaticVarCompensator : "
                          + successValue.getStaticVarCompensatorId());
                  if (successValue.getStaticVarCompensatorId().equals(staticVarCompensatorId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfStaticVarCompensatorList - 1,
      ...
      subscriber
          .staticVarCompensatorSubscribe(staticVarCompensatorId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for StaticVarCompensator : "
                          + successValue.getStaticVarCompensatorId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on staticVarCompensator for staticVarCompensatorId consumed"));
  ...
  protected StaticVarCompensator read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read StaticVarCompensator with primary key");
    msg.append(staticVarCompensatorId);

    StaticVarCompensatorFetchOneSummary fetchOneSummary =
        new StaticVarCompensatorFetchOneSummary(staticVarCompensatorId);

    try {
      entity =
          StaticVarCompensatorBusinessDelegate.getStaticVarCompensatorInstance()
              .getStaticVarCompensator(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      StaticVarCompensatorBusinessDelegate proxy =
          StaticVarCompensatorBusinessDelegate.getStaticVarCompensatorInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      StaticVarCompensatorBusinessDelegate.getStaticVarCompensatorInstance()
          .delete(new DeleteStaticVarCompensatorCommand(entity.getStaticVarCompensatorId()))
          .get();
  ...
  protected List<StaticVarCompensator> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          StaticVarCompensatorBusinessDelegate.getStaticVarCompensatorInstance()
              .getAllStaticVarCompensator();
      assertNotNull(
          collection, "An Empty collection of StaticVarCompensator was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getStaticVarCompensatorId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateStaticVarCompensatorCommand generateNewCommand() {
    CreateStaticVarCompensatorCommand command =
        new CreateStaticVarCompensatorCommand(null, SVCControlMode.values()[0]);
  ...
  protected UpdateStaticVarCompensatorCommand generateUpdateCommand() {
    UpdateStaticVarCompensatorCommand command =
        new UpdateStaticVarCompensatorCommand(
            null, null, null, null, SVCControlMode.values()[0], null);
