...
public class BaseVoltageTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for BaseVoltage : "
                          + successValue.getBaseVoltageId());
                  if (successValue.getBaseVoltageId().equals(baseVoltageId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfBaseVoltageList - 1,
      ...
      subscriber
          .baseVoltageSubscribe(baseVoltageId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for BaseVoltage : "
                          + successValue.getBaseVoltageId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      BaseVoltageBusinessDelegate.getBaseVoltageInstance()
          .delete(new DeleteBaseVoltageCommand(entity.getBaseVoltageId()))
          .get();
