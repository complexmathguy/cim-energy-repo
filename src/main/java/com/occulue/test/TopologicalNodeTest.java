...
public class TopologicalNodeTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for TopologicalNode : "
                          + successValue.getTopologicalNodeId());
                  if (successValue.getTopologicalNodeId().equals(topologicalNodeId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfTopologicalNodeList - 1,
      ...
      subscriber
          .topologicalNodeSubscribe(topologicalNodeId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for TopologicalNode : "
                          + successValue.getTopologicalNodeId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected TopologicalNode read() throws Throwable {
    ...
    TopologicalNodeFetchOneSummary fetchOneSummary =
        new TopologicalNodeFetchOneSummary(topologicalNodeId);

    try {
      entity =
          TopologicalNodeBusinessDelegate.getTopologicalNodeInstance()
              .getTopologicalNode(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      TopologicalNodeBusinessDelegate proxy =
          TopologicalNodeBusinessDelegate.getTopologicalNodeInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      TopologicalNodeBusinessDelegate.getTopologicalNodeInstance()
          .delete(new DeleteTopologicalNodeCommand(entity.getTopologicalNodeId()))
          .get();
  ...
  protected List<TopologicalNode> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getTopologicalNodeId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateTopologicalNodeCommand generateUpdateCommand() {
    UpdateTopologicalNodeCommand command =
        new UpdateTopologicalNodeCommand(null, null, null, null, null, null, null, null);
