...
public class UnresolvednameTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Unresolvedname : "
                          + successValue.getUnresolvednameId());
                  if (successValue.getUnresolvednameId().equals(unresolvednameId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfUnresolvednameList - 1,
      ...
      subscriber
          .unresolvednameSubscribe(unresolvednameId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Unresolvedname : "
                          + successValue.getUnresolvednameId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected Unresolvedname read() throws Throwable {
    ...
    UnresolvednameFetchOneSummary fetchOneSummary =
        new UnresolvednameFetchOneSummary(unresolvednameId);

    try {
      entity =
          UnresolvednameBusinessDelegate.getUnresolvednameInstance()
              .getUnresolvedname(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      UnresolvednameBusinessDelegate proxy =
          UnresolvednameBusinessDelegate.getUnresolvednameInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      UnresolvednameBusinessDelegate.getUnresolvednameInstance()
          .delete(new DeleteUnresolvednameCommand(entity.getUnresolvednameId()))
          .get();
  ...
  protected List<Unresolvedname> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          UnresolvednameBusinessDelegate.getUnresolvednameInstance().getAllUnresolvedname();
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getUnresolvednameId(), "-- entity in Collection has a null primary key");
