...
public class EnergySourceTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for EnergySource : "
                          + successValue.getEnergySourceId());
                  if (successValue.getEnergySourceId().equals(energySourceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfEnergySourceList - 1,
      ...
      subscriber
          .energySourceSubscribe(energySourceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for EnergySource : "
                          + successValue.getEnergySourceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected EnergySource read() throws Throwable {
    ...
    try {
      entity =
          EnergySourceBusinessDelegate.getEnergySourceInstance().getEnergySource(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      EnergySourceBusinessDelegate.getEnergySourceInstance()
          .delete(new DeleteEnergySourceCommand(entity.getEnergySourceId()))
          .get();
