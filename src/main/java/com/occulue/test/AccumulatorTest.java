...
public class AccumulatorTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Accumulator : "
                          + successValue.getAccumulatorId());
                  if (successValue.getAccumulatorId().equals(accumulatorId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfAccumulatorList - 1,
      ...
      subscriber
          .accumulatorSubscribe(accumulatorId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Accumulator : "
                          + successValue.getAccumulatorId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      AccumulatorBusinessDelegate.getAccumulatorInstance()
          .delete(new DeleteAccumulatorCommand(entity.getAccumulatorId()))
          .get();
