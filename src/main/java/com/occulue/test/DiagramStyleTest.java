...
public class DiagramStyleTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DiagramStyle : "
                          + successValue.getDiagramStyleId());
                  if (successValue.getDiagramStyleId().equals(diagramStyleId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDiagramStyleList - 1,
      ...
      subscriber
          .diagramStyleSubscribe(diagramStyleId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DiagramStyle : "
                          + successValue.getDiagramStyleId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DiagramStyle read() throws Throwable {
    ...
    try {
      entity =
          DiagramStyleBusinessDelegate.getDiagramStyleInstance().getDiagramStyle(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DiagramStyleBusinessDelegate.getDiagramStyleInstance()
          .delete(new DeleteDiagramStyleCommand(entity.getDiagramStyleId()))
          .get();
