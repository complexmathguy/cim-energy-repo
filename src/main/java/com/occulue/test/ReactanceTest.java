...
public class ReactanceTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    ReactanceBusinessDelegate.getReactanceInstance().createReactance(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Reactance : " + successValue.getReactanceId());
                  if (successValue.getReactanceId().equals(reactanceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfReactanceList - 1,
      ...
      subscriber
          .reactanceSubscribe(reactanceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Reactance : "
                          + successValue.getReactanceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + reactanceId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ReactanceBusinessDelegate.getReactanceInstance()
          .delete(new DeleteReactanceCommand(entity.getReactanceId()))
          .get();
  ...
  protected CreateReactanceCommand generateNewCommand() {
    CreateReactanceCommand command =
        new CreateReactanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateReactanceCommand generateUpdateCommand() {
    UpdateReactanceCommand command =
        new UpdateReactanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
