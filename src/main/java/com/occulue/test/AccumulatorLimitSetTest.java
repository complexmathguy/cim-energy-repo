...
public class AccumulatorLimitSetTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for AccumulatorLimitSet : "
                          + successValue.getAccumulatorLimitSetId());
                  if (successValue.getAccumulatorLimitSetId().equals(accumulatorLimitSetId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfAccumulatorLimitSetList - 1,
      ...
      subscriber
          .accumulatorLimitSetSubscribe(accumulatorLimitSetId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for AccumulatorLimitSet : "
                          + successValue.getAccumulatorLimitSetId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on accumulatorLimitSet for accumulatorLimitSetId consumed"));
  ...
  protected AccumulatorLimitSet read() throws Throwable {
    ...
    AccumulatorLimitSetFetchOneSummary fetchOneSummary =
        new AccumulatorLimitSetFetchOneSummary(accumulatorLimitSetId);

    try {
      entity =
          AccumulatorLimitSetBusinessDelegate.getAccumulatorLimitSetInstance()
              .getAccumulatorLimitSet(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      AccumulatorLimitSetBusinessDelegate proxy =
          AccumulatorLimitSetBusinessDelegate.getAccumulatorLimitSetInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      AccumulatorLimitSetBusinessDelegate.getAccumulatorLimitSetInstance()
          .delete(new DeleteAccumulatorLimitSetCommand(entity.getAccumulatorLimitSetId()))
          .get();
  ...
  protected List<AccumulatorLimitSet> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of AccumulatorLimitSet was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getAccumulatorLimitSetId(), "-- entity in Collection has a null primary key");
