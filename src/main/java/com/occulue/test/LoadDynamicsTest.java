...
public class LoadDynamicsTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for LoadDynamics : "
                          + successValue.getLoadDynamicsId());
                  if (successValue.getLoadDynamicsId().equals(loadDynamicsId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfLoadDynamicsList - 1,
      ...
      subscriber
          .loadDynamicsSubscribe(loadDynamicsId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for LoadDynamics : "
                          + successValue.getLoadDynamicsId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected LoadDynamics read() throws Throwable {
    ...
    try {
      entity =
          LoadDynamicsBusinessDelegate.getLoadDynamicsInstance().getLoadDynamics(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      LoadDynamicsBusinessDelegate.getLoadDynamicsInstance()
          .delete(new DeleteLoadDynamicsCommand(entity.getLoadDynamicsId()))
          .get();
