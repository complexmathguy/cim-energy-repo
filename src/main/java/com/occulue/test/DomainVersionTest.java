...
public class DomainVersionTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DomainVersion : "
                          + successValue.getDomainVersionId());
                  if (successValue.getDomainVersionId().equals(domainVersionId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDomainVersionList - 1,
      ...
      subscriber
          .domainVersionSubscribe(domainVersionId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DomainVersion : "
                          + successValue.getDomainVersionId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DomainVersion read() throws Throwable {
    ...
    DomainVersionFetchOneSummary fetchOneSummary =
        new DomainVersionFetchOneSummary(domainVersionId);

    try {
      entity =
          DomainVersionBusinessDelegate.getDomainVersionInstance()
              .getDomainVersion(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DomainVersionBusinessDelegate proxy =
          DomainVersionBusinessDelegate.getDomainVersionInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DomainVersionBusinessDelegate.getDomainVersionInstance()
          .delete(new DeleteDomainVersionCommand(entity.getDomainVersionId()))
          .get();
  ...
  protected List<DomainVersion> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDomainVersionId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateDomainVersionCommand generateUpdateCommand() {
    UpdateDomainVersionCommand command =
        new UpdateDomainVersionCommand(null, null, null, null, null);
