...
public class AngleDegreesTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for AngleDegrees : "
                          + successValue.getAngleDegreesId());
                  if (successValue.getAngleDegreesId().equals(angleDegreesId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfAngleDegreesList - 1,
      ...
      subscriber
          .angleDegreesSubscribe(angleDegreesId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for AngleDegrees : "
                          + successValue.getAngleDegreesId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected AngleDegrees read() throws Throwable {
    ...
    try {
      entity =
          AngleDegreesBusinessDelegate.getAngleDegreesInstance().getAngleDegrees(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      AngleDegreesBusinessDelegate.getAngleDegreesInstance()
          .delete(new DeleteAngleDegreesCommand(entity.getAngleDegreesId()))
          .get();
  ...
  protected CreateAngleDegreesCommand generateNewCommand() {
    CreateAngleDegreesCommand command =
        new CreateAngleDegreesCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateAngleDegreesCommand generateUpdateCommand() {
    UpdateAngleDegreesCommand command =
        new UpdateAngleDegreesCommand(
