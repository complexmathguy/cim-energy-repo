...
public class SubstationTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    SubstationBusinessDelegate.getSubstationInstance().createSubstation(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Substation : " + successValue.getSubstationId());
                  if (successValue.getSubstationId().equals(substationId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSubstationList - 1,
      ...
      subscriber
          .substationSubscribe(substationId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Substation : "
                          + successValue.getSubstationId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SubstationBusinessDelegate.getSubstationInstance()
          .delete(new DeleteSubstationCommand(entity.getSubstationId()))
          .get();
