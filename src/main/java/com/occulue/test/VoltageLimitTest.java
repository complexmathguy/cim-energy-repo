...
public class VoltageLimitTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for VoltageLimit : "
                          + successValue.getVoltageLimitId());
                  if (successValue.getVoltageLimitId().equals(voltageLimitId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfVoltageLimitList - 1,
      ...
      subscriber
          .voltageLimitSubscribe(voltageLimitId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for VoltageLimit : "
                          + successValue.getVoltageLimitId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected VoltageLimit read() throws Throwable {
    ...
    try {
      entity =
          VoltageLimitBusinessDelegate.getVoltageLimitInstance().getVoltageLimit(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      VoltageLimitBusinessDelegate.getVoltageLimitInstance()
          .delete(new DeleteVoltageLimitCommand(entity.getVoltageLimitId()))
          .get();
