...
public class AccumulatorResetTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for AccumulatorReset : "
                          + successValue.getAccumulatorResetId());
                  if (successValue.getAccumulatorResetId().equals(accumulatorResetId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfAccumulatorResetList - 1,
      ...
      subscriber
          .accumulatorResetSubscribe(accumulatorResetId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for AccumulatorReset : "
                          + successValue.getAccumulatorResetId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected AccumulatorReset read() throws Throwable {
    ...
    AccumulatorResetFetchOneSummary fetchOneSummary =
        new AccumulatorResetFetchOneSummary(accumulatorResetId);

    try {
      entity =
          AccumulatorResetBusinessDelegate.getAccumulatorResetInstance()
              .getAccumulatorReset(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      AccumulatorResetBusinessDelegate proxy =
          AccumulatorResetBusinessDelegate.getAccumulatorResetInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      AccumulatorResetBusinessDelegate.getAccumulatorResetInstance()
          .delete(new DeleteAccumulatorResetCommand(entity.getAccumulatorResetId()))
          .get();
  ...
  protected List<AccumulatorReset> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of AccumulatorReset was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getAccumulatorResetId(), "-- entity in Collection has a null primary key");
