...
public class StateVariablesVersionTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for StateVariablesVersion : "
                          + successValue.getStateVariablesVersionId());
                  if (successValue.getStateVariablesVersionId().equals(stateVariablesVersionId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfStateVariablesVersionList - 1,
      ...
      subscriber
          .stateVariablesVersionSubscribe(stateVariablesVersionId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for StateVariablesVersion : "
                          + successValue.getStateVariablesVersionId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on stateVariablesVersion for stateVariablesVersionId consumed"));
  ...
  protected StateVariablesVersion read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read StateVariablesVersion with primary key");
    msg.append(stateVariablesVersionId);

    StateVariablesVersionFetchOneSummary fetchOneSummary =
        new StateVariablesVersionFetchOneSummary(stateVariablesVersionId);

    try {
      entity =
          StateVariablesVersionBusinessDelegate.getStateVariablesVersionInstance()
              .getStateVariablesVersion(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      StateVariablesVersionBusinessDelegate proxy =
          StateVariablesVersionBusinessDelegate.getStateVariablesVersionInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      StateVariablesVersionBusinessDelegate.getStateVariablesVersionInstance()
          .delete(new DeleteStateVariablesVersionCommand(entity.getStateVariablesVersionId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted StateVariablesVersion with id " + stateVariablesVersionId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete StateVariablesVersion with id " + stateVariablesVersionId);
  ...
  protected List<StateVariablesVersion> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of StateVariablesVersion was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getStateVariablesVersionId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateStateVariablesVersionCommand generateUpdateCommand() {
    UpdateStateVariablesVersionCommand command =
        new UpdateStateVariablesVersionCommand(
