...
public class StringProxyTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for StringProxy : "
                          + successValue.getStringProxyId());
                  if (successValue.getStringProxyId().equals(stringProxyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfStringProxyList - 1,
      ...
      subscriber
          .stringProxySubscribe(stringProxyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for StringProxy : "
                          + successValue.getStringProxyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      StringProxyBusinessDelegate.getStringProxyInstance()
          .delete(new DeleteStringProxyCommand(entity.getStringProxyId()))
          .get();
