...
public class RegulatingCondEqTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for RegulatingCondEq : "
                          + successValue.getRegulatingCondEqId());
                  if (successValue.getRegulatingCondEqId().equals(regulatingCondEqId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfRegulatingCondEqList - 1,
      ...
      subscriber
          .regulatingCondEqSubscribe(regulatingCondEqId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for RegulatingCondEq : "
                          + successValue.getRegulatingCondEqId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected RegulatingCondEq read() throws Throwable {
    ...
    RegulatingCondEqFetchOneSummary fetchOneSummary =
        new RegulatingCondEqFetchOneSummary(regulatingCondEqId);

    try {
      entity =
          RegulatingCondEqBusinessDelegate.getRegulatingCondEqInstance()
              .getRegulatingCondEq(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      RegulatingCondEqBusinessDelegate proxy =
          RegulatingCondEqBusinessDelegate.getRegulatingCondEqInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      RegulatingCondEqBusinessDelegate.getRegulatingCondEqInstance()
          .delete(new DeleteRegulatingCondEqCommand(entity.getRegulatingCondEqId()))
          .get();
  ...
  protected List<RegulatingCondEq> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          RegulatingCondEqBusinessDelegate.getRegulatingCondEqInstance().getAllRegulatingCondEq();
      assertNotNull(
          collection, "An Empty collection of RegulatingCondEq was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getRegulatingCondEqId(), "-- entity in Collection has a null primary key");
