...
public class SecondsTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    secondsId =
        SecondsBusinessDelegate.getSecondsInstance().createSeconds(generateNewCommand()).get();
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Seconds : " + successValue.getSecondsId());
                  if (successValue.getSecondsId().equals(secondsId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSecondsList - 1, "value not deleted from list");
                      LOGGER.info("**********************************************************");
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetOne update received for Seconds : "
                          + successValue.getSecondsId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + secondsId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SecondsBusinessDelegate.getSecondsInstance()
          .delete(new DeleteSecondsCommand(entity.getSecondsId()))
          .get();
  ...
  protected CreateSecondsCommand generateNewCommand() {
    CreateSecondsCommand command =
        new CreateSecondsCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateSecondsCommand generateUpdateCommand() {
    UpdateSecondsCommand command =
        new UpdateSecondsCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
