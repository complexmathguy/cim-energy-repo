...
public class ShuntCompensatorTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ShuntCompensator : "
                          + successValue.getShuntCompensatorId());
                  if (successValue.getShuntCompensatorId().equals(shuntCompensatorId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfShuntCompensatorList - 1,
      ...
      subscriber
          .shuntCompensatorSubscribe(shuntCompensatorId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ShuntCompensator : "
                          + successValue.getShuntCompensatorId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected ShuntCompensator read() throws Throwable {
    ...
    ShuntCompensatorFetchOneSummary fetchOneSummary =
        new ShuntCompensatorFetchOneSummary(shuntCompensatorId);

    try {
      entity =
          ShuntCompensatorBusinessDelegate.getShuntCompensatorInstance()
              .getShuntCompensator(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ShuntCompensatorBusinessDelegate proxy =
          ShuntCompensatorBusinessDelegate.getShuntCompensatorInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ShuntCompensatorBusinessDelegate.getShuntCompensatorInstance()
          .delete(new DeleteShuntCompensatorCommand(entity.getShuntCompensatorId()))
          .get();
  ...
  protected List<ShuntCompensator> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          ShuntCompensatorBusinessDelegate.getShuntCompensatorInstance().getAllShuntCompensator();
      assertNotNull(
          collection, "An Empty collection of ShuntCompensator was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getShuntCompensatorId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateShuntCompensatorCommand generateUpdateCommand() {
    UpdateShuntCompensatorCommand command =
        new UpdateShuntCompensatorCommand(null, null, null, null, null, null, null, null, null);
