...
public class PUTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    pUId = PUBusinessDelegate.getPUInstance().createPU(generateNewCommand()).get();
    ...
    PUBusinessDelegate.getPUInstance().createPU(generateNewCommand()).get();
  }
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetOne update received for PU : "
                          + successValue.getPUId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + pUId + " : command -" + command + " : " + e);
  ...
  protected CreatePUCommand generateNewCommand() {
    CreatePUCommand command =
        new CreatePUCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdatePUCommand generateUpdateCommand() {
    UpdatePUCommand command =
        new UpdatePUCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
