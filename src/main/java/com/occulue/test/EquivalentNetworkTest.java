...
public class EquivalentNetworkTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for EquivalentNetwork : "
                          + successValue.getEquivalentNetworkId());
                  if (successValue.getEquivalentNetworkId().equals(equivalentNetworkId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfEquivalentNetworkList - 1,
      ...
      subscriber
          .equivalentNetworkSubscribe(equivalentNetworkId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for EquivalentNetwork : "
                          + successValue.getEquivalentNetworkId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on equivalentNetwork for equivalentNetworkId consumed"));
  ...
  protected EquivalentNetwork read() throws Throwable {
    ...
    EquivalentNetworkFetchOneSummary fetchOneSummary =
        new EquivalentNetworkFetchOneSummary(equivalentNetworkId);

    try {
      entity =
          EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance()
              .getEquivalentNetwork(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      EquivalentNetworkBusinessDelegate proxy =
          EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance()
          .delete(new DeleteEquivalentNetworkCommand(entity.getEquivalentNetworkId()))
          .get();
  ...
  protected List<EquivalentNetwork> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          EquivalentNetworkBusinessDelegate.getEquivalentNetworkInstance()
              .getAllEquivalentNetwork();
      assertNotNull(
          collection, "An Empty collection of EquivalentNetwork was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getEquivalentNetworkId(), "-- entity in Collection has a null primary key");
