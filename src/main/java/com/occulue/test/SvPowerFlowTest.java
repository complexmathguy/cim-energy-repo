...
public class SvPowerFlowTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for SvPowerFlow : "
                          + successValue.getSvPowerFlowId());
                  if (successValue.getSvPowerFlowId().equals(svPowerFlowId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSvPowerFlowList - 1,
      ...
      subscriber
          .svPowerFlowSubscribe(svPowerFlowId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for SvPowerFlow : "
                          + successValue.getSvPowerFlowId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SvPowerFlowBusinessDelegate.getSvPowerFlowInstance()
          .delete(new DeleteSvPowerFlowCommand(entity.getSvPowerFlowId()))
          .get();
  ...
  protected UpdateSvPowerFlowCommand generateUpdateCommand() {
    UpdateSvPowerFlowCommand command = new UpdateSvPowerFlowCommand(null, null, null);
