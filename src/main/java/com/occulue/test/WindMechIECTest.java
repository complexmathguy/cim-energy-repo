...
public class WindMechIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindMechIEC : "
                          + successValue.getWindMechIECId());
                  if (successValue.getWindMechIECId().equals(windMechIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindMechIECList - 1,
      ...
      subscriber
          .windMechIECSubscribe(windMechIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindMechIEC : "
                          + successValue.getWindMechIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindMechIECBusinessDelegate.getWindMechIECInstance()
          .delete(new DeleteWindMechIECCommand(entity.getWindMechIECId()))
          .get();
