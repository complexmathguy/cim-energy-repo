...
public class DynamicsVersionTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DynamicsVersion : "
                          + successValue.getDynamicsVersionId());
                  if (successValue.getDynamicsVersionId().equals(dynamicsVersionId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDynamicsVersionList - 1,
      ...
      subscriber
          .dynamicsVersionSubscribe(dynamicsVersionId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DynamicsVersion : "
                          + successValue.getDynamicsVersionId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DynamicsVersion read() throws Throwable {
    ...
    DynamicsVersionFetchOneSummary fetchOneSummary =
        new DynamicsVersionFetchOneSummary(dynamicsVersionId);

    try {
      entity =
          DynamicsVersionBusinessDelegate.getDynamicsVersionInstance()
              .getDynamicsVersion(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DynamicsVersionBusinessDelegate proxy =
          DynamicsVersionBusinessDelegate.getDynamicsVersionInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DynamicsVersionBusinessDelegate.getDynamicsVersionInstance()
          .delete(new DeleteDynamicsVersionCommand(entity.getDynamicsVersionId()))
          .get();
  ...
  protected List<DynamicsVersion> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDynamicsVersionId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateDynamicsVersionCommand generateUpdateCommand() {
    UpdateDynamicsVersionCommand command =
        new UpdateDynamicsVersionCommand(
