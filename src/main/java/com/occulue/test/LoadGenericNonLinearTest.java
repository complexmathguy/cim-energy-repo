...
public class LoadGenericNonLinearTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for LoadGenericNonLinear : "
                          + successValue.getLoadGenericNonLinearId());
                  if (successValue.getLoadGenericNonLinearId().equals(loadGenericNonLinearId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfLoadGenericNonLinearList - 1,
      ...
      subscriber
          .loadGenericNonLinearSubscribe(loadGenericNonLinearId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for LoadGenericNonLinear : "
                          + successValue.getLoadGenericNonLinearId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on loadGenericNonLinear for loadGenericNonLinearId consumed"));
  ...
  protected LoadGenericNonLinear read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read LoadGenericNonLinear with primary key");
    msg.append(loadGenericNonLinearId);

    LoadGenericNonLinearFetchOneSummary fetchOneSummary =
        new LoadGenericNonLinearFetchOneSummary(loadGenericNonLinearId);

    try {
      entity =
          LoadGenericNonLinearBusinessDelegate.getLoadGenericNonLinearInstance()
              .getLoadGenericNonLinear(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LoadGenericNonLinearBusinessDelegate proxy =
          LoadGenericNonLinearBusinessDelegate.getLoadGenericNonLinearInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      LoadGenericNonLinearBusinessDelegate.getLoadGenericNonLinearInstance()
          .delete(new DeleteLoadGenericNonLinearCommand(entity.getLoadGenericNonLinearId()))
          .get();
  ...
  protected List<LoadGenericNonLinear> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of LoadGenericNonLinear was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getLoadGenericNonLinearId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateLoadGenericNonLinearCommand generateNewCommand() {
    CreateLoadGenericNonLinearCommand command =
        new CreateLoadGenericNonLinearCommand(null, GenericNonLinearLoadModelKind.values()[0]);
  ...
  protected UpdateLoadGenericNonLinearCommand generateUpdateCommand() {
    UpdateLoadGenericNonLinearCommand command =
        new UpdateLoadGenericNonLinearCommand(
            null,
            null,
            null,
            GenericNonLinearLoadModelKind.values()[0],
            null,
            null,
            null,
            null,
