...
public class ConformLoadTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ConformLoad : "
                          + successValue.getConformLoadId());
                  if (successValue.getConformLoadId().equals(conformLoadId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfConformLoadList - 1,
      ...
      subscriber
          .conformLoadSubscribe(conformLoadId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ConformLoad : "
                          + successValue.getConformLoadId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ConformLoadBusinessDelegate.getConformLoadInstance()
          .delete(new DeleteConformLoadCommand(entity.getConformLoadId()))
          .get();
