...
public class LoadStaticTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    LoadStaticBusinessDelegate.getLoadStaticInstance().createLoadStatic(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for LoadStatic : " + successValue.getLoadStaticId());
                  if (successValue.getLoadStaticId().equals(loadStaticId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfLoadStaticList - 1,
      ...
      subscriber
          .loadStaticSubscribe(loadStaticId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for LoadStatic : "
                          + successValue.getLoadStaticId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      LoadStaticBusinessDelegate.getLoadStaticInstance()
          .delete(new DeleteLoadStaticCommand(entity.getLoadStaticId()))
          .get();
  ...
  protected CreateLoadStaticCommand generateNewCommand() {
    CreateLoadStaticCommand command =
        new CreateLoadStaticCommand(null, StaticLoadModelKind.values()[0]);
  ...
  protected UpdateLoadStaticCommand generateUpdateCommand() {
    UpdateLoadStaticCommand command =
        new UpdateLoadStaticCommand(
            null,
            null,
            null,
            null,
