...
public class GeneratingUnitTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for GeneratingUnit : "
                          + successValue.getGeneratingUnitId());
                  if (successValue.getGeneratingUnitId().equals(generatingUnitId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfGeneratingUnitList - 1,
      ...
      subscriber
          .generatingUnitSubscribe(generatingUnitId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for GeneratingUnit : "
                          + successValue.getGeneratingUnitId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected GeneratingUnit read() throws Throwable {
    ...
    GeneratingUnitFetchOneSummary fetchOneSummary =
        new GeneratingUnitFetchOneSummary(generatingUnitId);

    try {
      entity =
          GeneratingUnitBusinessDelegate.getGeneratingUnitInstance()
              .getGeneratingUnit(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      GeneratingUnitBusinessDelegate proxy =
          GeneratingUnitBusinessDelegate.getGeneratingUnitInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      GeneratingUnitBusinessDelegate.getGeneratingUnitInstance()
          .delete(new DeleteGeneratingUnitCommand(entity.getGeneratingUnitId()))
          .get();
  ...
  protected List<GeneratingUnit> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getGeneratingUnitId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateGeneratingUnitCommand generateNewCommand() {
    CreateGeneratingUnitCommand command =
        new CreateGeneratingUnitCommand(null, GeneratorControlSource.values()[0]);
  ...
  protected UpdateGeneratingUnitCommand generateUpdateCommand() {
    UpdateGeneratingUnitCommand command =
        new UpdateGeneratingUnitCommand(
            null,
            GeneratorControlSource.values()[0],
            null,
            null,
            null,
            null,
