...
public class DynamicsFunctionBlockTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DynamicsFunctionBlock : "
                          + successValue.getDynamicsFunctionBlockId());
                  if (successValue.getDynamicsFunctionBlockId().equals(dynamicsFunctionBlockId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDynamicsFunctionBlockList - 1,
      ...
      subscriber
          .dynamicsFunctionBlockSubscribe(dynamicsFunctionBlockId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DynamicsFunctionBlock : "
                          + successValue.getDynamicsFunctionBlockId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on dynamicsFunctionBlock for dynamicsFunctionBlockId consumed"));
  ...
  protected DynamicsFunctionBlock read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read DynamicsFunctionBlock with primary key");
    msg.append(dynamicsFunctionBlockId);

    DynamicsFunctionBlockFetchOneSummary fetchOneSummary =
        new DynamicsFunctionBlockFetchOneSummary(dynamicsFunctionBlockId);

    try {
      entity =
          DynamicsFunctionBlockBusinessDelegate.getDynamicsFunctionBlockInstance()
              .getDynamicsFunctionBlock(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DynamicsFunctionBlockBusinessDelegate proxy =
          DynamicsFunctionBlockBusinessDelegate.getDynamicsFunctionBlockInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DynamicsFunctionBlockBusinessDelegate.getDynamicsFunctionBlockInstance()
          .delete(new DeleteDynamicsFunctionBlockCommand(entity.getDynamicsFunctionBlockId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted DynamicsFunctionBlock with id " + dynamicsFunctionBlockId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete DynamicsFunctionBlock with id " + dynamicsFunctionBlockId);
  ...
  protected List<DynamicsFunctionBlock> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of DynamicsFunctionBlock was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDynamicsFunctionBlockId(), "-- entity in Collection has a null primary key");
