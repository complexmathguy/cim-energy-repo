...
public class MeasurementValueSourceTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for MeasurementValueSource : "
                          + successValue.getMeasurementValueSourceId());
                  if (successValue.getMeasurementValueSourceId().equals(measurementValueSourceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfMeasurementValueSourceList - 1,
      ...
      subscriber
          .measurementValueSourceSubscribe(measurementValueSourceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for MeasurementValueSource : "
                          + successValue.getMeasurementValueSourceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on measurementValueSource for measurementValueSourceId consumed"));
  ...
  protected MeasurementValueSource read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read MeasurementValueSource with primary key");
    msg.append(measurementValueSourceId);

    MeasurementValueSourceFetchOneSummary fetchOneSummary =
        new MeasurementValueSourceFetchOneSummary(measurementValueSourceId);

    try {
      entity =
          MeasurementValueSourceBusinessDelegate.getMeasurementValueSourceInstance()
              .getMeasurementValueSource(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      MeasurementValueSourceBusinessDelegate proxy =
          MeasurementValueSourceBusinessDelegate.getMeasurementValueSourceInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read MeasurementValueSource with id " + measurementValueSourceId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read MeasurementValueSource with id " + measurementValueSourceId);
    ...
    try {
      MeasurementValueSourceBusinessDelegate.getMeasurementValueSourceInstance()
          .delete(new DeleteMeasurementValueSourceCommand(entity.getMeasurementValueSourceId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted MeasurementValueSource with id " + measurementValueSourceId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete MeasurementValueSource with id " + measurementValueSourceId);
  ...
  protected List<MeasurementValueSource> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          MeasurementValueSourceBusinessDelegate.getMeasurementValueSourceInstance()
              .getAllMeasurementValueSource();
      assertNotNull(
          collection, "An Empty collection of MeasurementValueSource was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getMeasurementValueSourceId(), "-- entity in Collection has a null primary key");
