...
public class PhaseTapChangerTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for PhaseTapChanger : "
                          + successValue.getPhaseTapChangerId());
                  if (successValue.getPhaseTapChangerId().equals(phaseTapChangerId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfPhaseTapChangerList - 1,
      ...
      subscriber
          .phaseTapChangerSubscribe(phaseTapChangerId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for PhaseTapChanger : "
                          + successValue.getPhaseTapChangerId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected PhaseTapChanger read() throws Throwable {
    ...
    PhaseTapChangerFetchOneSummary fetchOneSummary =
        new PhaseTapChangerFetchOneSummary(phaseTapChangerId);

    try {
      entity =
          PhaseTapChangerBusinessDelegate.getPhaseTapChangerInstance()
              .getPhaseTapChanger(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      PhaseTapChangerBusinessDelegate proxy =
          PhaseTapChangerBusinessDelegate.getPhaseTapChangerInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      PhaseTapChangerBusinessDelegate.getPhaseTapChangerInstance()
          .delete(new DeletePhaseTapChangerCommand(entity.getPhaseTapChangerId()))
          .get();
  ...
  protected List<PhaseTapChanger> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          PhaseTapChangerBusinessDelegate.getPhaseTapChangerInstance().getAllPhaseTapChanger();
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getPhaseTapChangerId(), "-- entity in Collection has a null primary key");
