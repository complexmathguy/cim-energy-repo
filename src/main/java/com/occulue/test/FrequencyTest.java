...
public class FrequencyTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    FrequencyBusinessDelegate.getFrequencyInstance().createFrequency(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Frequency : " + successValue.getFrequencyId());
                  if (successValue.getFrequencyId().equals(frequencyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfFrequencyList - 1,
      ...
      subscriber
          .frequencySubscribe(frequencyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Frequency : "
                          + successValue.getFrequencyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + frequencyId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      FrequencyBusinessDelegate.getFrequencyInstance()
          .delete(new DeleteFrequencyCommand(entity.getFrequencyId()))
          .get();
  ...
  protected CreateFrequencyCommand generateNewCommand() {
    CreateFrequencyCommand command =
        new CreateFrequencyCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateFrequencyCommand generateUpdateCommand() {
    UpdateFrequencyCommand command =
        new UpdateFrequencyCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
