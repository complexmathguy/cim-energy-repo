...
public class RotationSpeedTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for RotationSpeed : "
                          + successValue.getRotationSpeedId());
                  if (successValue.getRotationSpeedId().equals(rotationSpeedId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfRotationSpeedList - 1,
      ...
      subscriber
          .rotationSpeedSubscribe(rotationSpeedId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for RotationSpeed : "
                          + successValue.getRotationSpeedId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected RotationSpeed read() throws Throwable {
    ...
    RotationSpeedFetchOneSummary fetchOneSummary =
        new RotationSpeedFetchOneSummary(rotationSpeedId);

    try {
      entity =
          RotationSpeedBusinessDelegate.getRotationSpeedInstance()
              .getRotationSpeed(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      RotationSpeedBusinessDelegate proxy =
          RotationSpeedBusinessDelegate.getRotationSpeedInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      RotationSpeedBusinessDelegate.getRotationSpeedInstance()
          .delete(new DeleteRotationSpeedCommand(entity.getRotationSpeedId()))
          .get();
  ...
  protected List<RotationSpeed> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getRotationSpeedId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateRotationSpeedCommand generateNewCommand() {
    CreateRotationSpeedCommand command =
        new CreateRotationSpeedCommand(
            null,
  ...
  protected UpdateRotationSpeedCommand generateUpdateCommand() {
    UpdateRotationSpeedCommand command =
        new UpdateRotationSpeedCommand(
            null,
            UnitMultiplier.values()[0],
            UnitSymbol.values()[0],
            UnitMultiplier.values()[0],
