...
public class VoltageTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    voltageId =
        VoltageBusinessDelegate.getVoltageInstance().createVoltage(generateNewCommand()).get();
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Voltage : " + successValue.getVoltageId());
                  if (successValue.getVoltageId().equals(voltageId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfVoltageList - 1, "value not deleted from list");
                      LOGGER.info("**********************************************************");
      ...
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetOne update received for Voltage : "
                          + successValue.getVoltageId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + voltageId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      VoltageBusinessDelegate.getVoltageInstance()
          .delete(new DeleteVoltageCommand(entity.getVoltageId()))
          .get();
  ...
  protected CreateVoltageCommand generateNewCommand() {
    CreateVoltageCommand command =
        new CreateVoltageCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateVoltageCommand generateUpdateCommand() {
    UpdateVoltageCommand command =
        new UpdateVoltageCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
