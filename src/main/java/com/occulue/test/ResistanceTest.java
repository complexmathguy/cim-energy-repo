...
public class ResistanceTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    ResistanceBusinessDelegate.getResistanceInstance().createResistance(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Resistance : " + successValue.getResistanceId());
                  if (successValue.getResistanceId().equals(resistanceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfResistanceList - 1,
      ...
      subscriber
          .resistanceSubscribe(resistanceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Resistance : "
                          + successValue.getResistanceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ResistanceBusinessDelegate.getResistanceInstance()
          .delete(new DeleteResistanceCommand(entity.getResistanceId()))
          .get();
  ...
  protected CreateResistanceCommand generateNewCommand() {
    CreateResistanceCommand command =
        new CreateResistanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateResistanceCommand generateUpdateCommand() {
    UpdateResistanceCommand command =
        new UpdateResistanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
