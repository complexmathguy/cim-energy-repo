...
public class DiagramObjectTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for DiagramObject : "
                          + successValue.getDiagramObjectId());
                  if (successValue.getDiagramObjectId().equals(diagramObjectId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfDiagramObjectList - 1,
      ...
      subscriber
          .diagramObjectSubscribe(diagramObjectId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for DiagramObject : "
                          + successValue.getDiagramObjectId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected DiagramObject read() throws Throwable {
    ...
    DiagramObjectFetchOneSummary fetchOneSummary =
        new DiagramObjectFetchOneSummary(diagramObjectId);

    try {
      entity =
          DiagramObjectBusinessDelegate.getDiagramObjectInstance()
              .getDiagramObject(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      DiagramObjectBusinessDelegate proxy =
          DiagramObjectBusinessDelegate.getDiagramObjectInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      DiagramObjectBusinessDelegate.getDiagramObjectInstance()
          .delete(new DeleteDiagramObjectCommand(entity.getDiagramObjectId()))
          .get();
  ...
  protected List<DiagramObject> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getDiagramObjectId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateDiagramObjectCommand generateUpdateCommand() {
    UpdateDiagramObjectCommand command =
        new UpdateDiagramObjectCommand(null, null, null, null, null, null);
