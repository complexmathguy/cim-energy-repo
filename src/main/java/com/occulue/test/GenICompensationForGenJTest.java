...
public class GenICompensationForGenJTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for GenICompensationForGenJ : "
                          + successValue.getGenICompensationForGenJId());
                  if (successValue
                      .getGenICompensationForGenJId()
                      .equals(genICompensationForGenJId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfGenICompensationForGenJList - 1,
      ...
      subscriber
          .genICompensationForGenJSubscribe(genICompensationForGenJId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for GenICompensationForGenJ : "
                          + successValue.getGenICompensationForGenJId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on genICompensationForGenJ for genICompensationForGenJId consumed"));
  ...
  protected GenICompensationForGenJ read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read GenICompensationForGenJ with primary key");
    msg.append(genICompensationForGenJId);

    GenICompensationForGenJFetchOneSummary fetchOneSummary =
        new GenICompensationForGenJFetchOneSummary(genICompensationForGenJId);

    try {
      entity =
          GenICompensationForGenJBusinessDelegate.getGenICompensationForGenJInstance()
              .getGenICompensationForGenJ(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      GenICompensationForGenJBusinessDelegate proxy =
          GenICompensationForGenJBusinessDelegate.getGenICompensationForGenJInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read GenICompensationForGenJ with id " + genICompensationForGenJId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read GenICompensationForGenJ with id " + genICompensationForGenJId);
    ...
    try {
      GenICompensationForGenJBusinessDelegate.getGenICompensationForGenJInstance()
          .delete(new DeleteGenICompensationForGenJCommand(entity.getGenICompensationForGenJId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted GenICompensationForGenJ with id " + genICompensationForGenJId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete GenICompensationForGenJ with id " + genICompensationForGenJId);
  ...
  protected List<GenICompensationForGenJ> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          GenICompensationForGenJBusinessDelegate.getGenICompensationForGenJInstance()
              .getAllGenICompensationForGenJ();
      assertNotNull(
          collection, "An Empty collection of GenICompensationForGenJ was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getGenICompensationForGenJId(),
  ...
  protected UpdateGenICompensationForGenJCommand generateUpdateCommand() {
    UpdateGenICompensationForGenJCommand command =
        new UpdateGenICompensationForGenJCommand(null, null, null);
