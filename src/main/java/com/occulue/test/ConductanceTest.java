...
public class ConductanceTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Conductance : "
                          + successValue.getConductanceId());
                  if (successValue.getConductanceId().equals(conductanceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfConductanceList - 1,
      ...
      subscriber
          .conductanceSubscribe(conductanceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Conductance : "
                          + successValue.getConductanceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ConductanceBusinessDelegate.getConductanceInstance()
          .delete(new DeleteConductanceCommand(entity.getConductanceId()))
          .get();
  ...
  protected CreateConductanceCommand generateNewCommand() {
    CreateConductanceCommand command =
        new CreateConductanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateConductanceCommand generateUpdateCommand() {
    UpdateConductanceCommand command =
        new UpdateConductanceCommand(
