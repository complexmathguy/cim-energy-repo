...
public class ReactivePowerTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ReactivePower : "
                          + successValue.getReactivePowerId());
                  if (successValue.getReactivePowerId().equals(reactivePowerId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfReactivePowerList - 1,
      ...
      subscriber
          .reactivePowerSubscribe(reactivePowerId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ReactivePower : "
                          + successValue.getReactivePowerId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected ReactivePower read() throws Throwable {
    ...
    ReactivePowerFetchOneSummary fetchOneSummary =
        new ReactivePowerFetchOneSummary(reactivePowerId);

    try {
      entity =
          ReactivePowerBusinessDelegate.getReactivePowerInstance()
              .getReactivePower(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ReactivePowerBusinessDelegate proxy =
          ReactivePowerBusinessDelegate.getReactivePowerInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ReactivePowerBusinessDelegate.getReactivePowerInstance()
          .delete(new DeleteReactivePowerCommand(entity.getReactivePowerId()))
          .get();
  ...
  protected List<ReactivePower> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getReactivePowerId(), "-- entity in Collection has a null primary key");
  ...
  protected CreateReactivePowerCommand generateNewCommand() {
    CreateReactivePowerCommand command =
        new CreateReactivePowerCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateReactivePowerCommand generateUpdateCommand() {
    UpdateReactivePowerCommand command =
        new UpdateReactivePowerCommand(
