...
public class ValueAliasSetTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ValueAliasSet : "
                          + successValue.getValueAliasSetId());
                  if (successValue.getValueAliasSetId().equals(valueAliasSetId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfValueAliasSetList - 1,
      ...
      subscriber
          .valueAliasSetSubscribe(valueAliasSetId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ValueAliasSet : "
                          + successValue.getValueAliasSetId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected ValueAliasSet read() throws Throwable {
    ...
    ValueAliasSetFetchOneSummary fetchOneSummary =
        new ValueAliasSetFetchOneSummary(valueAliasSetId);

    try {
      entity =
          ValueAliasSetBusinessDelegate.getValueAliasSetInstance()
              .getValueAliasSet(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ValueAliasSetBusinessDelegate proxy =
          ValueAliasSetBusinessDelegate.getValueAliasSetInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ValueAliasSetBusinessDelegate.getValueAliasSetInstance()
          .delete(new DeleteValueAliasSetCommand(entity.getValueAliasSetId()))
          .get();
  ...
  protected List<ValueAliasSet> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getValueAliasSetId(), "-- entity in Collection has a null primary key");
