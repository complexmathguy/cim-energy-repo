...
public class WindContQIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindContQIEC : "
                          + successValue.getWindContQIECId());
                  if (successValue.getWindContQIECId().equals(windContQIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindContQIECList - 1,
      ...
      subscriber
          .windContQIECSubscribe(windContQIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindContQIEC : "
                          + successValue.getWindContQIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected WindContQIEC read() throws Throwable {
    ...
    try {
      entity =
          WindContQIECBusinessDelegate.getWindContQIECInstance().getWindContQIEC(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindContQIECBusinessDelegate.getWindContQIECInstance()
          .delete(new DeleteWindContQIECCommand(entity.getWindContQIECId()))
          .get();
  ...
  protected CreateWindContQIECCommand generateNewCommand() {
    CreateWindContQIECCommand command =
        new CreateWindContQIECCommand(
            null, WindLVRTQcontrolModesKind.values()[0], WindQcontrolModesKind.values()[0]);
  ...
  protected UpdateWindContQIECCommand generateUpdateCommand() {
    UpdateWindContQIECCommand command =
        new UpdateWindContQIECCommand(
            null,
            null,
            null,
            null,
            ...
            null,
            null,
            WindLVRTQcontrolModesKind.values()[0],
            WindQcontrolModesKind.values()[0],
            null);
