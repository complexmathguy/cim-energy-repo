...
public class CsConverterTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for CsConverter : "
                          + successValue.getCsConverterId());
                  if (successValue.getCsConverterId().equals(csConverterId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfCsConverterList - 1,
      ...
      subscriber
          .csConverterSubscribe(csConverterId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for CsConverter : "
                          + successValue.getCsConverterId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      CsConverterBusinessDelegate.getCsConverterInstance()
          .delete(new DeleteCsConverterCommand(entity.getCsConverterId()))
          .get();
  ...
  protected UpdateCsConverterCommand generateUpdateCommand() {
    UpdateCsConverterCommand command =
        new UpdateCsConverterCommand(null, null, null, null, null, null, null, null);
