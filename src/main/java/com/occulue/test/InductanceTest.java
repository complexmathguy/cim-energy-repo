...
public class InductanceTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    InductanceBusinessDelegate.getInductanceInstance().createInductance(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for Inductance : " + successValue.getInductanceId());
                  if (successValue.getInductanceId().equals(inductanceId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfInductanceList - 1,
      ...
      subscriber
          .inductanceSubscribe(inductanceId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for Inductance : "
                          + successValue.getInductanceId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      InductanceBusinessDelegate.getInductanceInstance()
          .delete(new DeleteInductanceCommand(entity.getInductanceId()))
          .get();
  ...
  protected CreateInductanceCommand generateNewCommand() {
    CreateInductanceCommand command =
        new CreateInductanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0]);
  ...
  protected UpdateInductanceCommand generateUpdateCommand() {
    UpdateInductanceCommand command =
        new UpdateInductanceCommand(null, UnitMultiplier.values()[0], UnitSymbol.values()[0], null);
