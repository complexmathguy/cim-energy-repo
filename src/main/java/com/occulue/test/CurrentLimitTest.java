...
public class CurrentLimitTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for CurrentLimit : "
                          + successValue.getCurrentLimitId());
                  if (successValue.getCurrentLimitId().equals(currentLimitId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfCurrentLimitList - 1,
      ...
      subscriber
          .currentLimitSubscribe(currentLimitId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for CurrentLimit : "
                          + successValue.getCurrentLimitId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected CurrentLimit read() throws Throwable {
    ...
    try {
      entity =
          CurrentLimitBusinessDelegate.getCurrentLimitInstance().getCurrentLimit(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      CurrentLimitBusinessDelegate.getCurrentLimitInstance()
          .delete(new DeleteCurrentLimitCommand(entity.getCurrentLimitId()))
          .get();
