...
public class SvVoltageTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    SvVoltageBusinessDelegate.getSvVoltageInstance().createSvVoltage(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for SvVoltage : " + successValue.getSvVoltageId());
                  if (successValue.getSvVoltageId().equals(svVoltageId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSvVoltageList - 1,
      ...
      subscriber
          .svVoltageSubscribe(svVoltageId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for SvVoltage : "
                          + successValue.getSvVoltageId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          msg.toString() + " : primarykey = " + svVoltageId + " : command -" + command + " : " + e);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SvVoltageBusinessDelegate.getSvVoltageInstance()
          .delete(new DeleteSvVoltageCommand(entity.getSvVoltageId()))
          .get();
  ...
  protected UpdateSvVoltageCommand generateUpdateCommand() {
    UpdateSvVoltageCommand command = new UpdateSvVoltageCommand(null, null, null);
