...
public class IntegerProxyTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for IntegerProxy : "
                          + successValue.getIntegerProxyId());
                  if (successValue.getIntegerProxyId().equals(integerProxyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfIntegerProxyList - 1,
      ...
      subscriber
          .integerProxySubscribe(integerProxyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for IntegerProxy : "
                          + successValue.getIntegerProxyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected IntegerProxy read() throws Throwable {
    ...
    try {
      entity =
          IntegerProxyBusinessDelegate.getIntegerProxyInstance().getIntegerProxy(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      IntegerProxyBusinessDelegate.getIntegerProxyInstance()
          .delete(new DeleteIntegerProxyCommand(entity.getIntegerProxyId()))
          .get();
