...
public class EnergyAreaTest {
  ...
  protected void jumpStart() throws Throwable {
    ...
    EnergyAreaBusinessDelegate.getEnergyAreaInstance().createEnergyArea(generateNewCommand()).get();
  }
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for EnergyArea : " + successValue.getEnergyAreaId());
                  if (successValue.getEnergyAreaId().equals(energyAreaId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfEnergyAreaList - 1,
      ...
      subscriber
          .energyAreaSubscribe(energyAreaId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for EnergyArea : "
                          + successValue.getEnergyAreaId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      EnergyAreaBusinessDelegate.getEnergyAreaInstance()
          .delete(new DeleteEnergyAreaCommand(entity.getEnergyAreaId()))
          .get();
