...
public class ReportingGroupTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ReportingGroup : "
                          + successValue.getReportingGroupId());
                  if (successValue.getReportingGroupId().equals(reportingGroupId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfReportingGroupList - 1,
      ...
      subscriber
          .reportingGroupSubscribe(reportingGroupId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ReportingGroup : "
                          + successValue.getReportingGroupId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected ReportingGroup read() throws Throwable {
    ...
    ReportingGroupFetchOneSummary fetchOneSummary =
        new ReportingGroupFetchOneSummary(reportingGroupId);

    try {
      entity =
          ReportingGroupBusinessDelegate.getReportingGroupInstance()
              .getReportingGroup(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ReportingGroupBusinessDelegate proxy =
          ReportingGroupBusinessDelegate.getReportingGroupInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ReportingGroupBusinessDelegate.getReportingGroupInstance()
          .delete(new DeleteReportingGroupCommand(entity.getReportingGroupId()))
          .get();
  ...
  protected List<ReportingGroup> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getReportingGroupId(), "-- entity in Collection has a null primary key");
