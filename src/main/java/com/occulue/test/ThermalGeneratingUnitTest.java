...
public class ThermalGeneratingUnitTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ThermalGeneratingUnit : "
                          + successValue.getThermalGeneratingUnitId());
                  if (successValue.getThermalGeneratingUnitId().equals(thermalGeneratingUnitId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfThermalGeneratingUnitList - 1,
      ...
      subscriber
          .thermalGeneratingUnitSubscribe(thermalGeneratingUnitId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ThermalGeneratingUnit : "
                          + successValue.getThermalGeneratingUnitId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on thermalGeneratingUnit for thermalGeneratingUnitId consumed"));
  ...
  protected ThermalGeneratingUnit read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read ThermalGeneratingUnit with primary key");
    msg.append(thermalGeneratingUnitId);

    ThermalGeneratingUnitFetchOneSummary fetchOneSummary =
        new ThermalGeneratingUnitFetchOneSummary(thermalGeneratingUnitId);

    try {
      entity =
          ThermalGeneratingUnitBusinessDelegate.getThermalGeneratingUnitInstance()
              .getThermalGeneratingUnit(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ThermalGeneratingUnitBusinessDelegate proxy =
          ThermalGeneratingUnitBusinessDelegate.getThermalGeneratingUnitInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ThermalGeneratingUnitBusinessDelegate.getThermalGeneratingUnitInstance()
          .delete(new DeleteThermalGeneratingUnitCommand(entity.getThermalGeneratingUnitId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted ThermalGeneratingUnit with id " + thermalGeneratingUnitId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete ThermalGeneratingUnit with id " + thermalGeneratingUnitId);
  ...
  protected List<ThermalGeneratingUnit> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          ThermalGeneratingUnitBusinessDelegate.getThermalGeneratingUnitInstance()
              .getAllThermalGeneratingUnit();
      assertNotNull(
          collection, "An Empty collection of ThermalGeneratingUnit was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getThermalGeneratingUnitId(), "-- entity in Collection has a null primary key");
