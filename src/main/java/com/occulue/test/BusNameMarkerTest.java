...
public class BusNameMarkerTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for BusNameMarker : "
                          + successValue.getBusNameMarkerId());
                  if (successValue.getBusNameMarkerId().equals(busNameMarkerId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfBusNameMarkerList - 1,
      ...
      subscriber
          .busNameMarkerSubscribe(busNameMarkerId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for BusNameMarker : "
                          + successValue.getBusNameMarkerId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected BusNameMarker read() throws Throwable {
    ...
    BusNameMarkerFetchOneSummary fetchOneSummary =
        new BusNameMarkerFetchOneSummary(busNameMarkerId);

    try {
      entity =
          BusNameMarkerBusinessDelegate.getBusNameMarkerInstance()
              .getBusNameMarker(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      BusNameMarkerBusinessDelegate proxy =
          BusNameMarkerBusinessDelegate.getBusNameMarkerInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      BusNameMarkerBusinessDelegate.getBusNameMarkerInstance()
          .delete(new DeleteBusNameMarkerCommand(entity.getBusNameMarkerId()))
          .get();
  ...
  protected List<BusNameMarker> getAll() throws Throwable {
    ...
    try {
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getBusNameMarkerId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateBusNameMarkerCommand generateUpdateCommand() {
    UpdateBusNameMarkerCommand command = new UpdateBusNameMarkerCommand(null, null);
