...
public class ActivePowerPerFrequencyTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ActivePowerPerFrequency : "
                          + successValue.getActivePowerPerFrequencyId());
                  if (successValue
                      .getActivePowerPerFrequencyId()
                      .equals(activePowerPerFrequencyId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfActivePowerPerFrequencyList - 1,
      ...
      subscriber
          .activePowerPerFrequencySubscribe(activePowerPerFrequencyId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ActivePowerPerFrequency : "
                          + successValue.getActivePowerPerFrequencyId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on activePowerPerFrequency for activePowerPerFrequencyId consumed"));
  ...
  protected ActivePowerPerFrequency read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read ActivePowerPerFrequency with primary key");
    msg.append(activePowerPerFrequencyId);

    ActivePowerPerFrequencyFetchOneSummary fetchOneSummary =
        new ActivePowerPerFrequencyFetchOneSummary(activePowerPerFrequencyId);

    try {
      entity =
          ActivePowerPerFrequencyBusinessDelegate.getActivePowerPerFrequencyInstance()
              .getActivePowerPerFrequency(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      ActivePowerPerFrequencyBusinessDelegate proxy =
          ActivePowerPerFrequencyBusinessDelegate.getActivePowerPerFrequencyInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read ActivePowerPerFrequency with id " + activePowerPerFrequencyId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read ActivePowerPerFrequency with id " + activePowerPerFrequencyId);
    ...
    try {
      ActivePowerPerFrequencyBusinessDelegate.getActivePowerPerFrequencyInstance()
          .delete(new DeleteActivePowerPerFrequencyCommand(entity.getActivePowerPerFrequencyId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted ActivePowerPerFrequency with id " + activePowerPerFrequencyId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete ActivePowerPerFrequency with id " + activePowerPerFrequencyId);
  ...
  protected List<ActivePowerPerFrequency> getAll() throws Throwable {
    ...
    try {
      ...
      collection =
          ActivePowerPerFrequencyBusinessDelegate.getActivePowerPerFrequencyInstance()
              .getAllActivePowerPerFrequency();
      assertNotNull(
          collection, "An Empty collection of ActivePowerPerFrequency was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getActivePowerPerFrequencyId(),
  ...
  protected CreateActivePowerPerFrequencyCommand generateNewCommand() {
    CreateActivePowerPerFrequencyCommand command =
        new CreateActivePowerPerFrequencyCommand(
            null,
  ...
  protected UpdateActivePowerPerFrequencyCommand generateUpdateCommand() {
    UpdateActivePowerPerFrequencyCommand command =
        new UpdateActivePowerPerFrequencyCommand(
            null,
            UnitMultiplier.values()[0],
            UnitSymbol.values()[0],
            UnitMultiplier.values()[0],
