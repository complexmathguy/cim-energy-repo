...
public class WindContRotorRIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindContRotorRIEC : "
                          + successValue.getWindContRotorRIECId());
                  if (successValue.getWindContRotorRIECId().equals(windContRotorRIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindContRotorRIECList - 1,
      ...
      subscriber
          .windContRotorRIECSubscribe(windContRotorRIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindContRotorRIEC : "
                          + successValue.getWindContRotorRIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on windContRotorRIEC for windContRotorRIECId consumed"));
  ...
  protected WindContRotorRIEC read() throws Throwable {
    ...
    WindContRotorRIECFetchOneSummary fetchOneSummary =
        new WindContRotorRIECFetchOneSummary(windContRotorRIECId);

    try {
      entity =
          WindContRotorRIECBusinessDelegate.getWindContRotorRIECInstance()
              .getWindContRotorRIEC(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      WindContRotorRIECBusinessDelegate proxy =
          WindContRotorRIECBusinessDelegate.getWindContRotorRIECInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindContRotorRIECBusinessDelegate.getWindContRotorRIECInstance()
          .delete(new DeleteWindContRotorRIECCommand(entity.getWindContRotorRIECId()))
          .get();
  ...
  protected List<WindContRotorRIEC> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of WindContRotorRIEC was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getWindContRotorRIECId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateWindContRotorRIECCommand generateUpdateCommand() {
    UpdateWindContRotorRIECCommand command =
        new UpdateWindContRotorRIECCommand(null, null, null, null, null, null, null, null, null);
