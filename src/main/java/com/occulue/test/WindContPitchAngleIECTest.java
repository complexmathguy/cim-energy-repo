...
public class WindContPitchAngleIECTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for WindContPitchAngleIEC : "
                          + successValue.getWindContPitchAngleIECId());
                  if (successValue.getWindContPitchAngleIECId().equals(windContPitchAngleIECId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfWindContPitchAngleIECList - 1,
      ...
      subscriber
          .windContPitchAngleIECSubscribe(windContPitchAngleIECId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for WindContPitchAngleIEC : "
                          + successValue.getWindContPitchAngleIECId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on windContPitchAngleIEC for windContPitchAngleIECId consumed"));
  ...
  protected WindContPitchAngleIEC read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read WindContPitchAngleIEC with primary key");
    msg.append(windContPitchAngleIECId);

    WindContPitchAngleIECFetchOneSummary fetchOneSummary =
        new WindContPitchAngleIECFetchOneSummary(windContPitchAngleIECId);

    try {
      entity =
          WindContPitchAngleIECBusinessDelegate.getWindContPitchAngleIECInstance()
              .getWindContPitchAngleIEC(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      WindContPitchAngleIECBusinessDelegate proxy =
          WindContPitchAngleIECBusinessDelegate.getWindContPitchAngleIECInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      WindContPitchAngleIECBusinessDelegate.getWindContPitchAngleIECInstance()
          .delete(new DeleteWindContPitchAngleIECCommand(entity.getWindContPitchAngleIECId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted WindContPitchAngleIEC with id " + windContPitchAngleIECId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete WindContPitchAngleIEC with id " + windContPitchAngleIECId);
  ...
  protected List<WindContPitchAngleIEC> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of WindContPitchAngleIEC was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getWindContPitchAngleIECId(), "-- entity in Collection has a null primary key");
  ...
  protected UpdateWindContPitchAngleIECCommand generateUpdateCommand() {
    UpdateWindContPitchAngleIECCommand command =
        new UpdateWindContPitchAngleIECCommand(
