...
public class ValueToAliasTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for ValueToAlias : "
                          + successValue.getValueToAliasId());
                  if (successValue.getValueToAliasId().equals(valueToAliasId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfValueToAliasList - 1,
      ...
      subscriber
          .valueToAliasSubscribe(valueToAliasId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for ValueToAlias : "
                          + successValue.getValueToAliasId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected ValueToAlias read() throws Throwable {
    ...
    try {
      entity =
          ValueToAliasBusinessDelegate.getValueToAliasInstance().getValueToAlias(fetchOneSummary);
  ...
  protected void delete() throws Throwable {
    ...
    try {
      ValueToAliasBusinessDelegate.getValueToAliasInstance()
          .delete(new DeleteValueToAliasCommand(entity.getValueToAliasId()))
          .get();
  ...
  protected UpdateValueToAliasCommand generateUpdateCommand() {
    UpdateValueToAliasCommand command = new UpdateValueToAliasCommand(null, null);
