...
public class PhaseTapChangerTabularTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for PhaseTapChangerTabular : "
                          + successValue.getPhaseTapChangerTabularId());
                  if (successValue.getPhaseTapChangerTabularId().equals(phaseTapChangerTabularId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfPhaseTapChangerTabularList - 1,
      ...
      subscriber
          .phaseTapChangerTabularSubscribe(phaseTapChangerTabularId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for PhaseTapChangerTabular : "
                          + successValue.getPhaseTapChangerTabularId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
              ...
              () ->
                  LOGGER.info(
                      "Subscription on phaseTapChangerTabular for phaseTapChangerTabularId consumed"));
  ...
  protected PhaseTapChangerTabular read() throws Throwable {
    ...
    StringBuilder msg =
        new StringBuilder("-- Failed to read PhaseTapChangerTabular with primary key");
    msg.append(phaseTapChangerTabularId);

    PhaseTapChangerTabularFetchOneSummary fetchOneSummary =
        new PhaseTapChangerTabularFetchOneSummary(phaseTapChangerTabularId);

    try {
      entity =
          PhaseTapChangerTabularBusinessDelegate.getPhaseTapChangerTabularInstance()
              .getPhaseTapChangerTabular(fetchOneSummary);
  ...
  protected void update() throws Throwable {
    ...
    try {
      ...
      PhaseTapChangerTabularBusinessDelegate proxy =
          PhaseTapChangerTabularBusinessDelegate.getPhaseTapChangerTabularInstance();
  ...
  protected void delete() throws Throwable {
    ...
    try {
      entity = read();
      LOGGER.info(
          "-- Successfully read PhaseTapChangerTabular with id " + phaseTapChangerTabularId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to read PhaseTapChangerTabular with id " + phaseTapChangerTabularId);
    ...
    try {
      PhaseTapChangerTabularBusinessDelegate.getPhaseTapChangerTabularInstance()
          .delete(new DeletePhaseTapChangerTabularCommand(entity.getPhaseTapChangerTabularId()))
          .get();
      LOGGER.info(
          "-- Successfully deleted PhaseTapChangerTabular with id " + phaseTapChangerTabularId);
    } catch (Throwable e) {
      LOGGER.warning(unexpectedErrorMsg);
      LOGGER.warning(
          "-- Failed to delete PhaseTapChangerTabular with id " + phaseTapChangerTabularId);
  ...
  protected List<PhaseTapChangerTabular> getAll() throws Throwable {
    ...
    try {
      ...
      assertNotNull(
          collection, "An Empty collection of PhaseTapChangerTabular was incorrectly returned.");
      ...
      while (iter.hasNext()) {
        ...
        assertNotNull(
            entity.getPhaseTapChangerTabularId(), "-- entity in Collection has a null primary key");
