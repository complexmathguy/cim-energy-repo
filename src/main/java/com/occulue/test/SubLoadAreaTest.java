...
public class SubLoadAreaTest {
  ...
  protected void setUpQuerySubscriptions() throws Throwable {
    ...
    try {
      subscriber
          ...
          .subscribe(
              successValue -> {
                ...
                try {
                  LOGGER.info(
                      "GetAll update received for SubLoadArea : "
                          + successValue.getSubLoadAreaId());
                  if (successValue.getSubLoadAreaId().equals(subLoadAreaId)) {
                    if (testingStep.equals("create")) {
                      ...
                      state(
                          getAll().size() == sizeOfSubLoadAreaList - 1,
      ...
      subscriber
          .subLoadAreaSubscribe(subLoadAreaId)
          .updates()
          .subscribe(
              successValue -> {
                LOGGER.info(successValue.toString());
                try {
                  LOGGER.info(
                      "GetOne update received for SubLoadArea : "
                          + successValue.getSubLoadAreaId()
                          + " in step "
                          + testingStep);
                  testingStep = "delete";
  ...
  protected void delete() throws Throwable {
    ...
    try {
      SubLoadAreaBusinessDelegate.getSubLoadAreaInstance()
          .delete(new DeleteSubLoadAreaCommand(entity.getSubLoadAreaId()))
          .get();
